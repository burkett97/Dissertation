Ext.define("Ext.draw.ContainerBase", {
    ...
    addElementListener: function() {
        var b = this,
            a = arguments;
        if (b.rendered) { b.el.on.apply(b.el, a) } else { b.on("render", function() { b.el.on.apply(b.el, a) }) }
    },
    removeElementListener: function() {
        var b = this,
            a = arguments;
        if (b.rendered) { b.el.un.apply(b.el, a) }
    },
    ...
    getItems: function() {
        var b = this,
            a = b.items;
        if (!a || !a.isMixedCollection) { b.initItems() }
        return b.items
    },
    onRender: function() {
        this.callParent(arguments);
    ...
    setItems: function(a) { this.items = a; return a },
    ...
    preview: function() {
        var b = this.getImage(),
            a;
        if (Ext.isIE8) { return false }
        if (b.type === "svg-markup") { a = { xtype: "container", html: b.data } } else {
            a = {
                xtype: "image",
                mode: "img",
        ...
        new Ext.window.Window({ title: this.previewTitleText, closeable: true, renderTo: Ext.getBody(), autoShow: true, maximizeable: true, maximized: true, border: true, layout: { type: "hbox", pack: "center", align: "middle" }, items: { xtype: "container", items: a } })
    },
    privates: {
        getTargetEl: function() { return this.innerElement },
        reattachToBody: function() {
            var a = this;
            if (a.pendingDetachSize) { a.handleResize() }
...
Ext.define("Ext.draw.sprite.AnimationParser", function() {
    function a(d, c, b) { return d + (c - d) * b }
    return {
        singleton: true,
        ...
        color: {
            parseInitial: function(c, b) {
                if (Ext.isString(c)) { c = Ext.util.Color.create(c) }
                if (Ext.isString(b)) { b = Ext.util.Color.create(b) }
            ...
            compute: function(d, c, b) { if (!Ext.isArray(d) || !Ext.isArray(c)) { return c || d } else { return [a(d[0], c[0], b), a(d[1], c[1], b), a(d[2], c[2], b), a(d[3], c[3], b)] } },
            serve: function(c) { var b = Ext.util.Color.fly(c[0], c[1], c[2], c[3]); return b.toString() }
        },
        number: { parse: function(b) { return b === null ? null : +b }, compute: function(d, c, b) { if (!Ext.isNumber(d) || !Ext.isNumber(c)) { return c || d } else { return a(d, c, b) } } },
        angle: { parseInitial: function(c, b) { if (b - c > Math.PI) { b -= Math.PI * 2 } else { if (b - c < -Math.PI) { b += Math.PI * 2 } } return [c, b] }, compute: function(d, c, b) { if (!Ext.isNumber(d) || !Ext.isNumber(c)) { return c || d } else { return a(d, c, b) } } },
        path: {
            parseInitial: function(m, n) {
                ...
                for (e = k; e < p; e++) { c.push(c[k - 1].slice(0)) }
                for (e = p; e < k; e++) { o.push(l.slice(0)) }
                b = c.length;
                o.path = n;
                ...
                for (e = 0; e < b; e++) {
                    ...
                    for (d = p; d < k; d += 6) { f.push(l[0], l[1], l[0], l[1], l[0], l[1]) }
                    g = o[o.length - 1];
                    l = [g[g.length - 2], g[g.length - 1]];
                    for (d = k; d < p; d += 6) { h.push(l[0], l[1], l[0], l[1], l[0], l[1]) }
                    for (e = 0; e < f.length; e++) { f[e] -= h[e] }
                    for (e = 2; e < f.length; e += 6) { o.temp.commands.push("C") }
                }
                return [c, o]
            },
            compute: function(c, l, m) {
                if (m >= 1) { return l.path }
                var e = 0,
                    f = c.length,
                    d = 0,
        ...
        text: { compute: function(d, c, b) { return d.substr(0, Math.round(d.length * (1 - b))) + c.substr(Math.round(c.length * (1 - b))) } },
        limited: "number",
        limited01: "number"
    }
});
(function() {
    if (!Ext.global.Float32Array) {
        var a = function(d) { if (typeof d === "number") { this.length = d } else { if ("length" in d) { this.length = d.length; for (var c = 0, b = d.length; c < b; c++) { this[c] = +d[c] } } } };
        a.prototype = [];
...
Ext.define("Ext.draw.Draw", {
    ...
    radian: Math.PI / 180,
    pi2: Math.PI * 2,
    reflectFn: function(b) { return b },
    rad: function(a) { return (a % 360) * this.radian },
    degrees: function(a) { return (a / this.radian) % 360 },
    isBBoxIntersect: function(b, a, c) { c = c || 0; return (Math.max(b.x, a.x) - c > Math.min(b.x + b.width, a.x + a.width)) || (Math.max(b.y, a.y) - c > Math.min(b.y + b.height, a.y + a.height)) },
    isPointInBBox: function(a, c, b) { return !!b && a >= b.x && a <= (b.x + b.width) && c >= b.y && c <= (b.y + b.height) },
    spline: function(m) {
        var e, c, k = m.length,
            b, h, l, f, a = 0,
    ...
    beginUpdateIOS: Ext.os.is.iOS ? function() { this.iosUpdateEl = Ext.getBody().createChild({ style: "position: absolute; top: 0px; bottom: 0px; left: 0px; right: 0px; background: rgba(0,0,0,0.001); z-index: 100000" }) } : Ext.emptyFn,
    endUpdateIOS: function() { this.iosUpdateEl = Ext.destroy(this.iosUpdateEl) }
});
Ext.define("Ext.draw.gradient.Gradient", {
    ...
    applyStops: function(f) {
        ...
        for (c = 0; c < d; c++) {
            b = f[c];
            a = b.color;
            if (!(a && a.isColor)) { a = Ext.util.Color.fly(a || Ext.util.Color.NONE) }
            e.push({ offset: Math.min(1, Math.max(0, "offset" in b ? b.offset : b.position || 0)), color: a.toString() })
        }
        e.sort(function(h, g) { return h.offset - g.offset });
        return e
    },
    onClassExtended: function(a, b) { if (!b.alias && b.type) { b.alias = "gradient." + b.type } },
...
Ext.define("Ext.draw.gradient.GradientDefinition", {
    ...
    add: function(a) {
        var b = this.gradients,
            c, e, d;
        for (c = 0, e = a.length; c < e; c++) { d = a[c]; if (Ext.isString(d.id)) { b[d.id] = d } }
    },
    get: function(d) {
        var a = this.gradients,
            b = d.match(this.urlStringRe),
            c;
        if (b && b[1] && (c = a[b[1]])) { return c || d }
        return d
    }
});
Ext.define("Ext.draw.sprite.AttributeParser", {
    ...
    number: function(a) { if (Ext.isNumber(+a)) { return a } },
    angle: function(a) { if (Ext.isNumber(a)) { a %= Math.PI * 2; if (a < -Math.PI) { a += Math.PI * 2 } else { if (a >= Math.PI) { a -= Math.PI * 2 } } return a } },
    data: function(a) { if (Ext.isArray(a)) { return a.slice() } else { if (a instanceof Float32Array) { return new Float32Array(a) } } },
    bool: function(a) { return !!a },
    color: function(a) { if (a && a.isColor) { return a.toString() } else { if (a && a.isGradient) { return a } else { if (!a) { return Ext.util.Color.NONE } else { if (Ext.isString(a)) { if (a.substr(0, 3) === "url") { a = Ext.draw.gradient.GradientDefinition.get(a); if (Ext.isString(a)) { return a } } else { return Ext.util.Color.fly(a).toString() } } } } } if (a.type === "linear") { return Ext.create("Ext.draw.gradient.Linear", a) } else { if (a.type === "radial") { return Ext.create("Ext.draw.gradient.Radial", a) } else { if (a.type === "pattern") { return Ext.create("Ext.draw.gradient.Pattern", a) } else { return Ext.util.Color.NONE } } } },
    limited: function(a, b) { return function(c) { c = +c; return Ext.isNumber(c) ? Math.min(Math.max(c, a), b) : undefined } },
    limited01: function(a) { a = +a; return Ext.isNumber(a) ? Math.min(Math.max(a, 0), 1) : undefined },
    enums: function() {
        var d = {},
            a = Array.prototype.slice.call(arguments, 0),
            b, c;
        for (b = 0, c = a.length; b < c; b++) { d[a[b]] = true }
        return function(e) { return e in d ? e : undefined }
    }
});
Ext.define("Ext.draw.sprite.AttributeDefinition", {
    ...
    applyDefaults: function(b, a) { a = Ext.apply(a || {}, this.normalize(b)); return a },
    applyAliases: function(b, a) { return Ext.apply(a || {}, b) },
    applyProcessors: function(e, i) {
        ...
        for (b in e) {
            ...
            if (typeof f === "string") {
                c = f.match(a);
                if (c) { f = h[c[1]].apply(h, c[2].split(",")) } else {
                    if (h[f]) {
                        g[b] = f;
                        d = true;
                        f = h[f]
        ...
        if (d) { this.setAnimationProcessors(g) }
        return j
    },
    applyAnimationProcessors: function(c, a) {
        ...
        for (b in c) {
            d = c[b];
            if (d === "none") { a[b] = null } else {
                if (Ext.isString(d) && !(b in a)) {
                    if (d in e) {
                        while (Ext.isString(e[d])) { d = e[d] }
                        a[b] = e[d]
                    }
                } else { if (Ext.isObject(d)) { a[b] = d } }
            }
        }
        return a
    },
    updateDirtyTriggers: function(a) { this.setTriggers(a) },
    applyTriggers: function(b, c) { if (!c) { c = {} } for (var a in b) { c[a] = b[a].split(",") } return c },
    applyUpdaters: function(b, a) { return Ext.apply(a || {}, b) },
    batchedNormalize: function(f, n) {
        ...
        var j = this.getProcessors(),
            ...
            o = {},
            g, h, b, e, p, c, m, l, k;
        if ("rotation" in f) { p = f.rotation } else { p = ("rotate" in f) ? f.rotate : undefined }
        if ("scaling" in f) { c = f.scaling } else { c = ("scale" in f) ? f.scale : undefined }
        if (typeof c !== "undefined") {
            if (Ext.isNumber(c)) {
                o.scalingX = c;
                o.scalingY = c
            } else { if ("x" in c) { o.scalingX = c.x } if ("y" in c) { o.scalingY = c.y } if ("centerX" in c) { o.scalingCenterX = c.centerX } if ("centerY" in c) { o.scalingCenterY = c.centerY } }
        }
        if (typeof p !== "undefined") {
            if (Ext.isNumber(p)) {
                p = Ext.draw.Draw.rad(p);
                o.rotationRads = p
            } else { if ("rads" in p) { o.rotationRads = p.rads } else { if ("degrees" in p) { if (Ext.isArray(p.degrees)) { o.rotationRads = Ext.Array.map(p.degrees, function(i) { return Ext.draw.Draw.rad(i) }) } else { o.rotationRads = Ext.draw.Draw.rad(p.degrees) } } } if ("centerX" in p) { o.rotationCenterX = p.centerX } if ("centerY" in p) { o.rotationCenterY = p.centerY } }
        }
        if (typeof a !== "undefined") { if ("x" in a) { o.translationX = a.x } if ("y" in a) { o.translationY = a.y } }
        if ("matrix" in f) {
        ...
        for (b in f) { e = f[b]; if (typeof e === "undefined") { continue } else { if (Ext.isArray(e)) { if (b in d) { b = d[b] } if (b in j) { o[b] = []; for (g = 0, h = e.length; g < h; g++) { l = j[b].call(this, e[g]); if (typeof l !== "undefined") { o[b][g] = l } } } else { if (n) { o[b] = e } } } else { if (b in d) { b = d[b] } if (b in j) { e = j[b].call(this, e); if (typeof e !== "undefined") { o[b] = e } } else { if (n) { o[b] = e } } } } }
        return o
    },
    normalize: function(i, j) {
        ...
        var f = this.getProcessors(),
            ...
            k = {},
            b, e, l, c, h, g;
        if ("rotation" in i) { l = i.rotation } else { l = ("rotate" in i) ? i.rotate : undefined }
        if ("scaling" in i) { c = i.scaling } else { c = ("scale" in i) ? i.scale : undefined }
        if (a) { if ("x" in a) { k.translationX = a.x } if ("y" in a) { k.translationY = a.y } }
        if (typeof c !== "undefined") {
            if (Ext.isNumber(c)) {
                k.scalingX = c;
                k.scalingY = c
            } else { if ("x" in c) { k.scalingX = c.x } if ("y" in c) { k.scalingY = c.y } if ("centerX" in c) { k.scalingCenterX = c.centerX } if ("centerY" in c) { k.scalingCenterY = c.centerY } }
        }
        if (typeof l !== "undefined") {
            if (Ext.isNumber(l)) {
                l = Ext.draw.Draw.rad(l);
                k.rotationRads = l
            } else { if ("rads" in l) { k.rotationRads = l.rads } else { if ("degrees" in l) { k.rotationRads = Ext.draw.Draw.rad(l.degrees) } } if ("centerX" in l) { k.rotationCenterX = l.centerX } if ("centerY" in l) { k.rotationCenterY = l.centerY } }
        }
        if ("matrix" in i) {
        ...
        for (b in i) { e = i[b]; if (typeof e === "undefined") { continue } if (b in d) { b = d[b] } if (b in f) { e = f[b].call(this, e); if (typeof e !== "undefined") { k[b] = e } } else { if (j) { k[b] = e } } }
        return k
    },
    setBypassingNormalization: function(a, c, b) { return c.pushDown(a, b) },
    set: function(a, c, b) { b = this.normalize(b); return this.setBypassingNormalization(a, c, b) }
});
Ext.define("Ext.draw.Matrix", {
    ...
    statics: {
        ...
        createPanZoomFromTwoPair: function(q, e, p, c, h, s, n, g) {
            if (arguments.length === 2) { return this.createPanZoomFromTwoPair.apply(this, q.concat(e)) }
            var k = p - q,
                j = c - e,
                d = (q + p) * 0.5,
                b = (e + c) * 0.5,
        ...
        fly: (function() {
            var a = null,
                b = function(c) { a.elements = c; return a };
            return function(c) {
                if (!a) { a = new Ext.draw.Matrix() }
                a.elements = c;
                Ext.draw.Matrix.fly = b;
                return a
            }
        })(),
        create: function(a) { if (a instanceof this) { return a } return new this(a) }
    },
    constructor: function(e, d, a, f, c, b) { if (e && e.length === 6) { this.elements = e.slice() } else { if (e !== undefined) { this.elements = [e, d, a, f, c, b] } else { this.elements = [1, 0, 0, 1, 0, 0] } } },
    ...
    prependMatrix: function(a) { return this.prepend.apply(this, a.elements) },
    ...
    appendMatrix: function(a) { return this.append.apply(this, a.elements) },
    ...
    inverse: function(i) {
        ...
        l *= n;
        k *= n;
        if (i) { i.set(k, -m, -l, o, l * h - k * j, m * j - o * h); return i } else { return new Ext.draw.Matrix(k, -m, -l, o, l * h - k * j, m * j - o * h) }
    },
    translate: function(a, c, b) { if (b) { return this.prepend(1, 0, 0, 1, a, c) } else { return this.append(1, 0, 0, 1, a, c) } },
    scale: function(f, e, c, a, b) { var d = this; if (e == null) { e = f } if (c === undefined) { c = 0 } if (a === undefined) { a = 0 } if (b) { return d.prepend(f, 0, 0, e, c - c * f, a - a * e) } else { return d.append(f, 0, 0, e, c - c * f, a - a * e) } },
    rotate: function(g, e, c, b) {
        ...
        e = e || 0;
        c = c || 0;
        if (b) { return d.prepend(f, a, -a, f, e - f * e + c * a, c - f * c - e * a) } else { return d.append(f, a, -a, f, e - f * e + c * a, c - f * c - e * a) }
    },
    rotateFromVector: function(a, h, c) {
        var e = this,
            ...
            f = a / g,
            b = h / g;
        if (c) { return e.prepend(f, b, -b, f, 0, 0) } else { return e.append(f, b, -b, f, 0, 0) }
    },
    clone: function() { return new Ext.draw.Matrix(this.elements) },
    flipX: function() { return this.append(-1, 0, 0, 1, 0, 0) },
    flipY: function() { return this.append(1, 0, 0, -1, 0, 0) },
    skewX: function(a) { return this.append(1, 0, Math.tan(a), 1, 0, 0) },
    skewY: function(a) { return this.append(1, Math.tan(a), 0, 1, 0, 0) },
    shearX: function(a) { return this.append(1, 0, a, 1, 0, 0) },
    shearY: function(a) { return this.append(1, a, 0, 1, 0, 0) },
    reset: function() { return this.set(1, 0, 0, 1, 0, 0) },
    ...
    x: function(a, c) { var b = this.elements; return a * b[0] + c * b[2] + b[4] },
    y: function(a, c) { var b = this.elements; return a * b[1] + c * b[3] + b[5] },
    get: function(b, a) { return +this.elements[b + a * 2].toFixed(4) },
    ...
    isIdentity: function() { var a = this.elements; return a[0] === 1 && a[1] === 0 && a[2] === 0 && a[3] === 1 && a[4] === 0 && a[5] === 0 },
    isEqual: function(a) {
        var c = a && a.isMatrix ? a.elements : a,
            b = this.elements;
        return b[0] === c[0] && b[1] === c[1] && b[2] === c[2] && b[3] === c[3] && b[4] === c[4] && b[5] === c[5]
    },
    equals: function(a) { return this.isEqual(a) },
    toArray: function() { var a = this.elements; return [a[0], a[2], a[4], a[1], a[3], a[5]] },
    toVerticalArray: function() { return this.elements.slice() },
    toString: function() { var a = this; return [a.get(0, 0), a.get(0, 1), a.get(1, 0), a.get(1, 1), a.get(2, 0), a.get(2, 1)].join(",") },
    toContext: function(a) { a.transform.apply(a, this.elements); return this },
    toSvg: function() { var a = this.elements; return "matrix(" + a[0].toFixed(9) + "," + a[1].toFixed(9) + "," + a[2].toFixed(9) + "," + a[3].toFixed(9) + "," + a[4].toFixed(9) + "," + a[5].toFixed(9) + ")" },
    getScaleX: function() { var a = this.elements; return Math.sqrt(a[0] * a[0] + a[2] * a[2]) },
    getScaleY: function() { var a = this.elements; return Math.sqrt(a[1] * a[1] + a[3] * a[3]) },
    getXX: function() { return this.elements[0] },
    getXY: function() { return this.elements[1] },
    getYX: function() { return this.elements[2] },
    getYY: function() { return this.elements[3] },
    getDX: function() { return this.elements[4] },
    getDY: function() { return this.elements[5] },
    split: function() {
        var b = this.elements,
            d = b[0],
            c = b[1],
    ...
    function b(e, c, d) { e[c] = { get: function() { return this.elements[d] }, set: function(f) { this.elements[d] = f } } }
...
Ext.define("Ext.draw.modifier.Target", {
    ...
    prepareAttributes: function(a) {
        if (this._lower) { this._lower.prepareAttributes(a) }
    ...
    applyChanges: function(f, k) {
        ...
        var l = this.getSprite(),
            ...
            p, a, m, b, e, n, d, c, g;
        for (b in k) { e = true; if ((p = h[b])) { l.scheduleUpdaters(f, p, [b]) } if (f.template && k.removeFromInstance && k.removeFromInstance[b]) { delete f[b] } }
        if (!e) { return }
        if (o.canvas) {
            n = o.canvas;
            delete o.canvas;
    ...
    popUp: function(a, b) { this.applyChanges(a, b) },
    pushDown: function(a, b) {
        if (this._lower) { b = this._lower.pushDown(a, b) }
...
Ext.define("Ext.draw.TimingFunctions", function() {
    var g = Math.pow,
        ...
        c = { pow: function(o, i) { return g(o, i || 6) }, expo: function(i) { return g(2, 8 * (i - 1)) }, circ: function(i) { return 1 - l(1 - i * i) }, sine: function(i) { return 1 - j((1 - i) * e / 2) }, back: function(i, o) { o = o || 1.616; return i * i * ((o + 1) * i - o) }, bounce: function(q) { for (var o = 0, i = 1; 1; o += i, i /= 2) { if (q >= (7 - 4 * o) / 11) { return i * i - g((11 - 6 * o - 11 * q) / 4, 2) } } }, elastic: function(o, i) { return g(2, 10 * --o) * m(20 * o * e * (i || 1) / 3) } },
        k = {},
        a, f, d;

    function h(i) { return function(o) { return g(o, i) } }

    function n(i, o) {
        k[i + "In"] = function(p) { return o(p) };
        k[i + "Out"] = function(p) { return 1 - o(1 - p) };
        k[i + "InOut"] = function(p) { return (p <= 0.5) ? o(2 * p) / 2 : (2 - o(2 * (1 - p))) / 2 }
    }
    for (d = 0, f = b.length; d < f; ++d) { c[b[d]] = h(d + 2) }
    for (a in c) { n(a, c[a]) }
...
}, function(a) { Ext.apply(a, a.easingMap) });
Ext.define("Ext.draw.Animator", {
    ...
    animations: [],
    running: false,
    animationTime: function() { return Ext.AnimationQueue.frameStartTime - this.frameStartTimeOffset },
    add: function(b) {
        var a = this;
        if (!a.contains(b)) {
            a.animations.push(b);
            a.ignite();
            if ("fireEvent" in b) { b.fireEvent("animationstart", b) }
        }
    },
    remove: function(d) {
        var c = this,
            ...
            b = 0,
            a = e.length;
        for (; b < a; ++b) { if (e[b] === d) { e.splice(b, 1); if ("fireEvent" in d) { d.fireEvent("animationend", d) } return } }
    },
    contains: function(a) { return Ext.Array.indexOf(this.animations, a) > -1 },
    empty: function() { return this.animations.length === 0 },
    step: function(d) {
        var c = this,
            f = c.animations,
            e, a = 0,
        ...
        for (; a < b; a++) {
            ...
            if (!e.animating) {
                ...
                a--;
                b--;
                if (e.fireEvent) { e.fireEvent("animationend", e) }
            }
        }
    },
    ...
    scheduleIf: function(e, b) {
        ...
        var c = Ext.draw.Animator.frameCallbacks,
            a, d;
        if (Ext.isString(e)) { e = b[e] }
        for (d in c) { a = c[d]; if (a.once && a.fn === e && a.scope === b) { return null } }
    ...
    removeFrameCallback: function(a) { delete Ext.draw.Animator.frameCallbacks[a] },
...
Ext.define("Ext.draw.modifier.Animation", {
    ...
    prepareAttributes: function(a) {
        ...
        if (this._lower) { this._lower.prepareAttributes(a.animationOriginal) }
    },
    updateSprite: function(a) { this.setConfig(a.config.fx) },
    updateDuration: function(a) { this.anyAnimation = a > 0 },
    applyEasing: function(a) { if (typeof a === "string") { a = Ext.draw.TimingFunctions.easingMap[a] } return a },
    applyCustomEasings: function(a, e) {
        ...
        for (d in a) {
            ...
            h = a[d];
            b = d.split(",");
            if (typeof h === "string") { h = Ext.draw.TimingFunctions.easingMap[h] }
            for (c = 0, f = b.length; c < f; c++) { e[b[c]] = h }
        }
        if (g) { this.anySpecialAnimations = g }
        return e
    },
    ...
    clearEasingOn: function(a) {
        ...
        var b = 0,
            c = a.length;
        for (; b < c; b++) { delete this._customEasings[a[b]] }
    },
    applyCustomDurations: function(g, h) {
        ...
        for (c in g) {
            ...
            f = g[c];
            a = c.split(",");
            for (b = 0, d = a.length; b < d; b++) { h[a[b]] = f }
        }
        if (e) { this.anySpecialAnimations = e }
        return h
    },
    ...
    clearDurationOn: function(a) {
        ...
        var b = 0,
            c = a.length;
        for (; b < c; b++) { delete this._customDurations[a[b]] }
    },
    setAnimating: function(a, b) {
        ...
        if (a.animating !== b) {
            ...
            if (b) {
                d.push(a);
                if (e.animating === 0) { Ext.draw.Animator.add(e) }
                e.animating++
            } else {
                for (var c = d.length; c--;) { if (d[c] === a) { d.splice(c, 1) } }
    ...
    setAttrs: function(r, t) {
        ...
        if (!n) {
            for (u in t) {
                if (r[u] === t[u]) { delete t[u] } else { r[u] = t[u] }
                delete o[u];
                delete m[u]
            }
            return t
            ...
            for (u in t) {
                ...
                if (l !== p && p !== undefined && p !== null && (c = h[u])) {
                    q = f;
                    a = e;
                    if (g) { if (u in i) { q = i[u] } if (u in j) { a = j[u] } }
                    if (p && p.isGradient || l && l.isGradient) { a = 0 }
                    if (a) {
                        if (!m[u]) { m[u] = {} }
                        k = m[u];
                        k.start = 0;
        ...
        if (d && !r.animating) { s.setAnimating(r, true) }
        return t
    },
    updateAttributes: function(g) {
        ...
        var h = {},
            ...
            a, b, i;
        if (g.lastUpdate === c) { return null }
        for (a in d) {
            b = d[a];
            if (!b.start) {
                b.start = c;
                i = 0
            } else { i = (c - b.start) / b.duration }
            if (i >= 1) {
                h[a] = f[a];
                delete f[a];
                if (d[a].remove) {
    ...
    pushDown: function(a, b) { b = this.callParent([a.animationOriginal, b]); return this.setAttrs(a, b) },
    popUp: function(a, b) {
        a = a.prototype;
        b = this.setAttrs(a, b);
        if (this._upper) { return this._upper.popUp(a, b) } else { return Ext.apply(a, b) }
    },
    step: function(g) {
        var f = this,
        ...
        for (; b < e; b++) {
            ...
            if (d && f._upper) { f._upper.popUp(a, d) }
        }
    },
    stop: function() {
        this.step();
        var d = this,
            b = d.animatingPool,
            a, c;
        for (a = 0, c = b.length; a < c; a++) { b[a].animating = false }
...
Ext.define("Ext.draw.modifier.Highlight", {
    ...
    preFx: true,
    applyHighlightStyle: function(b, a) { a = a || {}; if (this.getSprite()) { Ext.apply(a, this.getSprite().self.def.normalize(b)) } else { Ext.apply(a, b) } return a },
    prepareAttributes: function(a) {
        ...
        if (this._lower) { this._lower.prepareAttributes(a.highlightOriginal) }
    },
    updateSprite: function(b, a) {
        if (b) {
            if (this.getHighlightStyle()) { this._highlightStyle = b.self.def.normalize(this.getHighlightStyle()) }
    ...
    filterChanges: function(a, d) {
        ...
        for (b in d) { if (b !== "highlighted" && f[b] === d[b]) { delete d[b] } }
        return d
    },
    pushDown: function(e, g) {
        ...
        if (g.hasOwnProperty("highlighted")) {
            ...
            if (this._lower) { g = this._lower.pushDown(c, g) }
            g = this.filterChanges(e, g);
            if (d !== e.highlighted) {
                if (d) {
                    for (a in f) {
                        if (a in g) { c[a] = g[a] } else {
...
Ext.define("Ext.draw.sprite.Sprite", {
    ...
    inheritableStatics: {
        def: {
            ...
            updaters: {
                bbox: "bboxUpdater",
                zIndex: function(a) { a.dirtyZIndex = true },
    ...
    onClassExtended: function(d, c) {
        ...
        } else { d.def = new Ext.draw.sprite.AttributeDefinition(b) }
        d.def.spriteClass = d
    },
    ...
    getDirty: function() { return this.attr.dirty },
    setDirty: function(b) { this.attr.dirty = b; if (b) { var a = this.getParent(); if (a) { a.setDirty(true) } } },
    addModifier: function(a, b) {
        var c = this;
        if (!(a instanceof Ext.draw.modifier.Modifier)) { a = Ext.factory(a, null, null, "modifier") }
        a.setSprite(c);
        if (a.preFx || a.config && a.config.preFx) {
            if (c.fx._lower) { c.fx._lower.setUpper(a) }
            a.setUpper(c.fx)
        } else {
            c.topModifier._lower.setUpper(a);
            a.setUpper(c.topModifier)
        }
        if (b) { c.initializeAttributes() }
        return a
    },
    prepareModifiers: function(d) {
        ...
        for (a = 0, b = d.length; a < b; a++) { c.addModifier(d[a], false) }
    },
    getAnimation: function() { return this.fx },
    setAnimation: function(a) { this.fx.setConfig(a) },
    initializeAttributes: function() { this.topModifier.prepareAttributes(this.attr) },
    callUpdaters: function(d) {
        ...
        do {
            ...
            for (g in h) {
                ...
                delete h[g];
                f = i[g];
                if (typeof f === "string") { f = e[f] }
                if (f) { f.call(e, d, b) }
            }
            a = a || c
        } while (c);
        delete e.callUpdaters;
        if (a) { e.setDirty(true) }
    },
    ...
    scheduleUpdater: function(a, c, b) {
        ...
        if (c in d) { if (b.length) { d[c] = Ext.Array.merge(d[c], b) } } else { d[c] = b }
    },
    setAttributes: function(g, a, i) {
        var f = this,
            ...
            d, b, h, c;
        if (a) {
            if (i) { f.topModifier.pushDown(e, g) } else {
                c = {};
                for (b in g) { h = g[b]; if (h !== e[b]) { c[b] = h } }
    ...
    setAttributesBypassingNormalization: function(b, a) { return this.setAttributes(b, true, a) },
    bboxUpdater: function(b) {
        ...
        if (c && d || a && e) { this.scheduleUpdater(b, "transform") }
    },
    getBBox: function(d) {
        var e = this,
            a = e.attr,
    ...
    updateTransformedBBox: function(a, b) { this.attr.matrix.transformBBox(b, 0, a) },
    getBBoxCenter: function(a) { var b = this.getBBox(a); if (b) { return [b.x + b.width * 0.5, b.y + b.height * 0.5] } else { return [0, 0] } },
    hide: function() {
    ...
    useAttributes: function(i, f) {
        ...
        if (e) {
            if (e.isGradient) {
                i.strokeStyle = "black";
                i.strokeGradient = e
            } else { i.strokeGradient = false }
        }
        if (g) {
            if (g.isGradient) {
                i.fillStyle = "black";
                i.fillGradient = g
            } else { i.fillGradient = false }
        }
        if (b) { i.setLineDash(b) }
        if (Ext.isNumber(c) && Ext.isNumber(i.lineDashOffset)) { i.lineDashOffset = c }
        for (a in h) { if (h[a] !== undefined && h[a] !== i[a]) { i[a] = h[a] } }
        this.setGradientBBox(i, f)
    },
    setGradientBBox: function(b, c) { var a = this.attr; if (a.constrainGradients) { b.setGradientBBox({ x: c[0], y: c[1], width: c[2], height: c[3] }) } else { b.setGradientBBox(this.getBBox(a.transformFillStroke)) } },
    applyTransformations: function(b) {
        if (!b && !this.attr.dirtyTransform) { return }
        var r = this,
            k = r.attr,
    ...
    transform: function(b, c) {
        var a = this.attr,
            e = a.matrix,
            d;
        if (b && b.isMatrix) { d = b.elements } else { d = b }
        e.prepend.apply(e, d.slice());
        e.inverse(a.inverseMatrix);
        if (c) { this.updateTransformAttributes() }
    ...
    resetTransform: function(b) {
        ...
        if (!b) { this.updateTransformAttributes() }
        a.dirtyTransform = false;
        a.bbox.transform.dirty = true;
        this.setDirty(true);
        return this
    ...
    repaint: function() { var a = this.getSurface(); if (a) { a.renderFrame() } },
    remove: function() { var a = this.getSurface(); if (a && a.isSurface) { return a.remove(this) } return null },
    destroy: function() {
        var b = this,
            a = b.topModifier,
            c;
        ...
        delete b.attr;
        b.remove();
        if (b.fireEvent("beforedestroy", b) !== false) { b.fireEvent("destroy", b) }
        b.callParent()
    }
}, function() {
...
Ext.define("Ext.draw.Path", {
    ...
    dirt: function() { this.svgString = "" },
    moveTo: function(a, c) {
        var b = this;
        if (!b.cursor) { b.cursor = [a, c] }
        b.params.push(a, c);
        b.commands.push("M");
        b.startX = a;
        b.startY = c;
    ...
    arcTo: function(A, f, z, d, j, i, v) {
        ...
        if (i === undefined) { i = j }
        if (v === undefined) { v = 0 }
        if (!E.cursor) { E.moveTo(A, f); return }
        if (j === 0 || i === 0) { E.lineTo(A, f); return }
        z -= A;
        d -= f;
        var B = E.cursor[0] - A,
            g = E.cursor[1] - f,
        ...
        var n = B * o - e,
            ...
            w = Math.atan2(p, n),
            s = Math.atan2(y, h);
        if (C > 0) { if (s < w) { s += Math.PI * 2 } } else { if (w < s) { w += Math.PI * 2 } }
        if (i !== j) {
            e = b * e * j - a * c * i + A;
            c = a * c * i + b * c * i + f;
    ...
    ellipse: function(h, f, c, a, q, n, d, e) {
        ...
        for (l = 2; l < m; l += 6) { o.commands.push("C") }
        o.dirt()
    },
    arc: function(b, f, a, d, c, e) { this.ellipse(b, f, a, a, 0, d, c, e) },
    ...
    arcSvg: function(j, h, r, m, w, t, c) {
        ...
        if (w) { if (n <= 0) { n += Math.PI * 2 } } else { if (n >= 0) { n -= Math.PI * 2 } }
        x.ellipse(e, b, j, h, r, q, q + n, 1 - w)
    },
    fromSvgString: function(e) {
        if (!e) { return }
        var m = this,
            h, l = { a: 7, c: 6, h: 1, l: 2, m: 2, q: 4, s: 4, t: 2, v: 1, z: 0, A: 7, C: 6, H: 1, L: 2, M: 2, Q: 4, S: 4, T: 2, V: 1, Z: 0 },
            k = "",
            g, f, c = 0,
            b = 0,
            d = false,
            j, n, a;
        if (Ext.isString(e)) { h = e.replace(Ext.draw.Path.pathRe, " $1 ").replace(Ext.draw.Path.pathRe2, " -").split(Ext.draw.Path.pathSplitRe) } else { if (Ext.isArray(e)) { h = e.join(",").split(Ext.draw.Path.pathSplitRe) } }
        for (j = 0, n = 0; j < h.length; j++) { if (h[j] !== "") { h[n++] = h[j] } }
        h.length = n;
        m.clear();
        for (j = 0; j < h.length;) {
            k = d;
    ...
    interpolate: function(f, e, j, i, g) { if (g === 0) { return f } if (g === 1) { return i } var h = (1 - g) / g; return g * g * g * (i + h * (3 * j + h * (3 * e + h * f))) },
    fromStripes: function(g) {
        ...
        for (; c < d; c++) {
            ...
            e.commands.push("M");
            for (b = 2, a = f.length; b < a; b += 6) { e.commands.push("C") }
        }
    ...
    toString: function() { if (!this.svgString) { this.updateSvgString() } return this.svgString }
});
Ext.define("Ext.draw.overrides.hittest.Path", {
    ...
    isPointInPath: function(o, n) {
        ...
        for (h = 0, g = 0; h < l; h++) {
            switch (c[h]) {
                case "M":
                    if (e !== null) { if (q.linesIntersection(e, d, b, a, p.x, p.y, o, n)) { k += 1 } }
                    e = b = f[g];
                    d = a = f[g + 1];
                    g += 2;
                    break;
                case "L":
                    if (q.linesIntersection(b, a, f[g], f[g + 1], p.x, p.y, o, n)) { k += 1 }
                    b = f[g];
                    a = f[g + 1];
                    g += 2;
                    break;
    ...
    isPointOnPath: function(n, m) {
        ...
        for (h = 0, g = 0; h < k; h++) {
            switch (c[h]) {
                case "M":
                    if (e !== null) { if (o.pointOnLine(e, d, b, a, n, m)) { return true } }
                    e = b = f[g];
                    d = a = f[g + 1];
                    g += 2;
                    break;
                case "L":
                    if (o.pointOnLine(b, a, f[g], f[g + 1], n, m)) { return true }
                    b = f[g];
                    a = f[g + 1];
                    g += 2;
                    break;
                case "C":
                    if (o.pointOnCubic(b, f[g], f[g + 2], f[g + 4], a, f[g + 1], f[g + 3], f[g + 5], n, m)) { return true }
                    b = f[g + 4];
                    a = f[g + 5];
                    g += 6;
                    break;
...
Ext.define("Ext.draw.sprite.Path", {
    ...
    inheritableStatics: {
        def: {
            processors: { path: function(b, a) { if (!(b instanceof Ext.draw.Path)) { b = new Ext.draw.Path(b) } return b } },
            aliases: { d: "path" },
            triggers: { path: "bbox" },
            updaters: {
                path: function(a) {
    ...
    updatePlainBBox: function(a) { if (this.attr.path) { this.attr.path.getDimension(a) } },
    updateTransformedBBox: function(a) { if (this.attr.path) { this.attr.path.getDimensionWithTransform(this.attr.matrix, a) } },
    render: function(b, c) {
        var d = this.attr.matrix,
            a = this.attr;
        if (!a.path || a.path.params.length === 0) { return }
        d.toContext(c);
        c.appendPath(a.path);
        c.fillStroke(a)
    },
...
Ext.define("Ext.draw.overrides.hittest.sprite.Path", {
    ...
    isPointInPath: function(c, g) {
        var b = this.attr;
        if (b.fillStyle === Ext.util.Color.RGBA_NONE) { return this.isPointOnPath(c, g) }
        var e = b.path,
            d = b.matrix,
            f, a;
        if (!d.isIdentity()) {
    ...
    hitTest: function(i, l) {
        ...
        if (l.fill && l.stroke) { b = c.fillStyle !== Ext.util.Color.NONE && c.fillStyle !== Ext.util.Color.RGBA_NONE; if (b) { if (k.isPointInPath(h, f)) { j = { sprite: e } } } else { if (k.isPointInPath(h, f) || k.isPointOnPath(h, f)) { j = { sprite: e } } } } else { if (l.stroke && !l.fill) { if (k.isPointOnPath(h, f)) { j = { sprite: e } } } else { if (l.fill && !l.stroke) { if (k.isPointInPath(h, f)) { j = { sprite: e } } } } }
        if (a) { k.params = a }
        return j
    },
    getIntersections: function(j) {
        if (!(j.isSprite && j.isPath)) { return [] }
        var e = this.attr,
            d = j.attr,
            i = e.path,
            h = d.path,
...
Ext.define("Ext.draw.sprite.Circle", {
    ...
    updatePath: function(b, a) { b.arc(a.cx, a.cy, a.r, 0, Math.PI * 2, false) }
});
Ext.define("Ext.draw.sprite.Arc", { extend: "Ext.draw.sprite.Circle", alias: "sprite.arc", type: "arc", inheritableStatics: { def: { processors: { startAngle: "number", endAngle: "number", anticlockwise: "bool" }, aliases: { from: "startAngle", to: "endAngle", start: "startAngle", end: "endAngle" }, defaults: { startAngle: 0, endAngle: Math.PI * 2, anticlockwise: false }, triggers: { startAngle: "path", endAngle: "path", anticlockwise: "path" } } }, updatePath: function(b, a) { b.arc(a.cx, a.cy, a.r, a.startAngle, a.endAngle, a.anticlockwise) } });
...
Ext.define("Ext.draw.sprite.Composite", {
    ...
    add: function(c) {
        if (!c) { return null }
        if (!c.isSprite) { c = Ext.create("sprite." + c.type, c) }
        c.setParent(this);
    ...
    removeSprite: function(a, c) {
        var b = this,
            e, d;
        if (a) {
            if (a.charAt) { a = b.map[a] }
            if (!a || !a.isSprite) { return null }
            if (a.isDestroyed || a.isDestroying) { return a }
            e = a.getId();
            d = b.map[e];
            delete b.map[e];
            if (c) { a.destroy() }
    ...
    updateSurface: function(a) { for (var b = 0, c = this.sprites.length; b < c; b++) { this.sprites[b].setSurface(a) } },
    addAll: function(b) { if (b.isSprite || b.type) { this.add(b) } else { if (Ext.isArray(b)) { var a = 0; while (a < b.length) { this.add(b[a++]) } } } },
    updatePlainBBox: function(g) {
        ...
        for (c = 0, d = e.sprites.length; c < d; c++) {
            ...
            k = j.getBBox();
            if (b > k.x) { b = k.x }
            if (h < k.x + k.width) { h = k.x + k.width }
            if (f > k.y) { f = k.y }
            if (a < k.y + k.height) { a = k.y + k.height }
        }
        g.x = b;
        g.y = f;
        g.width = h - b;
    ...
    render: function(a, j, g) {
        var f = this,
            ...
            b = 0;
        h.toContext(j);
        for (; b < e; b++) { a.renderSprite(d[b], g) }
    },
    destroy: function() {
        var c = this,
            d = c.sprites,
...
Ext.define("Ext.draw.sprite.Ellipse", {
    ...
    updatePath: function(b, a) { b.ellipse(a.cx, a.cy, a.rx, a.ry, a.axisRotation, 0, Math.PI * 2, false) }
});
Ext.define("Ext.draw.sprite.EllipticalArc", { extend: "Ext.draw.sprite.Ellipse", alias: "sprite.ellipticalArc", type: "ellipticalArc", inheritableStatics: { def: { processors: { startAngle: "number", endAngle: "number", anticlockwise: "bool" }, aliases: { from: "startAngle", to: "endAngle", start: "startAngle", end: "endAngle" }, defaults: { startAngle: 0, endAngle: Math.PI * 2, anticlockwise: false }, triggers: { startAngle: "path", endAngle: "path", anticlockwise: "path" } } }, updatePath: function(b, a) { b.ellipse(a.cx, a.cy, a.rx, a.ry, a.axisRotation, a.startAngle, a.endAngle, a.anticlockwise) } });
Ext.define("Ext.draw.sprite.Rect", {
    ...
    updateTransformedBBox: function(a, b) { this.attr.matrix.transformBBox(b, this.attr.radius, a) },
...
Ext.define("Ext.draw.sprite.Image", {
    ...
    updateSurface: function(a) { if (a) { this.updateSource(this.attr) } },
    updateSource: function(g) {
        ...
        if (!f) {
            e = new Image();
            f = Ext.draw.sprite.Image.imageLoaders[a] = { image: e, done: false, pendingSprites: [h], pendingSurfaces: [c] };
            e.width = b;
            e.height = j;
            e.onload = function() { var i; if (!f.done) { f.done = true; for (d = 0; d < f.pendingSprites.length; d++) { i = f.pendingSprites[d]; if (!i.destroyed) { i.setDirty(true) } } for (d = 0; d < f.pendingSurfaces.length; d++) { i = f.pendingSurfaces[d]; if (!i.destroyed) { i.renderFrame() } } } };
            e.src = a
        } else {
    ...
    render: function(c, l) {
        ...
        if (e && e.done) {
            k.toContext(l);
            d = e.image;
            l.drawImage(d, i, h, b || (d.naturalWidth || d.width) / c.devicePixelRatio, j || (d.naturalHeight || d.height) / c.devicePixelRatio)
        }
    },
    isVisible: function() {
        var b = this.attr,
...
Ext.define("Ext.draw.sprite.Instancing", {
    ...
    applyTemplate: function(b) {
        if (!b.isSprite) {
            if (!b.xclass && !b.type) { b.type = "circle" }
    ...
    updateTemplate: function(a, b) {
        if (b) { delete b.ownAttr }
        a.setSurface(this.getSurface());
        a.ownAttr = a.attr;
        this.clearAll()
    },
    updateInstances: function(c) { this.clearAll(); if (Ext.isArray(c)) { for (var a = 0, b = c.length; a < b; a++) { this.add(c[a]) } } },
    updateSurface: function(a) { var b = this.getTemplate(); if (b && !b.destroyed) { b.setSurface(a) } },
    get: function(a) { return this.instances[a] },
    getCount: function() { return this.instances.length },
    clearAll: function() {
        var a = this.getTemplate();
        a.attr.children = this.instances = [];
        this.position = 0
    },
    createInstance: function(b, c, a) { return this.add(b, c, a) },
    add: function(d, f, c) {
    ...
    isInstanceVisible: function(c) {
        var e = this,
            ...
            f = e.instances,
            a = false;
        if (!Ext.isNumber(c) || c < 0 || c >= f.length || !e.isVisible()) { return a }
        d.attr = f[c];
    ...
    setAttributesFor: function(c, e, f) {
        ...
        if (!a) { return }
        d.attr = a;
        if (f) { e = Ext.apply({}, e) } else { e = d.self.def.normalize(e) }
...
Ext.define("Ext.draw.sprite.Sector", {
    ...
    getMidAngle: function() { return this.midAngle || 0 },
...
Ext.define("Ext.draw.sprite.Square", {
    ...
    updatePath: function(e, b) {
        var c = b.size * 1.2,
            ...
            a = b.x - b.lineWidth / 2,
            f = b.y;
        e.fromSvgString("M".concat(a - c, ",", f - c, "l", [d, 0, 0, d, -d, 0, 0, -d, "z"]))
    }
});
Ext.define("Ext.draw.TextMeasurer", {
    ...
    measureCache: {},
    precise: Ext.isIE8,
    measureDivTpl: { tag: "div", style: { overflow: "hidden", position: "relative", "float": "left", width: 0, height: 0 }, children: { tag: "div", style: { display: "block", position: "absolute", x: -100000, y: -100000, padding: 0, margin: 0, "z-index": -100000, "white-space": "nowrap" } } },
    actualMeasureText: function(g, b) {
        var e = Ext.draw.TextMeasurer,
            ...
            c;
        if (!f) {
            var d = Ext.Element.create({ style: { overflow: "hidden", position: "relative", "float": "left", width: 0, height: 0 } });
            e.measureDiv = f = Ext.Element.create({ style: { position: "absolute", x: a, y: a, "z-index": -a, "white-space": "nowrap", display: "block", padding: 0, margin: 0 } });
            Ext.getBody().appendChild(d);
            d.appendChild(f)
        }
        if (b) { f.setStyle({ font: b, lineHeight: "normal" }) }
        f.setText("(" + g + ")");
        c = f.getSize();
        f.setText("()");
    ...
    measureTextSingleLine: function(h, d) {
        if (this.precise) { return this.preciseMeasureTextSingleLine(h, d) }
        h = h.toString();
    ...
    measureText: function(e, b) {
        var h = e.split("\n"),
            ...
            a = 0,
            j, c, g;
        if (d === 1) { return this.measureTextSingleLine(e, b) }
        g = [];
        for (c = 0; c < d; c++) {
...
Ext.define("Ext.draw.sprite.Text", function() {
    var d = { "xx-small": true, "x-small": true, small: true, medium: true, large: true, "x-large": true, "xx-large": true };
    var b = { normal: true, bold: true, bolder: true, lighter: true, 100: true, 200: true, 300: true, 400: true, 500: true, 600: true, 700: true, 800: true, 900: true };
    var a = { start: "start", left: "start", center: "center", middle: "center", end: "end", right: "end" };
    var c = { top: "top", hanging: "hanging", middle: "middle", center: "middle", alphabetic: "alphabetic", ideographic: "ideographic", bottom: "bottom" };
    return {
        extend: "Ext.draw.sprite.Sprite",
        ...
        constructor: function(e) {
            if (e && e.font) { e = Ext.clone(e); for (var f in e) { if (f !== "font" && f.indexOf("font") === 0) { delete e[f] } } }
        ...
        makeFontShorthand: function(e) {
            var f = [];
            if (e.fontStyle) { f.push(e.fontStyle) }
            if (e.fontVariant) { f.push(e.fontVariant) }
            if (e.fontWeight) { f.push(e.fontWeight) }
            if (e.fontSize) { f.push(e.fontSize) }
            if (e.fontFamily) { f.push(e.fontFamily) }
        ...
        parseFontShorthand: function(j) {
            ...
            while (e < k && i !== -1) {
                i = m.indexOf(" ", e);
                if (i < 0) { f = m.substr(e) } else { if (i > e) { f = m.substr(e, i - e) } else { continue } }
                g = f.indexOf("/");
                if (g > 0) { f = f.substr(0, g) } else { if (g === 0) { continue } }
                if (f !== "normal" && f !== "inherit") { h = n[f]; if (h) { l[h] = f } else { if (f.match(Ext.dom.Element.unitRe)) { l.fontSize = f } else { l.fontFamily = m.substr(e); break } } }
                e = i + 1
            }
            if (!l.fontStyle) { l.fontStyle = "" }
            if (!l.fontVariant) { l.fontVariant = "" }
            if (!l.fontWeight) { l.fontWeight = "" }
        ...
        setAttributes: function(g, i, e) {
            ...
            if (g && g.font) {
                h = {};
                for (f in g) { if (!(f in this.fontProperties)) { h[f] = g[f] } }
                g = h
            }
        ...
        getBBox: function(g) {
            ...
            if (e && e.getInherited().rtl && e.getFlipRtlText()) { h.updatePlainBBox(f, true) }
        ...
        updatePlainBBox: function(k, B) {
            var E = this,
                ...
                u, C;
            if (B && E.oldSize) { u = E.oldSize } else {
                C = Ext.draw.TextMeasurer.precise;
                if (Ext.isBoolean(D)) { Ext.draw.TextMeasurer.precise = D }
        ...
        setText: function(e) { this.setAttributes({ text: e }, true) },
        render: function(e, q, k) {
            ...
            p.toContext(q);
            if (e.getInherited().rtl) { l += g.bbox.plain.width }
            for (f = 0; f < r.length; f++) { if (q.fillStyle !== m) { q.fillText(r[f], l + (s[f] || 0), j + n * f) } if (q.strokeStyle !== m) { q.strokeText(r[f], l + (s[f] || 0), j + n * f) } }
        }
    }
});
...
Ext.define("Ext.draw.sprite.Triangle", {
    ...
    updatePath: function(d, b) {
        var c = b.size * 2.2,
            a = b.x,
            e = b.y;
        d.fromSvgString("M".concat(a, ",", e, "m0-", c * 0.48, "l", c * 0.5, ",", c * 0.87, "-", c, ",0z"))
    }
});
Ext.define("Ext.draw.gradient.Linear", {
    ...
    applyRadians: function(b, a) { if (Ext.isNumber(b)) { return b } return a },
    applyDegrees: function(b, a) { if (Ext.isNumber(b)) { return b } return a },
    updateRadians: function(a) { this.setDegrees(Ext.draw.Draw.degrees(a)) },
    updateDegrees: function(a) { this.setRadians(Ext.draw.Draw.rad(a)) },
    generateGradient: function(q, o) {
        ...
        if (Ext.isNumber(d) && Ext.isNumber(b) && f > 0 && m > 0) {
            ...
            for (e = 0; e < g; e++) { k.addColorStop(n[e].offset, n[e].color) }
            return k
        }
...
Ext.define("Ext.draw.gradient.Radial", {
    ...
    applyStart: function(a, b) { if (!b) { return a } var c = { x: b.x, y: b.y, r: b.r }; if ("x" in a) { c.x = a.x } else { if ("centerX" in a) { c.x = a.centerX } } if ("y" in a) { c.y = a.y } else { if ("centerY" in a) { c.y = a.centerY } } if ("r" in a) { c.r = a.r } else { if ("radius" in a) { c.r = a.radius } } return c },
    applyEnd: function(b, a) { if (!a) { return b } var c = { x: a.x, y: a.y, r: a.r }; if ("x" in b) { c.x = b.x } else { if ("centerX" in b) { c.x = b.centerX } } if ("y" in b) { c.y = b.y } else { if ("centerY" in b) { c.y = b.centerY } } if ("r" in b) { c.r = b.r } else { if ("radius" in b) { c.r = b.radius } } return c },
    generateGradient: function(n, m) {
        var a = this.getStart(),
            ...
            e = l.length,
            c;
        for (c = 0; c < e; c++) { g.addColorStop(l[c].offset, l[c].color) }
        return g
    }
});
Ext.define("Ext.draw.Surface", {
    ...
    deprecated: { "5.1.0": { statics: { methods: { stableSort: function(a) { return Ext.Array.sort(a, function(d, c) { return d.attr.zIndex - c.attr.zIndex }) } } } } },
    ...
    roundPixel: function(a) { return Math.round(this.devicePixelRatio * a) / this.devicePixelRatio },
    waitFor: function(a) {
        var b = this,
            c = b.predecessors;
        if (!Ext.Array.contains(c, a)) {
            c.push(a);
            a.successors.push(b);
            if (a.getDirty()) { b.dirtyPredecessorCount++ }
        }
    },
    updateDirty: function(d) {
        ...
        for (; b < e; b++) {
            ...
            if (d) {
                ...
                a.setDirty(true)
            } else { a.dirtyPredecessorCount--; if (a.dirtyPredecessorCount === 0 && a.isPendingRenderFrame) { a.renderFrame() } }
        }
    },
    applyBackground: function(a, b) { this.setDirty(true); if (Ext.isString(a)) { a = { fillStyle: a } } return Ext.factory(a, Ext.draw.sprite.Rect, b) },
    applyRect: function(a, b) { if (b && a[0] === b[0] && a[1] === b[1] && a[2] === b[2] && a[3] === b[3]) { return } if (Ext.isArray(a)) { return [a[0], a[1], a[2], a[3]] } else { if (Ext.isObject(a)) { return [a.x || a.left, a.y || a.top, a.width || (a.right - a.left), a.height || (a.bottom - a.top)] } } },
    updateRect: function(i) {
        ...
        if (e) { e.setAttributes({ x: 0, y: 0, width: Math.ceil(g - Math.floor(c)), height: Math.ceil(a - Math.floor(f)) }) }
        h.setDirty(true)
    },
    ...
    get: function(a) { return this.map[a] || this.getItems()[a] },
    add: function() {
        var h = this,
        ...
        for (c = 0, e = g.length; c < e; c++) {
            ...
            j = null;
            if (l.isSprite && !a[l.getId()]) { j = l } else { if (!a[l.id]) { j = this.createItem(l) } }
            if (j) {
                a[j.getId()] = j;
                d.push(j);
                b = j.getSurface();
                if (b && b.isSurface) { b.remove(j) }
                j.setParent(h);
                j.setSurface(h);
                h.onAdd(j)
            }
        ...
        g = h.getItems();
        if (g) { g.push.apply(g, d) }
        h.dirtyZIndex = true;
        h.setDirty(true);
        if (!k && d.length === 1) { return d[0] } else { return d }
    },
    onAdd: Ext.emptyFn,
    remove: function(a, d) {
        var b = this,
        ...
        if (a) {
            ...
            e = b.map[f];
            delete b.map[f];
            if (a.destroyed || a.destroying) { if (e && !c) { Ext.Array.remove(b.getItems(), a) } return a }
            if (!e) { if (d) { a.destroy() } return a }
            a.setParent(null);
            a.setSurface(null);
            if (d) { a.destroy() }
            if (!c) {
    ...
    removeAll: function(f) {
        ...
        e.map = {};
        e.dirtyZIndex = true;
        if (!e.destroying) { e.setDirty(true) }
    },
    applyItems: function(a) { if (this.getItems()) { this.removeAll(true) } return Ext.Array.from(this.add(a)) },
    createItem: function(a) { return Ext.create(a.xclass || "sprite." + a.type, a) },
    ...
    orderByZIndex: function() {
        var d = this,
            ...
            b, c;
        if (d.getDirty()) {
            for (b = 0, c = a.length; b < c; b++) { if (a[b].attr.dirtyZIndex) { e = true; break } }
            if (e) {
                Ext.Array.sort(a, function(g, f) { return g.attr.zIndex - f.attr.zIndex });
                this.setDirty(true)
            }
            for (b = 0, c = a.length; b < c; b++) { a[b].attr.dirtyZIndex = false }
        }
    },
    repaint: function() {
        var a = this;
    ...
    renderFrame: function() {
        var f = this;
        if (!(f.element && f.getDirty() && f.getRect())) { return }
        if (f.dirtyPredecessorCount > 0) { f.isPendingRenderFrame = true; return }
        ...
        if (f.getDirty()) {
            f.clear();
            f.clearTransform();
            if (c) { f.renderSprite(c) }
            for (b = 0, d = a.length; b < d; b++) {
                e = a[b];
                if (f.renderSprite(e) === false) { return }
    ...
    destroy: function() {
        ...
        if (a.hasListeners.destroy) { a.fireEvent("destroy", a) }
        a.callParent()
    }
});
Ext.define("Ext.draw.overrides.hittest.Surface", {
    ...
    hitTest: function(b, c) {
        ...
        for (e = g.length - 1; e >= 0; e--) { d = g[e]; if (d.hitTest) { a = d.hitTest(b, c); if (a) { return a } } }
        return null
    },
    hitTestEvent: function(b, a) { var c = this.getEventXY(b); return this.hitTest(c, a) }
});
Ext.define("Ext.draw.engine.SvgContext", {
    ...
    getElement: function(a) { return this.surface.getSvgElement(this.group, a, this.position++) },
    save: function() {
        var c = this.toSave,
            e = {},
            d = this.getElement("g"),
            b, a;
        for (a = 0; a < c.length; a++) { b = c[a]; if (b in this) { e[b] = this[b] } }
        this.position = 0;
    ...
    restore: function() {
        var d = this.toSave,
            ...
            c = e.dom.childNodes,
            b, a;
        while (c.length > this.position) { e.last().destroy() }
        for (a = 0; a < d.length; a++) { b = d[a]; if (b in f) { this[b] = f[b] } else { delete this[b] } }
    ...
    setTransform: function(e, a, d, f, c, b) {
        if (this.path) { this.path.transform(this.matrix) }
        this.matrix.reset();
        this.transform(e, a, d, f, c, b)
    },
    scale: function(a, b) { this.transform(a, 0, 0, b, 0, 0) },
    rotate: function(d) {
        var c = Math.cos(d),
            a = Math.sin(d),
            b = -Math.sin(d),
    ...
    translate: function(a, b) { this.transform(1, 0, 0, 1, a, b) },
    setGradientBBox: function(a) { this.bbox = a },
    beginPath: function() { this.path = new Ext.draw.Path() },
    moveTo: function(a, b) {
    ...
    strokeText: function(d, a, e) {
        ...
        if (this.strokeStyle) {
            ...
            this.surface.setElementAttributes(b, { x: a, y: e, transform: this.matrix.toSvg(), stroke: this.strokeStyle, fill: "none", opacity: this.globalAlpha, "stroke-opacity": this.strokeOpacity, style: "font: " + this.font, "stroke-dasharray": this.lineDash.join(","), "stroke-dashoffset": this.lineDashOffset });
            if (this.lineDash.length) { this.surface.setElementAttributes(b, { "stroke-dasharray": this.lineDash.join(","), "stroke-dashoffset": this.lineDashOffset }) }
            if (c.dom.firstChild) { c.dom.removeChild(c.dom.firstChild) }
    ...
    fillText: function(d, a, e) {
        ...
        if (this.fillStyle) {
            var b = this.getElement("text"),
                c = this.surface.getSvgElement(b, "tspan", 0);
            this.surface.setElementAttributes(b, { x: a, y: e, transform: this.matrix.toSvg(), fill: this.fillStyle, opacity: this.globalAlpha, "fill-opacity": this.fillOpacity, style: "font: " + this.font });
            if (c.dom.firstChild) { c.dom.removeChild(c.dom.firstChild) }
            this.surface.setElementAttributes(c, { "alignment-baseline": "alphabetic" });
            c.dom.appendChild(document.createTextNode(Ext.String.htmlDecode(d)))
        }
    ...
    drawImage: function(c, k, i, l, e, p, n, a, g) {
        ...
        f.surface.setElementAttributes(d, { viewBox: o, x: j, y: h, width: b, height: m, opacity: f.globalAlpha, transform: f.matrix.toSvg() })
    },
    fill: function() {
        var c = this;
        if (!c.path) { return }
        if (c.fillStyle) {
            var e, a = c.fillGradient,
                b = c.path.element,
                f = c.bbox,
                d;
            if (!b) {
                ...
                c.surface.setElementAttributes(b, { d: e, transform: c.matrix.toSvg() })
            }
            if (a && f) { d = a.generateGradient(c, f) } else { d = c.fillStyle }
            c.surface.setElementAttributes(b, { fill: d, "fill-opacity": c.fillOpacity * c.globalAlpha })
        }
    },
    stroke: function() {
        var b = this;
        ...
        if (b.strokeStyle) {
            ...
            if (!a || !b.path.svgString) {
                ...
                b.surface.setElementAttributes(a, { fill: "none", d: e, transform: b.matrix.toSvg() })
            }
            if (d && f) { c = d.generateGradient(b, f) } else { c = b.strokeStyle }
            b.surface.setElementAttributes(a, { stroke: c, "stroke-linecap": b.lineCap, "stroke-linejoin": b.lineJoin, "stroke-width": b.lineWidth, "stroke-opacity": b.strokeOpacity * b.globalAlpha, "stroke-dasharray": b.lineDash.join(","), "stroke-dashoffset": b.lineDashOffset });
            if (b.lineDash.length) { b.surface.setElementAttributes(a, { "stroke-dasharray": b.lineDash.join(","), "stroke-dashoffset": b.lineDashOffset }) }
        }
    },
    fillStroke: function(a, e) {
        var b = this,
            ...
            c = b.fillOpacity,
            f = b.strokeOpacity;
        if (e === undefined) { e = a.transformFillStroke }
        if (!e) { a.inverseMatrix.toContext(b) }
        if (d && c !== 0) { b.fill() }
        if (g && f !== 0) { b.stroke() }
    },
    appendPath: function(a) { this.path = a.clone() },
    setLineDash: function(a) { this.lineDash = a },
    getLineDash: function() { return this.lineDash },
    createLinearGradient: function(c, f, a, d) {
        var e = this,
            b = e.surface.getNextDef("linearGradient"),
            g;
        e.surface.setElementAttributes(b, { x1: c, y1: f, x2: a, y2: d, gradientUnits: "userSpaceOnUse" });
    ...
    createRadialGradient: function(b, i, d, a, h, c) {
        var f = this,
            ...
            g;
        f.surface.setElementAttributes(e, { fx: b, fy: i, cx: a, cy: h, r: c, gradientUnits: "userSpaceOnUse" });
...
Ext.define("Ext.draw.engine.SvgContext.Gradient", {
    ...
    addColorStop: function(e, b) {
        var d = this,
            ...
            a = d.compression;
        d.surface.setElementAttributes(c, { offset: (((1 - a) * e + a) * 100).toFixed(2) + "%", "stop-color": b, "stop-opacity": Ext.util.Color.fly(b).a.toFixed(15) })
    },
    toString: function() { var a = this.element.dom.childNodes; while (a.length > this.position) { Ext.fly(a[a.length - 1]).destroy() } return "url(#" + this.element.getId() + ")" }
});
Ext.define("Ext.draw.engine.Svg", {
    ...
    getElementConfig: function() { return { reference: "element", style: { position: "absolute" }, children: [{ reference: "innerElement", style: { width: "100%", height: "100%", position: "relative" }, children: [{ tag: "svg", reference: "svgElement", namespace: "http://www.w3.org/2000/svg", width: "100%", height: "100%", version: 1.1 }] }] } },
    constructor: function(a) {
        var b = this;
        b.callParent([a]);
    ...
    createSvgNode: function(a) { var b = document.createElementNS("http://www.w3.org/2000/svg", a); return Ext.get(b) },
    getSvgElement: function(e, b, a) {
        var f = e.dom.childNodes,
            d = f.length,
            c;
        if (a < d) { c = f[a]; if (c.tagName === b) { return Ext.get(c) } else { Ext.destroy(c) } } else { if (a > d) { Ext.raise("Invalid position.") } }
        c = Ext.get(this.createSvgNode(b));
        if (a === 0) { e.insertFirst(c) } else { c.insertAfter(Ext.fly(f[a - 1])) }
        c.cache = {};
        return c
    },
    ...
    removeSurplusDefs: function() {
        var d = this.defsElement,
            ...
            c = a.length,
            b;
        for (b = c - 1; b > this.defsPosition; b--) { d.removeChild(a[b]) }
    },
    renderSprite: function(b) {
        ...
        b.preRender(this);
        b.useAttributes(a, c);
        if (false === b.render(this, a, [0, 0, c[2], c[3]])) { return false }
        b.setDirty(false);
        a.restore()
    },
    toSVG: function(e, b) {
        ...
        for (d = 0; d < b.length; d++) {
            a = b[d];
            if (Ext.getClassName(a) !== f) { continue }
            g = a.getRect();
    ...
    flatten: function(c, a) {
        ...
        b += this.toSVG(c, a);
        return { data: "data:image/svg+xml;utf8," + encodeURIComponent(b), type: "svg" }
    },
    serializeNode: function(d) {
        var b = "",
            c, f, a, e;
        if (d.nodeType === document.TEXT_NODE) { return d.nodeValue }
        b += "<" + d.nodeName;
...
if (!document.createElement("canvas").getContext) {
    (function() {
        ...
        function g(j, m, p) { var i = t.call(arguments, 2); return function() { return j.apply(m, i.concat(t.call(arguments))) } }

        function af(i) { return String(i).replace(/&/g, "&amp;").replace(/"/g, "&quot;") }

        function Y(m, j, i) { Ext.onReady(function() { if (!m.namespaces[j]) { m.namespaces.add(j, i, "#default#VML") } }) }
        ...
        var e = {
            ...
            init_: function(p) { var m = p.getElementsByTagName("canvas"); for (var j = 0; j < m.length; j++) { this.initElement(m[j]) } },
            initElement: function(j) {
                if (!j.getContext) {
                    ...
                    var i = j.attributes;
                    if (i.width && i.width.specified) { j.style.width = i.width.nodeValue + "px" } else { j.width = j.clientWidth }
                    if (i.height && i.height.specified) { j.style.height = i.height.nodeValue + "px" } else { j.height = j.clientHeight }
                }
                return j
            }
        };
        ...
        e.init();
        var k = [];
        for (var ae = 0; ae < 16; ae++) { for (var ad = 0; ad < 16; ad++) { k[ae * 16 + ad] = ae.toString(16) + ad.toString(16) } }
        ...
        function J(p, m) {
            ...
            for (var i = 0; i < 3; i++) {
                for (var ah = 0; ah < 3; ah++) {
                    var Z = 0;
                    for (var ag = 0; ag < 3; ag++) { Z += p[i][ag] * m[ag][ah] }
                    j[i][ah] = Z
                }
            }
            return j
        ...
        function M(j) { var p = j.indexOf("(", 3); var i = j.indexOf(")", p + 1); var m = j.substring(p + 1, i).split(","); if (m.length != 4 || j.charAt(3) != "a") { m[3] = 1 } return m }
        ...
        function a(j, i, m) { if (m < 0) { m++ } if (m > 1) { m-- } if (6 * m < 1) { return j + (i - j) * 6 * m } else { if (2 * m < 1) { return i } else { if (3 * m < 2) { return j + (i - j) * (2 / 3 - m) * 6 } else { return j } } } }
        var C = {};
        ...
        function F(j) {
            ...
            var ag, Z = 1;
            j = String(j);
            if (j.charAt(0) == "#") { ag = j } else {
                if (/^rgb/.test(j)) {
                    var p = M(j);
                    var ag = "#",
                        ah;
                    for (var m = 0; m < 3; m++) {
                        if (p[m].indexOf("%") != -1) { ah = Math.floor(c(p[m]) * 255) } else { ah = +p[m] }
                        ag += k[r(ah, 0, 255)]
                    }
                    Z = +p[3]
                } else {
        ...
        function E(i) { if (L[i]) { return L[i] } var p = document.createElement("div"); var m = p.style; try { m.font = i } catch (j) {} return L[i] = { style: m.fontStyle || o.style, variant: m.fontVariant || o.variant, weight: m.fontWeight || o.weight, size: m.fontSize || o.size, family: m.fontFamily || o.family } }
        ...
        function u(m, j) {
            ...
            var ag = parseFloat(j.currentStyle.fontSize),
                Z = parseFloat(m.size);
            if (typeof m.size == "number") { i.size = m.size } else { if (m.size.indexOf("px") != -1) { i.size = Z } else { if (m.size.indexOf("em") != -1) { i.size = ag * Z } else { if (m.size.indexOf("%") != -1) { i.size = (ag / 100) * Z } else { if (m.size.indexOf("pt") != -1) { i.size = Z / 0.75 } else { i.size = ag } } } } }
            i.size *= 0.981;
            return i
        }
        ...
        q.getLineDash = function() { return this.lineDash };
        q.stroke = function(ak) {
            ...
            for (var ag = 0; ag < this.currentPath_.length; ag++) {
                ...
                if (j) { if (Z.x == null || j.x < Z.x) { Z.x = j.x } if (aj.x == null || j.x > aj.x) { aj.x = j.x } if (Z.y == null || j.y < Z.y) { Z.y = j.y } if (aj.y == null || j.y > aj.y) { aj.y = j.y } }
            }
            ai.push(' ">');
            if (!ak) { w(this, ai) } else { G(this, ai, Z, aj) }
...
Ext.define("Ext.draw.engine.Canvas", {
        ...
        contextOverrides: {
            ...
            fill: function() {
                ...
                if (c !== Ext.util.Color.RGBA_NONE && b !== 0) {
                    if (a && e) { this.fillStyle = a.generateGradient(this, e) }
                    if (b !== 1) { this.globalAlpha = d * b }
                    this.$fill();
                    if (b !== 1) { this.globalAlpha = d }
                    if (a && e) { this.fillStyle = c }
                }
            },
            stroke: function() {
                ...
                if (e !== Ext.util.Color.RGBA_NONE && a !== 0) {
                    if (c && d) { this.strokeStyle = c.generateGradient(this, d) }
                    if (a !== 1) { this.globalAlpha = b * a }
                    this.$stroke();
                    if (a !== 1) { this.globalAlpha = b }
                    if (c && d) { this.strokeStyle = e }
                }
            },
            fillStroke: function(d, e) {
                ...
                if (e === undefined) { e = d.transformFillStroke }
                if (!e) { d.inverseMatrix.toContext(j) }
                if (i !== g && h !== 0) {
                    j.fill();
                    j.shadowColor = g;
                    j.shadowBlur = 0
            ...
            setLineDash: function(a) { if (this.$setLineDash) { this.$setLineDash(a) } },
            getLineDash: function() { if (this.$getLineDash) { return this.$getLineDash() } },
            ...
            save: function() {
                var c = this.toSave,
                    ...
                    b = 0,
                    a;
                for (; b < d; b++) { a = c[b]; if (a in this) { e[a] = this[a] } }
                this.state.push(e);
                this.$save()
            },
            restore: function() {
    ...
    createCanvas: function() {
        ...
        var d = Ext.draw.engine.Canvas.contextOverrides,
            ...
            b;
        if (a.ellipse) { delete d.ellipse }
        a.state = [];
        a.toSave = this.toSave;
        for (b in d) { a["$" + b] = a[b] }
        Ext.apply(a, d);
        if (this.getHighPrecision()) { this.enablePrecisionCompensation(a) } else { this.disablePrecisionCompensation(a) }
    ...
    updateHighPrecision: function(d) {
        var e = this.contexts,
            c = e.length,
            b, a;
        for (b = 0; b < c; b++) { a = e[b]; if (d) { this.enablePrecisionCompensation(a) } else { this.disablePrecisionCompensation(a) } }
    },
    ...
    disablePrecisionCompensation: function(b) {
        var a = Ext.draw.engine.Canvas.contextOverrides,
            ...
            e = f.length,
            d, c;
        for (d = 0; d < e; d++) { c = f[d]; if (!(c in a)) { delete b[c] } }
        this.setDirty(true)
    },
    enablePrecisionCompensation: function(j) {
        ...
        var f = {
            toSave: c.toSave,
            rect: function(m, p, n, o) { return h.rect.call(this, m * a + l, p * g + k, n * a, o * g) },
            ...
            clearRect: function(m, p, n, o) { return h.clearRect.call(this, m * a + l, p * g + k, n * a, o * g) },
            moveTo: function(m, n) { return h.moveTo.call(this, m * a + l, n * g + k) },
            lineTo: function(m, n) { return h.lineTo.call(this, m * a + l, n * g + k) },
            ...
            scale: function(n, m) { this.transform(n, 0, 0, m, 0, 0) },
            translate: function(n, m) { this.transform(1, 0, 0, 1, n, m) },
            rotate: function(o) {
                var n = Math.cos(o),
                    m = Math.sin(o);
                this.transform(n, m, -m, n, 0, 0)
            },
            quadraticCurveTo: function(n, p, m, o) { h.quadraticCurveTo.call(this, n * a + l, p * g + k, m * a + l, o * g + k) },
            bezierCurveTo: function(r, p, o, n, m, q) { h.bezierCurveTo.call(this, r * a + l, p * g + k, o * a + l, n * g + k, m * a + l, q * g + k) },
            ...
            fillText: function(o, m, p, n) {
                ...
                if (typeof n === "undefined") { h.fillText.call(this, o, m, p) } else { h.fillText.call(this, o, m, p, n) }
                this.updatePrecisionCompensate()
            },
            strokeText: function(o, m, p, n) {
                h.setTransform.apply(this, i.elements);
                if (typeof n === "undefined") { h.strokeText.call(this, o, m, p) } else { h.strokeText.call(this, o, m, p, n) }
                this.updatePrecisionCompensate()
            },
            fill: function() {
                var m = this.fillGradient,
                    n = this.bbox;
                this.updatePrecisionCompensateRect();
                if (m && n) { this.fillStyle = m.generateGradient(this, n) }
                h.fill.call(this);
                this.updatePrecisionCompensate()
            },
            stroke: function() {
                var m = this.strokeGradient,
                    n = this.bbox;
                this.updatePrecisionCompensateRect();
                if (m && n) { this.strokeStyle = m.generateGradient(this, n) }
                h.stroke.call(this);
                this.updatePrecisionCompensate()
            },
    ...
    updateRect: function(a) {
        ...
        while (D.length > q) { D.pop().destroy() }
        C.clear()
    },
    clearTransform: function() {
        var f = this,
    ...
    renderSprite: function(q) {
        ...
        v.prependMatrix(e);
        a = q.getBBox();
        if (a) { a = v.transformBBox(a) }
        q.preRender(C);
        if (q.attr.hidden || q.attr.globalAlpha === 0) { q.setDirty(false); return }
        for (u = 0, z = 0; u < m; u++, z += p) {
            for (x = 0, A = 0; x < c; x++, A += p) {
                ...
                n = z;
                f = n + o;
                if (a) { if (a.x > B || a.x + a.width < d || a.y > f || a.y + a.height < n) { continue } }
                s.save();
                q.useAttributes(s, b);
                if (false === q.render(C, s, [d, n, r, o])) { return false }
                s.restore()
            }
        }
        q.setDirty(false)
    ...
    flatten: function(n, a) {
        ...
        for (e = 0; e < a.length; e++) {
            b = a[e];
            if (Ext.getClassName(b) !== f) { continue }
            h = b.getRect();
    ...
    var c = this,
        b = c.prototype,
        a = 10000000000;
    if (Ext.os.is.Android4 && Ext.browser.is.Chrome) { a = 3000 } else { if (Ext.is.iOS) { a = 2200 } }
    b.splitThreshold = a
});
Ext.define("Ext.draw.Container", {
    ...
    supportedOptions: { version: Ext.isNumber, data: Ext.isString, format: function(a) { return Ext.Array.indexOf(this.supportedFormats, a) >= 0 }, filename: Ext.isString, width: Ext.isNumber, height: Ext.isNumber, scale: Ext.isNumber, pdf: Ext.isObject, jpeg: Ext.isObject },
    initAnimator: function() { this.frameCallbackId = Ext.draw.Animator.addFrameCallback("renderFrame", this) },
    applyGradients: function(b) {
        var a = [],
            c, f, d, e;
        if (!Ext.isArray(b)) { return a }
        for (c = 0, f = b.length; c < f; c++) {
            d = b[c];
            if (!Ext.isObject(d)) { continue }
            if (typeof d.type !== "string") { d.type = "linear" }
            if (d.angle) {
                d.degrees = d.angle;
                delete d.angle
            }
    ...
    handleResize: function(d, b) {
        ...
        if (!b) { f.resizeTimerId = Ext.defer(f.handleResize, f.resizeDelay, f, [d, true]); return } else { f.resizeTimerId = 0 }
        f.fireEvent("bodyresize", f, d);
        a = c.call(f, d);
        if (a !== false) { f.renderFrame() }
    },
    defaultResizeHandler: function(a) { this.getItems().each(function(b) { b.setRect([0, 0, a.width, a.height]) }) },
    getSurface: function(e) {
        var d = this,
            ...
            a;
        a = d.createSurface(e);
        if (b.getCount() > c) { d.handleResize(null, true) }
        return a
    },
    createSurface: function(d) {
        ...
        var c = this,
            b = c.getItems(),
            a = b.get(d);
        if (!a) { a = c.add({ xclass: c.engine, id: d }) }
        return a
    },
    renderFrame: function() {
        var e = this,
            a = e.getItems(),
            b, d, c;
        for (b = 0, d = a.length; b < d; b++) { c = a.items[b]; if (c.isSurface) { c.renderFrame() } }
    },
    getImage: function(k) {
        ...
        b = a[0];
        if ((Ext.isIE || Ext.isEdge) && b.isSVG) { d = { data: b.toSVG(l, a), type: "svg-markup" } } else {
            d = b.flatten(l, a);
            if (k === "image") {
                g = new Image();
                g.src = d.data;
            ...
            if (k === "stream") { d.data = d.data.replace(/^data:image\/[^;]+/, "data:application/octet-stream"); return d }
        }
        return d
    },
    download: function(d) {
...
Ext.define("Ext.chart.theme.Base", {
    ...
    defaultOperators: { "*": function(b, a) { return b * a }, "+": function(b, a) { return b + a }, "-": function(b, a) { return b - a } },
    resolveDefaults: function() {
        ...
        Ext.onReady(function() {
            ...
            if (!a.superclass.defaults) {
                ...
                a.superclass.defaults = { fontFamily: h.getStyle("fontFamily"), fontWeight: h.getStyle("fontWeight"), fontSize: parseFloat(h.getStyle("fontSize")), fontVariant: h.getStyle("fontVariant"), fontStyle: h.getStyle("fontStyle") };
                h.destroy()
            }
    ...
    applyBaseColor: function(c) {
        ...
        if (c) {
            ...
            b = a.getHSL()[2];
            if (b < 0.15) { a = a.createLighter(0.3) } else { if (b < 0.3) { a = a.createLighter(0.15) } else { if (b > 0.85) { a = a.createDarker(0.3) } else { if (b > 0.7) { a = a.createDarker(0.15) } } } }
    ...
    applyColors: function(a) { return a || this.colorDefaults },
    updateUseGradients: function(a) { if (a) { this.updateGradients({ type: "linear", degrees: 90 }) } },
        ...
        if (Ext.isObject(a)) {
            for (f = 0, g = c && c.length || 0; f < g; f++) {
                ...
                if (b) {
                    ...
                    h = Ext.apply(Ext.Object.chain(a), { stops: [{ offset: 1, color: b.toString() }, { offset: 0, color: d.toString() }] });
                    e.push(h)
                }
            }
            this.setColors(e)
    ...
    applySeriesThemes: function(a) {
        ...
        if (!a) { a = { fillStyle: Ext.Array.clone(b), strokeStyle: Ext.Array.map(b, function(d) { var c = Ext.util.Color.fromString(d.stops ? d.stops[0].color : d); return c.createDarker(0.15).toString() }) } }
        return a
    }
});
...
Ext.define("Ext.chart.Markers", {
    ...
    getMarkerFor: function(b, a) { if (b in this.categories) { var c = this.categories[b]; if (a in c) { return this.get(c[a]) } } },
    clear: function(a) { a = a || this.defaultCategory; if (!(a in this.revisions)) { this.revisions[a] = 1 } else { this.revisions[a]++ } },
    putMarkerFor: function(e, b, c, h, f) {
        e = e || this.defaultCategory;
        var d = this,
            g = d.categories[e] || (d.categories[e] = {}),
            a;
        if (c in g) { d.setAttributesFor(g[c], b, h) } else {
            g[c] = d.getCount();
            d.add(b, h)
        }
        a = d.get(g[c]);
        if (a) { a.category = e; if (!f) { a.revision = d.revisions[e] || (d.revisions[e] = 1) } }
    },
    getMarkerBBoxFor: function(c, a, b) { if (c in this.categories) { var d = this.categories[c]; if (a in d) { return this.getBBoxFor(d[a], b) } } },
    getBBox: function() { return null },
    render: function(a, m, g) {
        ...
        for (c = 0; c < e; c++) {
            h = f.get(c);
            if (h.hidden || h.revision !== l[h.category]) { continue }
            m.save();
            j.attr = h;
            j.useAttributes(m, b);
            j.render(a, m, g);
...
Ext.define("Ext.chart.modifier.Callout", {
    ...
    prepareAttributes: function(a) {
        ...
        if (this._lower) { this._lower.prepareAttributes(a.calloutOriginal) }
    },
    setAttrs: function(e, h) {
        ...
        if ("callout" in h || "calloutPlaceX" in h || "calloutPlaceY" in h || "x" in h || "y" in h) {
            var n = "rotationRads" in h ? i.rotationRads = h.rotationRads : i.rotationRads,
                ...
                j;
            n %= Math.PI * 2;
            if (Math.cos(n) < 0) { n = (n + Math.PI) % (Math.PI * 2) }
            ...
            if (h.calloutStartX && h.calloutStartY) { h.calloutHasLine = (p > 0 && h.calloutStartX < h.calloutEndX) || (p <= 0 && h.calloutStartX > h.calloutEndX) || (o > 0 && h.calloutStartY < h.calloutEndY) || (o <= 0 && h.calloutStartY > h.calloutEndY) } else { h.calloutHasLine = true }
        }
        return h
    },
    pushDown: function(a, b) { b = this.callParent([a.calloutOriginal, b]); return this.setAttrs(a, b) },
    popUp: function(a, b) {
        a = a.prototype;
        b = this.setAttrs(a, b);
        if (this._upper) { return this._upper.popUp(a, b) } else { return Ext.apply(a, b) }
...
Ext.define("Ext.chart.sprite.Label", {
    ...
    inheritableStatics: { def: { processors: { callout: "limited01", calloutHasLine: "bool", calloutPlaceX: "number", calloutPlaceY: "number", calloutStartX: "number", calloutStartY: "number", calloutEndX: "number", calloutEndY: "number", calloutColor: "color", calloutWidth: "number", calloutVertical: "bool", labelOverflowPadding: "number", display: "enums(none,under,over,rotate,insideStart,insideEnd,inside,outside)", orientation: "enums(horizontal,vertical)", renderer: "default" }, defaults: { callout: 0, calloutHasLine: true, calloutPlaceX: 0, calloutPlaceY: 0, calloutStartX: 0, calloutStartY: 0, calloutEndX: 0, calloutEndY: 0, calloutWidth: 1, calloutVertical: false, calloutColor: "black", labelOverflowPadding: 5, display: "none", orientation: "", renderer: null }, triggers: { callout: "transform", calloutPlaceX: "transform", calloutPlaceY: "transform", labelOverflowPadding: "transform", calloutRotation: "transform", display: "hidden" }, updaters: { hidden: function(a) { a.hidden = (a.display === "none") } } } },
    config: { fx: { customDurations: { callout: 200 } }, field: null, calloutLine: true },
    applyCalloutLine: function(a) { if (a) { return Ext.apply({}, a) } },
    ...
    render: function(b, c) {
        ...
        if (c.globalAlpha > 0 && a.calloutHasLine) {
            if (d && d.isGradient) { d = d.getStops()[0].color }
            c.strokeStyle = d;
            c.fillStyle = d;
...
Ext.define("Ext.chart.series.Series", {
    ...
    config: { chart: null, title: null, renderer: null, showInLegend: true, triggerAfterDraw: false, style: {}, subStyle: {}, themeStyle: {}, colors: null, useDarkerStrokeColor: true, store: null, label: {}, labelOverflowPadding: null, showMarkers: true, marker: null, markerSubStyle: null, itemInstancing: null, background: null, highlightItem: null, surface: null, overlaySurface: null, hidden: false, highlight: false, highlightCfg: { merge: function(a) { return a }, $value: { fillStyle: "yellow", strokeStyle: "red" } }, animation: null, tooltip: null },
    directions: [],
    sprites: null,
    themeColorCount: function() { return 1 },
    isStoreDependantColorCount: false,
    themeMarkerCount: function() { return 0 },
    getFields: function(f) {
        var e = this,
            a = [],
            c, b, d;
        for (b = 0, d = f.length; b < d; b++) { c = e["get" + f[b] + "Field"](); if (Ext.isArray(c)) { a.push.apply(a, c) } else { a.push(c) } }
        return a
    },
    applyAnimation: function(a, b) { if (!a) { a = { duration: 0 } } else { if (a === true) { a = { easing: "easeInOut", duration: 500 } } } return b ? Ext.apply({}, a, b) : a },
    getAnimation: function() { var a = this.getChart(); if (a && a.animationSuspendCount) { return { duration: 0 } } else { return this.callParent() } },
    updateTitle: function() {
        var b = this,
            a = b.getChart();
        if (a && !a.isInitializing) { a.refreshLegendStore() }
    },
    applyHighlight: function(a, b) { if (Ext.isObject(a)) { a = Ext.merge({}, this.config.highlightCfg, a) } else { if (a === true) { a = this.config.highlightCfg } } return Ext.apply(b || {}, a) },
    updateHighlight: function(a) { this.getStyle(); if (!Ext.Object.isEmpty(a)) { this.addItemHighlight() } },
    updateHighlightCfg: function(a) { if (!Ext.Object.equals(a, this.defaultConfig.highlightCfg)) { this.addItemHighlight() } },
    applyItemInstancing: function(a, b) { return Ext.merge(b || {}, a) },
    setAttributesForItem: function(c, d) {
        var b = c && c.sprite,
            a;
        if (b) { if (b.itemsMarker && c.category === "items") { b.putMarker(c.category, d, c.index, false, true) } if (b.isMarkerHolder && c.category === "markers") { b.putMarker(c.category, d, c.index, false, true) } else { if (b.isInstancing) { b.setAttributesFor(c.index, d) } else { if (Ext.isArray(b)) { for (a = 0; a < b.length; a++) { b[a].setAttributes(d) } } else { b.setAttributes(d) } } } }
    },
    getBBoxForItem: function(a) { if (a && a.sprite) { if (a.sprite.itemsMarker && a.category === "items") { return a.sprite.getMarkerBBox(a.category, a.index) } else { if (a.sprite instanceof Ext.draw.sprite.Instancing) { return a.sprite.getBBoxFor(a.index) } else { return a.sprite.getBBox() } } } return null },
    ...
    constructor: function(a) {
        var b = this,
            c;
        a = a || {};
        if (a.tips) { a = Ext.apply({ tooltip: a.tips }, a) }
        if (a.highlightCfg) { a = Ext.apply({ highlight: a.highlightCfg }, a) }
        if ("id" in a) { c = a.id } else { if ("id" in b.config) { c = b.config.id } else { c = b.getId() } }
        b.setId(c);
        b.sprites = [];
        b.dataRange = [];
    ...
    lookupViewModel: function(a) { var b = this.getChart(); return b ? b.lookupViewModel(a) : null },
    applyTooltip: function(c, b) { var a = Ext.apply({ xtype: "tooltip", renderer: Ext.emptyFn, constrainPosition: true, shrinkWrapDock: true, autoHide: true, mouseOffset: [20, 20] }, c); return Ext.create(a) },
    updateTooltip: function() { this.addItemHighlight() },
    addItemHighlight: function() {
        var d = this.getChart();
        if (!d) { return }
        var e = d.getInteractions(),
            c, a, b;
        for (c = 0; c < e.length; c++) { a = e[c]; if (a.isItemHighlight || a.isItemEdit) { b = true; break } }
        if (!b) {
    ...
    showTooltip: function(c, b) {
        ...
        d.pointerEvent = b;
        d.currentTarget.attach((c.sprite.length ? c.sprite[0] : c.sprite).getSurface().el.dom);
        Ext.callback(d.renderer, d.scope, [d, c.record, c], 0, a);
        if (d.isVisible()) { d.handleAfterShow() } else { d.show() }
    },
    hideTooltip: function(b) {
        ...
        if (!c) { return }
        clearTimeout(a.tooltipTimeout);
        a.tooltipTimeout = Ext.defer(function() { c.hide() }, 1)
    },
    applyStore: function(a) { return a && Ext.StoreManager.lookup(a) },
    getStore: function() { return this._store || this.getChart() && this.getChart().getStore() },
    updateStore: function(b, a) {
        var h = this,
            ...
            f, j, e, d;
        a = a || c;
        if (a && a !== b) { a.un({ datachanged: "onDataChanged", update: "onDataChanged", scope: h }) }
        if (b) {
            b.on({ datachanged: "onDataChanged", update: "onDataChanged", scope: h });
            f = h.getSprites();
            for (d = 0, e = f.length; d < e; d++) { j = f[d]; if (j.setStore) { j.setStore(b) } }
            h.onDataChanged()
        }
        h.fireEvent("storechange", h, b, a)
    },
    onStoreChange: function(b, a, c) { if (!this._store) { this.updateStore(a, c) } },
    coordinate: function(o, m, e) {
        ...
        if (j.length > 0) {
            ...
            for (d = 0; d < j.length; d++) { j[d].setAttributes(a) }
        }
    },
    coordinateData: function(b, h, d) {
        var g = [],
            ...
            e = d && d.getLayout(),
            c, a;
        for (c = 0; c < f; c++) { a = b[c].data[h]; if (!Ext.isEmpty(a, true)) { if (e) { g[c] = e.getCoordFor(a, h, c, b) } else { g[c] = +a } } else { g[c] = a } }
        return g
    },
    getRangeOfData: function(g, b) {
        var e = g.length,
            ...
            a = b.max,
            c, f;
        for (c = 0; c < e; c++) { f = g[c]; if (f < d) { d = f } if (f > a) { a = f } }
        b.min = d;
        b.max = a
    },
    updateLabelData: function() {
        var h = this,
            ...
            c, b, e, d, m, k;
        if (!f.length || !n.length) { return }
        for (c = 0; c < f.length; c++) {
            d = [];
            m = f[c];
            k = m.getField();
            if (Ext.Array.indexOf(n, k) < 0) { k = n[c] }
            for (b = 0, e = g.length; b < e; b++) { d.push(g[b].get(k)) }
            m.setAttributes({ labels: d })
        }
    },
    processData: function() {
        var d = this;
        if (d.isProcessingData || !d.getStore()) { return }
        ...
        for (a = 0; a < c; a++) {
            e = f[a];
            b = d["get" + e + "Axis"]();
            if (b) { b.processData(d); continue }
            if (d["coordinate" + e]) { d["coordinate" + e]() }
        }
        d.updateLabelData();
        d.isProcessingData = false
    },
    applyBackground: function(a) { if (this.getChart()) { this.getSurface().setBackground(a); return this.getSurface().getBackground() } else { return a } },
    ...
    onAxesChange: function(h) {
        ...
        for (d = 0, f = g.length; d < f; d++) { c = g[d]; if (!a[c.getDirection()]) { a[c.getDirection()] = [c] } else { a[c.getDirection()].push(c) } }
        for (d = 0, f = j.length; d < f; d++) { l = j[d]; if (k["get" + l + "Axis"]()) { continue } if (a[l]) { c = k.findMatchingAxis(a[l], b[l]); if (c) { k["set" + l + "Axis"](c); if (c.getNeedHighPrecision()) { e = true } } } }
    ...
    findMatchingAxis: function(f, e) {
        ...
        for (b = 0; b < f.length; b++) {
            d = f[b];
            c = d.getFields();
            if (!c.length) { return d } else { if (e) { for (a = 0; a < e.length; a++) { if (Ext.Array.indexOf(c, e[a]) >= 0) { return d } } } }
        }
    },
    ...
    updateOverlaySurface: function(a) { var b = this; if (a) { if (b.getLabel()) { b.getOverlaySurface().add(b.getLabel()) } } },
    applyLabel: function(a, d) {
        ...
        if (!d) {
            ...
            if (a) {
                ...
                if (a.display) { d.setAttributes({ hidden: a.display === "none" }) }
            }
            d.setDirty(true);
            this.updateLabel()
        }
    ...
    updateLabel: function() { var a = this.getChart(); if (a && !a.isInitializing) { a.redraw() } },
    ...
    getDefaultSpriteConfig: function() { return { type: this.seriesType, renderer: this.getRenderer() } },
    updateRenderer: function(c) {
        var b = this,
            a = b.getChart(),
            d;
        if (a && a.isInitializing) { return }
        d = b.getSprites();
        if (d.length) { d[0].setAttributes({ renderer: c || null }); if (a && !a.isInitializing) { a.redraw() } }
    },
    updateShowMarkers: function(a) {
        var d = this.getSprites(),
            b = d && d[0],
            c = b && b.getMarker("markers");
        if (c) { c.getTemplate().setAttributes({ hidden: !a }) }
    },
    createSprite: function() {
        ...
        d.setSeries(f);
        if (e) { d.itemsMarker = f.createItemInstancingSprite(d, e) }
        if (d.bindMarker) {
            if (b) {
                ...
                g.setTemplate(c);
                g.getTemplate().fx.setCustomDurations({ translationX: 0, translationY: 0 });
                d.dataMarker = g;
                d.bindMarker("markers", g);
                f.getOverlaySurface().add(g)
            }
            if (f.getLabel().getTemplate().getField()) { d.bindMarker("labels", f.getLabel()) }
        }
        if (d.setStore) { d.setStore(f.getStore()) }
    ...
    onDataChanged: function() {
        var d = this,
            ...
            b = c && c.getStore(),
            a = d.getStore();
        if (a !== b) { d.processData() }
    },
    isXType: function(a) { return a === "series" },
    getItemId: function() { return this.getId() },
    applyThemeStyle: function(e, a) {
        var b = this,
            d, c;
        d = e && e.subStyle && e.subStyle.fillStyle;
        c = d && e.subStyle.strokeStyle;
        if (d && !c) { e.subStyle.strokeStyle = b.getStrokeColorsFromFillColors(d) }
        d = e && e.markerSubStyle && e.markerSubStyle.fillStyle;
        c = d && e.markerSubStyle.strokeStyle;
        if (d && !c) { e.markerSubStyle.strokeStyle = b.getStrokeColorsFromFillColors(d) }
        return Ext.apply(a || {}, e)
    },
    applyStyle: function(c, b) { var a = Ext.ClassManager.get(Ext.ClassManager.getNameByAlias("sprite." + this.seriesType)); if (a && a.def) { c = a.def.normalize(c) } return Ext.apply({}, c, b) },
    applySubStyle: function(b, c) { var a = Ext.ClassManager.get(Ext.ClassManager.getNameByAlias("sprite." + this.seriesType)); if (a && a.def) { b = a.def.batchedNormalize(b, true) } return Ext.merge({}, c, b) },
    ...
    applyMarkerSubStyle: function(c, a) {
        var d = (c && c.type) || (a && a.type) || "circle",
            b = Ext.ClassManager.get(Ext.ClassManager.getNameByAlias("sprite." + d));
        if (b && b.def) { c = b.def.batchedNormalize(c, true) }
        return Ext.merge(a || {}, c)
    },
    updateHidden: function(b) {
        ...
        a.processData();
        a.doUpdateStyles();
        if (!Ext.isArray(b)) { a.updateLegendStore(b) }
    },
    updateLegendStore: function(f, b) {
        var e = this,
            ...
            g = e.getId(),
            a;
        if (c) { if (arguments.length > 1) { a = c.findBy(function(h) { return h.get("series") === g && h.get("index") === b }); if (a !== -1) { a = c.getAt(a) } } else { a = c.findRecord("series", g) } if (a && a.get("disabled") !== f) { a.set("disabled", f) } }
    },
    ...
    updateTheme: function(d) {
        ...
        for (l in a) {
            ...
            if (i !== null && i !== undefined && e) {
                ...
                if (b || o) { h[e.names.set](i) }
            }
        }
    },
    updateChartColors: function(a) { var b = this; if (!b.getColors()) { b.updateThemeColors(a) } },
    updateColors: function(a) { this.updateThemeColors(a) },
    updateStyle: function() { this.doUpdateStyles() },
    updateSubStyle: function() { this.doUpdateStyles() },
    updateThemeStyle: function() { this.doUpdateStyles() },
    doUpdateStyles: function() {
        ...
        for (; c < f; c++) {
            ...
            if (d) { h[c].itemsMarker.getTemplate().setAttributes(e) }
            h[c].setAttributes(e);
            if (b && h[c].dataMarker) { h[c].dataMarker.getTemplate().setAttributes(g.getMarkerStyleByIndex(c)) }
        }
    },
    ...
    getMarkerStyleByIndex: function(d) {
        ...
        if (k.hasOwnProperty("hidden")) { k.hidden = k.hidden || !this.getConfig("showMarkers", true) }
    ...
    styleDataForIndex: function(d, c) { var e, b, a = {}; if (d) { for (b in d) { e = d[b]; if (Ext.isArray(e)) { a[b] = e[c % e.length] } else { a[b] = e } } } return a },
    ...
    getItemByIndex: function(a, e) {
        var d = this,
            ...
            c;
        if (!b) { return }
        if (e === undefined && b.isMarkerHolder) { e = d.getItemInstancing() ? "items" : "markers" } else { if (!e || e === "" || e === "sprites") { b = f[a] } }
        if (b) { c = { series: d, category: e, index: a, record: d.getStore().getData().items[a], field: d.getYField(), sprite: b }; return c }
    },
    onSpriteAnimationStart: function(a) { this.fireEvent("animationstart", this, a) },
    onSpriteAnimationEnd: function(a) { this.fireEvent("animationend", this, a) },
    resolveListenerScope: function(e) {
        var d = this,
            a = Ext._namedScopes[e],
            c = d.getChart(),
            b;
        if (!a) { b = c ? c.resolveListenerScope(e, false) : (e || d) } else { if (a.isThis) { b = d } else { if (a.isController) { b = c ? c.resolveListenerScope(e, false) : d } else { if (a.isSelf) { b = c ? c.resolveListenerScope(e, false) : d; if (b === c && !c.getInheritedConfig("defaultListenerScope")) { b = d } } } } }
        return b
    },
    provideLegendInfo: function(a) { a.push({ name: this.getTitle() || this.getId(), mark: "black", disabled: this.getHidden(), series: this.getId(), index: 0 }) },
    clearSprites: function() {
        var d = this.sprites,
            b, a, c;
        for (a = 0, c = d.length; a < c; a++) { b = d[a]; if (b && b.isSprite) { b.destroy() } }
        this.sprites = []
    },
    destroy: function() {
        var b = this,
        ...
        if (a && a.getAutoDestroy()) { Ext.destroy(a) }
        b.setChart(null);
        b.clearListeners();
        if (c) {
            Ext.destroy(c);
...
Ext.define("Ext.chart.interactions.Abstract", {
    ...
    updateEnabled: function(a) {
        var c = this,
            b = c.getChart();
        if (b) { if (a) { c.addChartListener() } else { c.removeChartListener(b) } }
    },
    ...
    addChartListener: function() {
        ...
        function d(f, g) {
            b.addElementListener(f, c.listeners[f] = function(j) {
                var i = c.getLocks(),
                    h;
                if (c.getEnabled() && (!(f in i) || i[f] === c)) { h = (Ext.isFunction(g) ? g : c[g]).apply(this, arguments); if (h === false && j && j.stopPropagation) { j.stopPropagation() } return h }
            }, c)
        }
    ...
    unlockEvents: function() {
        var c = this.getLocks(),
            a = Array.prototype.slice.call(arguments),
            b = a.length;
        while (b--) { delete c[a[b]] }
    },
    getLocks: function() { var a = this.getChart(); return a.lockedEvents || (a.lockedEvents = {}) },
    doSync: function() {
        var b = this,
            a = b.getChart();
        if (b.syncTimer) {
        ...
        if (b.stopAnimationBeforeSync) { a.animationSuspendCount++ }
        a.redraw();
        if (b.stopAnimationBeforeSync) { a.animationSuspendCount-- }
    ...
    sync: function() {
        ...
        if (a.throttleGap && Ext.frameStartTime < a.syncThrottle) {
            ...
            a.syncTimer = Ext.defer(function() { a.doSync() }, a.throttleGap)
        } else { a.doSync() }
    },
    getItemId: function() { return this.getId() },
    isXType: function(a) { return a === "interaction" },
    destroy: function() {
        var a = this;
        a.setChart(null);
        delete a.listeners;
...
}, function() { if (Ext.os.is.Android4) { this.prototype.throttleGap = 40 } });
Ext.define("Ext.chart.MarkerHolder", {
    extend: "Ext.Mixin",
    mixinConfig: { id: "markerHolder", after: { constructor: "constructor", preRender: "preRender" }, before: { destroy: "destroy" } },
    isMarkerHolder: true,
    surfaceMatrix: null,
    inverseSurfaceMatrix: null,
    deprecated: { 6: { methods: { getBoundMarker: { message: "Please use the 'getMarker' method instead.", fn: function(b) { var a = this.boundMarkers[b]; return a ? [a] : a } } } } },
    constructor: function() {
        this.boundMarkers = {};
    ...
    onMarkerDestroy: function(a) { this.releaseMarker(a) },
    releaseMarker: function(a) {
        var c = this.boundMarkers,
            b;
        if (a && a.isMarkers) { for (b in c) { if (c[b] === a) { delete c[b]; break } } } else {
            b = a;
            a = c[b];
            delete c[b]
        }
    ...
    getMarker: function(a) { return this.boundMarkers[a] || null },
    preRender: function(c, j, g) {
        var f = this,
            ...
            i = f.getParent(),
            a, d, h;
        if (f.surfaceMatrix) { h = f.surfaceMatrix.set(1, 0, 0, 1, 0, 0) } else { h = f.surfaceMatrix = new Ext.draw.Matrix() }
        f.cleanRedraw = !f.attr.dirty;
        if (!f.cleanRedraw) { for (a in e) { d = e[a]; if (d) { d.clear(b) } } }
    ...
    putMarker: function(d, a, c, g, e) {
        var b = this.boundMarkers[d],
            f = this.getId();
        if (b) { b.putMarkerFor(f, a, c, g, e) }
    },
    getMarkerBBox: function(c, b, d) {
        var a = this.boundMarkers[c],
            e = this.getId();
        if (a) { return a.getMarkerBBoxFor(e, b, d) }
    },
    destroy: function() {
        var c = this.boundMarkers,
            b, a;
...
Ext.define("Ext.chart.axis.sprite.Axis", {
    ...
    inheritableStatics: { def: { processors: { grid: "bool", axisLine: "bool", minorTicks: "bool", minorTickSize: "number", majorTicks: "bool", majorTickSize: "number", length: "number", startGap: "number", endGap: "number", dataMin: "number", dataMax: "number", visibleMin: "number", visibleMax: "number", position: "enums(left,right,top,bottom,angular,radial,gauge)", minStepSize: "number", estStepSize: "number", titleOffset: "number", textPadding: "number", min: "number", max: "number", centerX: "number", centerY: "number", radius: "number", totalAngle: "number", baseRotation: "number", data: "default", enlargeEstStepSizeByText: "bool" }, defaults: { grid: false, axisLine: true, minorTicks: false, minorTickSize: 3, majorTicks: true, majorTickSize: 5, length: 0, startGap: 0, endGap: 0, visibleMin: 0, visibleMax: 1, dataMin: 0, dataMax: 1, position: "", minStepSize: 0, estStepSize: 20, min: 0, max: 1, centerX: 0, centerY: 0, radius: 1, baseRotation: 0, data: null, titleOffset: 0, textPadding: 0, scalingCenterY: 0, scalingCenterX: 0, strokeStyle: "black", enlargeEstStepSizeByText: false }, triggers: { minorTickSize: "bbox", majorTickSize: "bbox", position: "bbox,layout", axisLine: "bbox,layout", minorTicks: "layout", min: "layout", max: "layout", length: "layout", minStepSize: "layout", estStepSize: "layout", data: "layout", dataMin: "layout", dataMax: "layout", visibleMin: "layout", visibleMax: "layout", enlargeEstStepSizeByText: "layout" }, updaters: { layout: "layoutUpdater" } } },
    config: { label: null, labelOffset: 10, layout: null, segmenter: null, renderer: null, layoutContext: null, axis: null },
    thickness: 0,
    stepSize: 0,
    getBBox: function() { return null },
    defaultRenderer: function(a) { return this.segmenter.renderer(a, this) },
    layoutUpdater: function() {
        var h = this,
            f = h.getAxis().getChart();
        if (f.isInitializing) { return }
        var e = h.attr,
            d = h.getLayout(),
        ...
        if (c === "left" || c === "right") {
            ...
            if (c === "top" || c === "bottom") {
                if (g) { e.translationX = e.length + b * e.length / (i - b) + 1 } else { e.translationX = -b * e.length / (i - b) }
                e.translationY = 0;
    ...
    iterate: function(e, j) {
        var c, g, a, b, h, d, k = Ext.Array.some,
            m = Math.abs,
            f;
        if (e.getLabel) { if (e.min < e.from) { j.call(this, e.min, e.getLabel(e.min), -1, e) } for (c = 0; c <= e.steps; c++) { j.call(this, e.get(c), e.getLabel(c), c, e) } if (e.max > e.to) { j.call(this, e.max, e.getLabel(e.max), e.steps + 1, e) } } else {
            b = this.getAxis();
            h = b.floatingAxes;
            d = [];
            f = (e.to - e.from) / (e.steps + 1);
            if (b.getFloating()) { for (a in h) { d.push(h[a]) } }

            function l(i) { return !d.length || k(d, function(n) { return m(n - i) > f }) }
            if (e.min < e.from && l(e.min)) { j.call(this, e.min, e.min, -1, e) }
            for (c = 0; c <= e.steps; c++) { g = e.get(c); if (l(g)) { j.call(this, g, g, c, e) } }
            if (e.max > e.to && l(e.max)) { j.call(this, e.max, e.max, e.steps + 1, e) }
        }
    },
    ...
    renderLabels: function(E, q, D, K) {
        ...
        if (I && F && !F.attr.hidden) {
            ...
            switch (u) {
                ...
                case "top":
                    e = a ? a.y + a.height : 0;
                    F.setAttributes({ translationY: E.roundPixel(e + (K[3] - e - G) / 2) - i }, true);
                    break;
                case "bottom":
                    e = a ? K[3] - a.y : 0;
                    F.setAttributes({ translationY: E.roundPixel(G + (K[3] - G - e) / 2) + i }, true);
                    break;
                case "radial":
            ...
            if (u === "left" || u === "right") {
                o.iterate(I, function(L, N, M) {
                    ...
                    if (B) { v = Ext.callback(B, null, [t, N, D, r], 0, t) } else { v = m.renderer(N, D, r) }
                    r = N;
                    F.setAttributes({ text: String(v), translationY: E.roundPixel(L * g + c) }, true);
                    F.applyTransformations();
                ...
                if (u === "top" || u === "bottom") {
                    o.iterate(I, function(L, N, M) {
                        ...
                        if (B) { v = Ext.callback(B, null, [t, N, D, r], 0, t) } else { v = m.renderer(N, D, r) }
                        r = N;
                        F.setAttributes({ text: String(v), translationX: E.roundPixel(L * x + d) }, true);
                        F.applyTransformations();
                    ...
                    if (u === "radial") {
                        o.iterate(I, function(L, N, M) {
                            ...
                            if (typeof v !== "undefined") {
                                F.setAttributes({ text: String(v), translationX: k.centerX - E.roundPixel(L) / k.max * k.length * Math.cos(k.baseRotation + Math.PI / 2), translationY: k.centerY - E.roundPixel(L) / k.max * k.length * Math.sin(k.baseRotation + Math.PI / 2) }, true);
                                F.applyTransformations();
                        ...
                        if (u === "angular") {
                            ...
                            o.iterate(I, function(L, N, M) {
                                ...
                                if (typeof v !== "undefined") {
                                    ...
                                    F.setAttributes({ text: String(v), translationX: k.centerX + (k.length + s) * Math.cos(O), translationY: k.centerY + (k.length + s) * Math.sin(O) }, true);
                                    F.applyTransformations();
                                    p = F.attr.matrix.transformBBox(F.getBBox(true));
                                    if (h && !f(p, h)) { return }
                                    E.renderSprite(F);
            ...
            if (k.enlargeEstStepSizeByText && b) {
                ...
                w += G;
                w *= 2;
                if (k.estStepSize < w) { k.estStepSize = w }
            }
    ...
    getGaugeAngles: function() {
        var a = this,
            c = a.attr.totalAngle,
            b;
        if (c <= Math.PI) { b = (Math.PI - c) * 0.5 } else { b = -(Math.PI * 2 - c) * 0.5 }
        b = Math.PI * 2 - b;
        ...
        if (l.grid) {
            if (q) {
                if (u === "left" || u === "right") {
                    ...
                    if (u === "top" || u === "bottom") {
                        i = l.min * c + h + d;
                        if (d) { t.putMarker(f + "-even", { x: 0, width: i }, -1, true) }
                        t.iterate(q, function(j, w, v) {
                            e = j * c + h + d;
                        ...
                        if (u === "radial") {
                            ...
                            if (u === "angular") {
                                t.iterate(q, function(j, w, v) {
                                    ...
                                    t.putMarker(f + "-" + (v % 2 ? "odd" : "even"), { rotationRads: e, rotationCenterX: 0, rotationCenterY: 0, scalingX: l.length, scalingY: l.length }, v, true);
                                    i = e
                                })
                            }
                        }
        ...
        if (u === "left" || u === "right") {
            for (q = 0, f = d.length; q < f; q++) {
                ...
                if (s.line.title) {
                    ...
                    v.setAttributesFor(v.position - 1, { x: g, y: s.line.y - c.height / 2, textAlign: j, fillStyle: s.line.title.fillStyle || s.line.strokeStyle })
                }
            }
        } else {
            if (u === "top" || u === "bottom") {
                for (q = 0, f = d.length; q < f; q++) {
                    ...
                    if (s.line.title) {
                        ...
                        v.setAttributesFor(v.position - 1, { x: s.line.x + c.height / 2, y: e, fillStyle: s.line.title.fillStyle || s.line.strokeStyle, rotationRads: Math.PI / 2 })
                    }
                }
            } else {
                if (u === "radial") {
                    for (q = 0, f = d.length; q < f; q++) {
                        ...
                        if (s.line.title) {
                            v.add(s.line.title);
                            c = v.getBBoxFor(v.position - 1);
                            v.setAttributesFor(v.position - 1, { x: m.centerX, y: m.centerY - l - c.height / 2, fillStyle: s.line.title.fillStyle || s.line.strokeStyle })
                        }
                    }
                } else {
                    if (u === "angular") {
                        for (q = 0, f = d.length; q < f; q++) {
                            ...
                            if (s.line.title) {
                                ...
                                v.setAttributesFor(v.position - 1, { x: m.centerX + 0.5 * m.length * Math.cos(l) + b * c.height / 2 * Math.sin(l), y: m.centerY + 0.5 * m.length * Math.sin(l) - b * c.height / 2 * Math.cos(l), rotationRads: b === 1 ? l : l - Math.PI, fillStyle: s.line.title.fillStyle || s.line.strokeStyle })
                            }
                        }
                    } else { if (u === "gauge") {} }
                }
    ...
    doThicknessChanged: function() { var a = this.getAxis(); if (a) { a.onThicknessChanged() } },
    render: function(a, c, e) {
        var d = this,
            b = d.getLayoutContext();
        if (b) {
            if (false === d.renderLabels(a, c, b, e)) { return false }
            c.beginPath();
...
Ext.define("Ext.chart.axis.segmenter.Segmenter", { config: { axis: null }, constructor: function(a) { this.initConfig(a) }, renderer: function(b, a) { return String(b) }, from: function(a) { return a }, diff: Ext.emptyFn, align: Ext.emptyFn, add: Ext.emptyFn, preferredStep: Ext.emptyFn });
...
Ext.define("Ext.chart.axis.segmenter.Numeric", {
    ...
    isNumeric: true,
    renderer: function(b, a) { return b.toFixed(Math.max(0, a.majorTicks.unit.fixes)) },
    diff: function(b, a, c) { return Math.floor((a - b) / c.scale) },
    align: function(d, c, b) { var a = b.scale * c; return Math.floor(d / a) * a },
    add: function(c, b, a) { return c + b * a.scale },
    ...
    leadingZeros: function(a) { return -Math.floor(Ext.Number.log10(Math.abs(a))) },
    exactStep: function(b, a) {
        var c = this.leadingZeros(a),
            d = Math.pow(10, c);
        return { unit: { fixes: c + (a % d === 0 ? 0 : 1), scale: a < 1 ? a : 1 }, step: a < 1 ? 1 : a }
    },
    adjustByMajorUnit: function(e, g, c) {
        var d = c[0],
            ...
            a = e * g,
            f = d % a;
        if (f !== 0) { c[0] = d - f + (d < 0 ? -a : 0) }
        f = b % a;
        if (f !== 0) { c[1] = b - f + (b > 0 ? a : 0) }
    }
});
Ext.define("Ext.chart.axis.segmenter.Time", {
    ...
    from: function(a) { return new Date(a) },
    diff: function(b, a, c) { if (isFinite(b)) { b = new Date(b) } if (isFinite(a)) { a = new Date(a) } return Ext.Date.diff(b, a, c) },
    updateStep: function() { var a = this.getAxis(); if (a && !this.isConfiguring) { a.performLayout() } },
    align: function(a, c, b) {
        if (b === "d" && c >= 7) {
            a = Ext.Date.align(a, "d", c);
            a.setDate(a.getDate() - a.getDay() + 1);
            return a
        } else { return Ext.Date.align(a, b, c) }
    },
    add: function(c, b, a) { return Ext.Date.add(new Date(c), a, b) },
    stepUnits: [
    ...
    preferredStep: function(b, e) {
        if (this.getStep()) { return this.getStep() }
        var f = new Date(+b),
        ...
        for (c = 0; c < d.length; c++) {
            ...
            h = this.diff(f, g, k);
            if (h > 0) {
                for (a = 1; a < d[c].length; a++) { if (h <= d[c][a]) { l = { unit: k, step: d[c][a] }; break } }
                if (!l) {
                    c--;
                    l = { unit: d[c][0], step: 1 }
                }
        ...
        if (!l) { l = { unit: Ext.Date.DAY, step: 1 } }
        return l
    }
});
Ext.define("Ext.chart.axis.layout.Layout", {
    mixins: { observable: "Ext.mixin.Observable" },
    config: { axis: null },
    constructor: function(a) { this.mixins.observable.constructor.call(this, a) },
    processData: function(b) {
        var e = this,
            ...
            g = c.boundSeries,
            a, d;
        if (b) { b["coordinate" + f]() } else { for (a = 0, d = g.length; a < d; a++) { g[a]["coordinate" + f]() } }
    },
    ...
    calculateMinorTicks: function(a) { if (this.snapMinorEnds) { a.minorTicks = this.snapMinorEnds(a) } },
    calculateLayout: function(b) {
        var c = this,
            a = b.attr;
        if (a.length === 0) { return null }
        if (a.majorTicks) { c.calculateMajorTicks(b); if (a.minorTicks) { c.calculateMinorTicks(b) } }
    },
    ...
    trimByRange: function(b, f, i, a) {
        ...
        if (e < f.steps) { f.to = g.add(f.from, e * f.step, j) }
        if (f.max > a) { f.max = f.to }
        if (f.from < i) {
...
Ext.define("Ext.chart.axis.layout.Discrete", {
    ...
    processData: function() {
        ...
        f.labels = [];
        f.labelMap = {};
        for (b = 0, e = c.length; b < e; b++) { a = c[b]; if (a["get" + g + "Axis"]() === d) { a["coordinate" + g]() } }
    ...
    snapEnds: function(e, d, a, b) {
        ...
        var c = Math.floor((a - d) / b),
            f = e.data;
        return { min: d, max: a, from: d, to: c * b + d, step: b, steps: c, unit: 1, getLabel: function(g) { return f[this.from + this.step * g] }, get: function(g) { return this.from + this.step * g } }
    },
    trimByRange: function(b, f, h, a) {
        ...
        if (f.min < h) { f.min = f.from }
        f.steps = e - c
    },
    getCoordFor: function(c, d, a, b) { this.labels.push(c); return this.labels.length - 1 }
});
...
Ext.define("Ext.chart.axis.layout.Continuous", {
    ...
    getCoordFor: function(c, d, a, b) { return +c },
    snapEnds: function(a, d, i, h) {
        var f = a.segmenter,
            ...
            j = l ? d : f.align(d, b, k),
            g = (l || f.diff(d, i, k)) + 1;
        return { min: f.from(d), max: f.from(i), from: j, to: f.add(j, g * b, k), step: b, steps: g, unit: k, get: function(m) { return f.add(this.from, this.step * m, this.unit) } }
    },
    snapMinorEnds: function(a) {
        var e = a.majorTicks,
            ...
            g = e.steps * m + h;
        return { min: d, max: i, from: d + j % n, to: f.add(k, g * b, l), step: b, steps: g, unit: l, get: function(o) { return (o % m + c + 1 !== 0) ? f.add(this.from, this.step * o, l) : null } }
    }
});
Ext.define("Ext.chart.axis.Axis", {
    ...
    yValues: [],
    masterAxis: null,
    applyRotation: function(b) { var a = Math.PI * 2; return (b % a + Math.PI) % a - Math.PI },
    updateRotation: function(b) {
        var c = this.getSprites(),
            a = this.getPosition();
        if (!this.getHidden() && a === "angular" && c[0]) { c[0].setAttributes({ baseRotation: b }) }
    },
    applyTitle: function(c, b) { var a; if (Ext.isString(c)) { c = { text: c } } if (!b) { b = Ext.create("sprite.text", c); if ((a = this.getSurface())) { a.add(b) } } else { b.setAttributes(c) } return b },
    applyFloating: function(b, a) { if (b === null) { b = { value: null, alongAxis: null } } else { if (Ext.isNumber(b)) { b = { value: b, alongAxis: null } } } if (Ext.isObject(b)) { if (a && a.alongAxis) { delete this.getChart().getAxis(a.alongAxis).floatingAxes[this.getId()] } return b } return a },
    constructor: function(a) {
        var b = this,
            c;
        b.sprites = [];
    ...
    getSurface: function() {
        ...
        if (d && !e.surface) {
            ...
            if (e.getLimits() && f) {
                f = f.replace("3d", "");
                e.limits = { surface: d.getSurface("overlay"), lines: new Ext.chart.Markers(), titles: new Ext.draw.sprite.Instancing() };
                e.limits.lines.setTemplate({ xclass: "grid." + f });
                e.limits.lines.getTemplate().setAttributes({ strokeStyle: "black" }, true);
    ...
    applyGrid: function(a) { if (a === true) { return {} } return a },
    updateGrid: function(b) {
        var e = this,
            d = e.getChart();
        if (!d) { e.on({ chartattached: Ext.bind(e.updateGrid, e, [b]), single: true }); return }
        var c = e.gridSurface,
            a = e.getSprites()[0],
            f = e.getGridAlignment(),
            g;
        if (b) {
            ...
            if (Ext.isObject(b)) { g.getTemplate().setAttributes(b); if (Ext.isObject(b.even)) { g.getTemplate().setAttributes(b.even) } }
            g = e.gridSpriteOdd;
            if (!g) {
            ...
            if (Ext.isObject(b)) { g.getTemplate().setAttributes(b); if (Ext.isObject(b.odd)) { g.getTemplate().setAttributes(b.odd) } }
        }
    },
    updateMinorTickSteps: function(d) {
        ...
        if (b) {
            ...
            a = c.getSurface();
            if (!c.isConfiguring && a) { a.renderFrame() }
        }
    },
    renderLimits: function() { this.getSprites()[0].renderLimits() },
    getCoordFor: function(c, d, a, b) { return this.getLayout().getCoordFor(c, d, a, b) },
    applyPosition: function(a) { return a.toLowerCase() },
    applyLength: function(b, a) { return b > 0 ? b : a },
    applyLabel: function(b, a) {
        if (!a) { a = new Ext.draw.sprite.Text({}) }
        if (this.limitTitleTpl) { this.limitTitleTpl.setAttributes(b) }
        a.setAttributes(b);
        return a
    },
    ...
    updateMinimum: function() { this.range = null },
    updateMaximum: function() { this.range = null },
    hideLabels: function() {
    ...
    renderFrame: function() { this.getSurface().renderFrame() },
    ...
    applyBackground: function(a) { var b = Ext.ClassManager.getByAlias("sprite.rect"); return b.def.normalize(a) },
    processData: function() {
        this.getLayout().processData();
        this.range = null
    },
    getDirection: function() { return this.getChart().getDirectionForAxis(this.getPosition()) },
    isSide: function() { var a = this.getPosition(); return a === "left" || a === "right" },
    applyFields: function(a) { return Ext.Array.from(a) },
    applyVisibleRange: function(a, c) {
        ...
        if (a[1] === a[0]) { a[1] += 1 / this.getMaxZoom() }
        if (a[1] > a[0] + 1) {
            a[0] = 0;
            a[1] = 1
        } else {
        ...
        if (c && a[0] === c[0] && a[1] === c[1]) { return undefined }
        return a
    },
    updateVisibleRange: function(a) { this.fireEvent("visiblerangechange", this, a) },
    onSeriesChange: function(e) {
        var f = this,
            ...
            c, d = b.length,
            a, h;
        for (c = 0; c < d; c++) { if (this === b[c][j]()) { g.push(b[c]) } }
        f.boundSeries = g;
        a = f.getLinkedTo();
        h = !Ext.isEmpty(a) && e.getAxis(a);
        if (h) { f.linkAxis(h) } else { f.getLayout().processData() }
    },
    linkAxis: function(a) {
        var c = this;
        ...
        if (c.masterAxis) {
            if (!c.masterAxis.destroyed) { b("un", c, c.masterAxis) }
            c.masterAxis = null
        }
        if (a) {
            if (a.type !== this.type) { Ext.Error.raise("Linked axes must be of the same type.") }
            b("on", c, a);
    ...
    onDataChange: function(a) { this.getLayout().labels = a },
    onMasterAxisRangeChange: function(b, a) { this.range = a },
    applyRange: function(a) { if (!a) { return this.dataRange.slice(0) } else { return [a[0] === null ? this.dataRange[0] : a[0], a[1] === null ? this.dataRange[1] : a[1]] } },
    getRange: function() {
        var m = this;
        if (m.range) { return m.range } else { if (m.masterAxis) { return m.masterAxis.range } }
        if (Ext.isNumber(m.getMinimum()) && Ext.isNumber(m.getMaximum())) { return m.range = [m.getMinimum(), m.getMaximum()] }
        var d = Infinity,
            n = -Infinity,
            o = m.boundSeries,
            h = m.getLayout(),
            ...
            a, j, g, f, e, k;
        for (e = 0, k = o.length; e < k; e++) { f = o[e]; var c = f[b](); if (c) { if (c[0] < d) { d = c[0] } if (c[1] > n) { n = c[1] } } }
        if (!isFinite(n)) { n = m.prevMax }
        if (!isFinite(d)) { d = m.prevMin }
        if (m.getLabelInSpan() || d === n) {
            n += m.getIncrement();
            d -= m.getIncrement()
        }
        if (Ext.isNumber(m.getMinimum())) { d = m.getMinimum() } else { m.prevMin = d }
        if (Ext.isNumber(m.getMaximum())) { n = m.getMaximum() } else { m.prevMax = n }
        m.range = [Ext.Number.correctFloat(d), Ext.Number.correctFloat(n)];
        if (m.getReconcileRange()) { m.reconcileRange() }
    ...
    reconcileRange: function() {
        ...
        for (b = 0, d = g.length; b < d; b++) {
            c = g[b];
            a = c.getRange();
            if (c === e || c.getDirection() !== f || !a || !c.getReconcileRange()) { continue }
            if (a[0] < e.range[0]) { e.range[0] = a[0] }
            if (a[1] > e.range[1]) { e.range[1] = a[1] }
        }
    },
    applyStyle: function(c, b) {
        var a = Ext.ClassManager.getByAlias("sprite." + this.seriesType);
        if (a && a.def) { c = a.def.normalize(c) }
        b = Ext.apply(b || {}, c);
        return b
    },
    ...
    updateCenter: function(b) {
        var e = this.getSprites(),
            ...
            d = b[0],
            c = b[1];
        if (a) { a.setAttributes({ centerX: d, centerY: c }) }
        if (this.gridSpriteEven) { this.gridSpriteEven.getTemplate().setAttributes({ translationX: d, translationY: c, rotationCenterX: d, rotationCenterY: c }) }
        if (this.gridSpriteOdd) { this.gridSpriteOdd.getTemplate().setAttributes({ translationX: d, translationY: c, rotationCenterX: d, rotationCenterY: c }) }
    },
    getSprites: function() {
        ...
        var i = this,
            ...
            d, a, b = i.getLength(),
            h = i.superclass;
        if (c === false) { c = { duration: 0 } }
        if (e) {
            a = Ext.applyIf({ position: f, axis: i, min: e[0], max: e[1], length: b, grid: i.getGrid(), hidden: i.getHidden(), titleOffset: i.titleOffset, layout: i.getLayout(), segmenter: i.getSegmenter(), totalAngle: i.getTotalAngle(), label: i.getLabel() }, i.getStyle());
            if (!i.sprites.length) {
                while (!h.xtype) { h = h.superclass }
            ...
            if (i.getRenderer()) { d.setRenderer(i.getRenderer()) }
        }
        return i.sprites
    },
    ...
    updateTitleSprite: function() {
        var f = this,
            b = f.getLength();
        if (!f.sprites[0] || !Ext.isNumber(b)) { return }
        ...
        if (g) {
            switch (e) {
                case "top":
                    g.setAttributes({ x: d, y: c + i / 2, textBaseline: "top", textAlign: "center" }, true);
                    g.applyTransformations();
                    f.titleOffset = g.getBBox().height + i;
                    break;
                case "bottom":
                    g.setAttributes({ x: d, y: h + i / 2, textBaseline: "top", textAlign: "center" }, true);
                    g.applyTransformations();
                    f.titleOffset = g.getBBox().height + i;
                    break;
                case "left":
                    g.setAttributes({ x: c + i / 2, y: d, textBaseline: "top", textAlign: "center", rotationCenterX: c + i / 2, rotationCenterY: d, rotationRads: -Math.PI / 2 }, true);
                    g.applyTransformations();
                    f.titleOffset = g.getBBox().width + i;
                    break;
                case "right":
                    g.setAttributes({ x: h - c + i / 2, y: d, textBaseline: "bottom", textAlign: "center", rotationCenterX: h + i / 2, rotationCenterY: d, rotationRads: Math.PI / 2 }, true);
                    g.applyTransformations();
                    f.titleOffset = g.getBBox().width + i;
                    break
            }
    ...
    onThicknessChanged: function() { this.getChart().onThicknessChanged() },
    getThickness: function() { if (this.getHidden()) { return 0 } return (this.sprites[0] && this.sprites[0].thickness || 1) + this.titleOffset + this.getMargin() },
    onAnimationStart: function() { this.spriteAnimationCount++; if (this.spriteAnimationCount === 1) { this.fireEvent("animationstart", this) } },
    onAnimationEnd: function() { this.spriteAnimationCount--; if (this.spriteAnimationCount === 0) { this.fireEvent("animationend", this) } },
    getItemId: function() { return this.getId() },
    getAncestorIds: function() { return [this.getChart().getId()] },
    isXType: function(a) { return a === "axis" },
    resolveListenerScope: function(e) {
        var d = this,
            a = Ext._namedScopes[e],
            c = d.getChart(),
...
Ext.define("Ext.chart.legend.LegendBase", {
    ...
    setStore: function(a) { this.bindStore(a) },
...
Ext.define("Ext.chart.legend.Legend", {
    ...
    toggleItem: function(c) {
        ...
        if (b) {
            f = b.getCount();
            for (d = 0; d < f; d++) { a = b.getAt(d); if (a.get("disabled")) { h++ } }
            g = f - h > 1;
            a = b.getAt(c);
            if (a) { e = a.get("disabled"); if (e || g) { a.set("disabled", !e) } }
        }
    }
});
Ext.define("Ext.chart.legend.sprite.Item", {
    ...
    inheritableStatics: { def: { processors: { enabled: "limited01", markerLabelGap: "number" }, animationProcessors: { enabled: null, markerLabelGap: null }, defaults: { enabled: true, markerLabelGap: 5 }, triggers: { enabled: "enabled", markerLabelGap: "layout" }, updaters: { layout: "layoutUpdater", enabled: "enabledUpdater" } } },
    config: { label: { $value: { type: "text" }, lazy: true }, marker: { $value: { type: "circle" }, lazy: true }, legend: null, store: null, record: null, series: null },
    applyLabel: function(a, c) {
        var b;
        if (a) {
            if (a.isSprite && a.type === "text") { b = a } else {
                if (c && a.type === c.type) {
                    c.setConfig(a);
                    b = c;
                    this.scheduleUpdater(this.attr, "layout")
                } else { b = new Ext.draw.sprite.Text(a) }
            }
        }
        return b
    },
    ...
    applyMarker: function(b) {
        var a;
        if (b) { if (b.isSprite) { a = b } else { a = this.createMarker(b) } }
    ...
    resetMarker: function(d, b) {
        ...
        if (b.type === "image") { d.setAttributes({ width: c, height: c }) } else {
            f = d.getBBox();
    ...
    updateSurface: function(a, c) {
        var b = this;
        b.callParent([a, c]);
        if (a) { b.scheduleUpdater(b.attr, "layout") }
    },
    enabledUpdater: function(a) { var b = this.getMarker(); if (b) { b.setAttributes({ globalAlpha: a.enabled ? 1 : 0.3 }) } },
    layoutUpdater: function(b) {
        ...
        e.transform([1, 0, 0, 1, -d.x + a.width + b.markerLabelGap, -d.y + (f - d.height) / 2], true);
        g.bboxUpdater(b)
    }
});
...
Ext.define("Ext.draw.PathUtil", function() {
    ...
    return {
        singleton: true,
        requires: ["Ext.draw.overrides.hittest.Path", "Ext.draw.overrides.hittest.sprite.Path"],
        cubicRoots: function(m) {
            var z = m[0],
                x = m[1],
                w = m[2],
                v = m[3];
            if (z === 0) { return this.quadraticRoots(x, w, v) }
            var s = x / z,
                r = w / z,
                q = v / z,
                k = (3 * r - c(s, 2)) / 9,
            ...
            for (u = 0; u < 3; u++) { if (n[u] < 0 || n[u] > 1) { n[u] = -1 } }
            return n
        },
        quadraticRoots: function(h, g, n) {
            var m, l, k, j;
            if (h === 0) { return this.linearRoot(g, n) }
            m = g * g - 4 * h * n;
            if (m === 0) { k = [-g / (2 * h)] } else {
                if (m > 0) {
                    l = d(m);
            ...
            for (j = 0; j < k.length; j++) { if (k[j] < 0 || k[j] > 1) { k[j] = -1 } }
            return k
        },
        linearRoot: function(h, g) { var i = -g / h; if (h === 0 || i < 0 || i > 1) { return [] } return [i] },
        ...
        cubicLineIntersections: function(I, G, F, E, l, k, j, h, M, p, K, n) {
            ...
            for (H = 0; H < x.length; H++) {
                ...
                if ((K - M) !== 0) { w = (o - M) / (K - M) } else { w = (m - p) / (n - p) }
                if (!(w < 0 || w > 1)) { N.push([o, m]) }
            }
            return N
        },
        ...
        interpolateCubic: function(h, g, l, k, i) { if (i === 0) { return h } if (i === 1) { return k } var j = (1 - i) / i; return i * i * i * (k + j * (3 * l + j * (3 * g + j * h))) },
        cubicsIntersections: function(r, q, p, o, A, z, y, v, g, F, E, D, m, l, k, i) {
            var C = this,
                ...
                j, h, u, t, w = [];
            if (x[0] > n[1] || x[1] < n[0] || B[0] > s[1] || B[1] < s[0]) { return [] }
        ...
        linesIntersection: function(k, p, j, o, h, n, q, m) {
            ...
            i = ((q - h) * (p - n) - (k - h) * (m - n)) / l;
            g = ((j - k) * (p - n) - (o - p) * (k - h)) / l;
            if (i >= 0 && i <= 1 && g >= 0 && g <= 1) { return [k + i * (j - k), p + i * (o - p)] }
            return null
        },
        ...
        pointOnCubic: function(w, u, s, r, l, k, h, g, p, o) {
            ...
            n = C.cubicRoots(B);
            m = C.cubicRoots(A);
            for (z = 0; z < n.length; z++) { q = n[z]; for (v = 0; v < m.length; v++) { if (q >= 0 && q <= 1 && a(q - m[v]) < 0.05) { return true } } }
            return false
        }
    }
});
...
Ext.define("Ext.chart.legend.SpriteLegend", {
    ...
    isLegend: true,
    isSpriteLegend: true,
    requires: ["Ext.chart.legend.sprite.Item", "Ext.chart.legend.sprite.Border", "Ext.draw.overrides.hittest.All", "Ext.draw.Animator"],
    config: { docked: "bottom", store: "ext-empty-store", chart: null, surface: null, size: { width: 0, height: 0 }, toggleable: true, padding: 10, label: { preciseMeasurement: true }, marker: {}, border: { $value: { type: "legendborder" }, lazy: true }, background: null },
    sprites: null,
    ...
    applyBorder: function(b) {
        var a;
        if (b) { if (b.isSprite) { a = b } else { a = Ext.create("sprite." + b.type, b) } }
        if (a) {
    ...
    updateBorder: function(c, b) {
        ...
        if (a) { if (b) { a.remove(b) } if (c) { this.borderSprite = a.add(c) } }
    },
    scheduleLayout: function() { if (!this.scheduledLayoutId) { this.scheduledLayoutId = Ext.draw.Animator.schedule("performLayout", this) } },
    ...
    performLayout: function() {
        ...
        if (w) { w.setAttributes({ x: E - p, y: C - p, width: c + p * 2, height: D + p * 2 }) }
        ...
        Ext.apply(I.oldSize, o);
        if (A) { I.resizeBackground(u, A) }
        u.renderFrame();
        return m
    },
    getSprites: function() { this.updateSprites(); return this.sprites },
    createSprite: function(b, f) {
        ...
        if (b) {
            ...
            if (g && g.type && !c.type) { c.type = g.type }
            i = j.getLabel();
            a = { type: "legenditem", zIndex: j.spriteZIndexes.item, text: d.name, enabled: !d.disabled, marker: c, label: i, series: d.series, record: f };
            k = b.add(a)
        }
        return k
    },
    ...
    updateChart: function(c, a) { var b = this; if (a) { b.setSurface(null) } if (c) { b.setSurface(c.getSurface("legend")) } },
    updateSurface: function(a, b) {
        if (b) { b.el.un("click", "onClick", this) }
        if (a) {
    ...
    onClick: function(e, c) {
        ...
        if (d && d.hasFirstLayout && c) {
            a = c.getEventXY(e);
            b = c.hitTest(a);
            if (b && b.sprite) { this.toggleItem(b.sprite) }
        }
    },
    applyBackground: function(b, e) {
        ...
        if (c) { c.setAttributes({ zIndex: f.spriteZIndexes.background }) }
        return c
    },
    resizeBackground: function(b, d) {
        var e = d.attr.width,
            a = d.attr.height,
            c = b.getRect();
        if (c && (e !== c[2] || a !== c[3])) { d.setAttributes({ width: c[2], height: c[3] }) }
    },
    ...
    updateTheme: function(u) {
        ...
        for (q = 0, f = e.length; q < f; q++) {
            ...
            if (m.isLegendItem) {
                ...
                continue
            } else { if (m.isLegendBorder) { s = t.border } else { continue } }
            if (s) {
                n = {};
                for (w in s) { if (!(w in m.config)) { n[w] = s[w] } }
                m.setAttributes(n)
            }
        }
        o = t.background;
        ...
        for (w in t) {
            ...
            if (o !== null && o !== undefined && c) {
                ...
                h = b === k[w];
                if (d) {
                    if (h && themeOnlyIfConfigured[w]) { continue }
                    o = Ext.merge({}, o, b)
                }
                if (h || d) { v[c.names.set](o) }
            }
        }
    },
    ...
    toggleItem: function(d) {
        if (!this.getToggleable() || !d.isLegendItem) { return }
        var b = this.getStore(),
            h = 0,
            e, g = true,
            c, f, a;
        if (b) {
            f = b.getCount();
            for (c = 0; c < f; c++) { a = b.getAt(c); if (a.get("disabled")) { h++ } }
            g = f - h > 1;
            a = d.getRecord();
            if (a) {
                e = a.get("disabled");
...
Ext.define("Ext.chart.AbstractChart", {
    ...
    requires: ["Ext.chart.theme.Default", "Ext.chart.series.Series", "Ext.chart.interactions.Abstract", "Ext.chart.axis.Axis", "Ext.data.StoreManager", "Ext.chart.legend.Legend", "Ext.chart.legend.SpriteLegend", "Ext.chart.legend.store.Store", "Ext.data.Store"],
    isChart: true,
    defaultBindProperty: "store",
    config: { store: "ext-empty-store", theme: "default", style: null, animation: !Ext.isIE8, series: [], axes: [], legend: null, colors: null, insetPadding: { top: 10, left: 10, right: 10, bottom: 10 }, background: null, interactions: [], mainRect: null, resizeHandler: null, highlightItem: null },
    animationSuspendCount: 0,
    chartLayoutSuspendCount: 0,
    axisThicknessSuspendCount: 0,
    isThicknessChanged: false,
    surfaceZIndexes: { background: 0, main: 1, grid: 2, series: 3, axis: 4, chart: 5, overlay: 6, legend: 7, title: 8 },
    constructor: function(a) {
        var b = this;
        b.itemListeners = {};
        b.surfaceMap = {};
    ...
    applyAnimation: function(a, b) { if (!a) { a = { duration: 0 } } else { if (a === true) { a = { easing: "easeInOut", duration: 500 } } } return b ? Ext.apply({}, a, b) : a },
    getAnimation: function() { if (this.animationSuspendCount) { return { duration: 0 } } else { return this.callParent() } },
    applyInsetPadding: function(b, a) { if (!Ext.isObject(b)) { return Ext.util.Format.parseBox(b) } else { if (!a) { return b } else { return Ext.apply(a, b) } } },
    ...
    suspendChartLayout: function() {
        ...
        if (a.chartLayoutSuspendCount === 1) {
            if (a.scheduledLayoutId) {
                ...
                a.cancelChartLayout()
            } else { a.layoutInSuspension = false }
        }
    },
    resumeChartLayout: function() {
        var a = this;
        a.chartLayoutSuspendCount--;
        if (a.chartLayoutSuspendCount === 0) { if (a.layoutInSuspension) { a.scheduleLayout() } }
    },
    ...
    scheduleLayout: function() { var a = this; if (a.allowSchedule() && !a.scheduledLayoutId) { a.scheduledLayoutId = Ext.draw.Animator.schedule("doScheduleLayout", a) } },
    allowSchedule: function() { return true },
    doScheduleLayout: function() {
        var a = this;
        if (a.chartLayoutSuspendCount) { a.layoutInSuspension = true } else { a.performLayout() }
        a.scheduledLayoutId = null
    },
    suspendThicknessChanged: function() { this.axisThicknessSuspendCount++ },
    resumeThicknessChanged: function() { if (this.axisThicknessSuspendCount > 0) { this.axisThicknessSuspendCount--; if (this.axisThicknessSuspendCount === 0 && this.isThicknessChanged) { this.onThicknessChanged() } } },
    onThicknessChanged: function() {
        if (this.axisThicknessSuspendCount === 0) {
            this.isThicknessChanged = false;
            this.performLayout()
        } else { this.isThicknessChanged = true }
    },
    ...
    initItems: function() {
        ...
        if (a && !a.isMixedCollection) {
            this.items = [];
            a = Ext.Array.from(a);
            for (b = 0, d = a.length; b < d; b++) { c = a[b]; if (c.type) { Ext.raise("To add custom sprites to the chart use the 'sprites' config.") } else { this.items.push(c) } }
        }
        this.callParent()
    },
    applyBackground: function(b, c) { var a = this.getSurface("background"); return this.refreshBackground(a, b, c) },
    refreshBackground: function(c, b, e) {
        ...
        if (b) {
            ...
            if (b.isSprite) { e = b } else {
                if (b.type === "image" && Ext.isString(b.src)) {
                    if (f) { e.setAttributes({ src: b.src }) } else {
                        c.remove(e, true);
                        e = c.add(b)
                    }
                } else {
                    if (f) { e.setAttributes({ fillStyle: b }) } else {
                        c.remove(e, true);
                        e = c.add({ type: "rect", fillStyle: b, fx: { customDurations: { x: 0, y: 0, width: 0, height: 0 } } })
                    }
                }
            }
        }
        if (d && a) { e.setAttributes({ width: d, height: a }) }
    ...
    refreshLegendStore: function() {
        ...
        if (e) {
            var c = g.getSeries(),
                ...
                a = [],
                b = 0;
            for (; b < f; b++) { d = c[b]; if (d.getShowInLegend()) { d.provideLegendInfo(a) } }
            e.setData(a)
        }
    },
    ...
    defaultResizeHandler: function(a) { this.scheduleLayout(); return false },
    applyMainRect: function(a, b) {
        if (!b) { return a }
        this.getSeries();
        this.getAxes();
        if (a[0] === b[0] && a[1] === b[1] && a[2] === b[2] && a[3] === b[3]) { return b } else { return a }
    },
    register: function(a) {
    ...
    get: function(a) { return this.chartComponents[a] },
    getAxis: function(a) { if (a instanceof Ext.chart.axis.Axis) { return a } else { if (Ext.isNumber(a)) { return this.getAxes()[a] } else { if (Ext.isString(a)) { return this.get(a) } } } },
    getSurface: function(b, c) {
        ...
        var d = this,
            ...
            f = d.surfaceZIndexes,
            e = d.surfaceMap;
        if (c in f) { a.element.setStyle("zIndex", f[c]) }
        if (!e[c]) { e[c] = [] }
    ...
    applyAxes: function(b, k) {
        ...
        for (f = 0, h = b.length; f < h; f++) {
            ...
            if (c instanceof Ext.chart.axis.Axis) {
                ...
                e = c.linkedTo;
                a = c.id;
                if (Ext.isNumber(e)) { c = Ext.merge({}, b[e], c) } else { if (Ext.isString(e)) { Ext.Array.each(b, function(i) { if (i.id === c.linkedTo) { c = Ext.merge({}, i, c); return false } }) } }
                c.id = a;
                c.chart = l;
                if (l.getInherited().rtl) { c.position = g[c.position] || c.position }
        ...
        for (f in j) { if (!m.map[f]) { j[f].destroy() } }
        l.animationSuspendCount--;
        return m
    },
    updateAxes: function() { if (!this.isDestroying) { this.scheduleLayout() } },
    circularCopyArray: function(e, f, d) {
        var c = [],
            b, a = e && e.length;
        if (a) { for (b = 0; b < d; b++) { c.push(e[(f + b) % a]) } }
        return c
    },
    circularCopyObject: function(f, g, d) {
        var c = this,
            b, e, a = {};
        if (d) { for (b in f) { if (f.hasOwnProperty(b)) { e = f[b]; if (Ext.isArray(e)) { a[b] = c.circularCopyArray(e, g, d) } else { a[b] = e } } } }
        return a
    },
    getColors: function() {
        var b = this,
            a = b.config.colors,
            c = b.getTheme();
        if (Ext.isArray(a) && a.length > 0) { a = b.applyColors(a) }
        return a || (c && c.getColors())
    },
    applyColors: function(a) { a = Ext.Array.map(a, function(b) { if (Ext.isString(b)) { return b } else { return b.toString() } }); return a },
    ...
    applyTheme: function(a) { if (a && a.isTheme) { return a } return Ext.Factory.chartTheme(a) },
    updateGradients: function(a) { if (!Ext.isEmpty(a)) { this.updateTheme(this.getTheme()) } },
    updateTheme: function(g) {
        ...
        for (b = 0; b < f.length; b++) { f[b].updateTheme(g) }
    ...
    updateSpriteTheme: function(c) {
        ...
        for (b = 0, g = h.length; b < g; b++) {
            ...
            if (a) {
                f = {};
                d = k.type === "text";
                for (l in a) { if (!(l in k.config)) { if (!(d && l.indexOf("font") === 0 && k.config.font)) { f[l] = a[l] } } }
                k.setAttributes(f)
            }
        }
    },
    ...
    removeSeries: function(d) {
        ...
        for (c = 0; c < a; c++) {
            e = d[c];
            if (typeof e !== "string") { e = e.getId() }
            g[e] = true
        }
        for (c = 0, a = b.length; c < a; c++) { if (!g[b[c].getId()]) { f.push(b[c]) } }
        this.setSeries(f)
    },
    applySeries: function(e, d) {
        ...
        for (c in h) { if (!j.map[h[c].id]) { h[c].destroy() } }
        g.animationSuspendCount--;
        return j
    },
    ...
    applyLegend: function(c) {
        var d = this,
            a, b;
        if (c) {
            if (Ext.isBoolean(c)) { a = Ext.create("legend." + d.defaultLegendType, { docked: "bottom", chart: d }) } else {
    ...
    updateLegend: function(b, a) {
        ...
        if (b) {
            c.getSurface("legend");
            c.getItems();
            c.legendStore = new Ext.chart.legend.store.Store({ chart: c, store: c.legendStore });
    ...
    updateSeries: function(b, a) {
        var c = this;
        if (c.isDestroying) { return }
        c.animationSuspendCount++;
        c.fireEvent("serieschange", c, b, a);
        c.refreshLegendStore();
        if (!Ext.isEmpty(b)) { c.updateTheme(c.getTheme()) }
        c.scheduleLayout();
        c.animationSuspendCount--
    },
    applyInteractions: function(h, d) {
        ...
        for (e in c) { if (!a.map[e]) { c[e].destroy() } }
        return a
    },
    getInteraction: function(e) {
        var f = this.getInteractions(),
            ...
            c = null,
            b, d;
        if (a) { for (d = 0; d < a; ++d) { b = f[d]; if (b.type === e) { c = b; break } } }
        return c
    },
    applyStore: function(a) { return a && Ext.StoreManager.lookup(a) },
    updateStore: function(a, c) {
        var b = this;
        if (c && !c.destroyed) { c.un({ datachanged: "onDataChanged", update: "onDataChanged", scope: b, order: "after" }); if (c.autoDestroy) { c.destroy() } }
        if (a) { a.on({ datachanged: "onDataChanged", update: "onDataChanged", scope: b, order: "after" }) }
        b.fireEvent("storechange", b, a, c);
        b.onDataChanged()
    },
    redraw: function() { this.fireEvent("redraw", this) },
    performLayout: function() {
        var e = this,
            c = e.getLegend(),
    ...
    getChartRect: function(d) {
        var c = this,
            b, a;
        if (d) { c.chartRect = null }
        if (c.chartRect) { b = c.chartRect } else {
            a = c.innerElement.getSize();
            b = c.chartRect = [0, 0, a.width, a.height]
        }
        if (d) { c.computeSpriteLegendRect(b) }
        return b
    },
    ...
    getEventXY: function(a) { return this.getSurface().getEventXY(a) },
    getItemForPoint: function(h, g) {
        var f = this,
            ...
            c, j;
        if (!(e && h >= 0 && h <= e[2] && g >= 0 && g <= e[3])) { return null }
        for (; b >= 0; b--) {
            c = a[b];
    ...
    onAnimationStart: function() { this.fireEvent("animationstart", this) },
    onAnimationEnd: function() { this.fireEvent("animationend", this) },
    onDataChanged: function() {
        var d = this;
        if (d.isInitializing) { return }
        var c = d.getMainRect(),
            a = d.getStore(),
            b = d.getSeries(),
            e = d.getAxes();
        if (!a || !e || !b) { return }
        if (!c) { d.on({ redraw: d.onDataChanged, scope: d, single: true }); return }
        d.processData();
        d.redraw()
    },
    recordCount: 0,
    processData: function() {
        ...
        for (; b < f; b++) {
            a = c[b];
            a.processData();
            if (!d && a.isStoreDependantColorCount) { d = true }
        }
    ...
    bindStore: function(a) { this.setStore(a) },
    applyHighlightItem: function(f, a) {
        ...
        if (Ext.isObject(f) && Ext.isObject(a)) {
            ...
            if (c === b && e.index === d.index) { return }
        }
        return f
    },
    updateHighlightItem: function(b, a) {
        if (a) { a.series.setAttributesForItem(a, { highlighted: false }) }
        if (b) {
    ...
    destroyChart: function() {
        var f = this,
            ...
            a, e;
        f.surfaceMap = null;
        for (a = 0, e = h.length; a < e; a++) { h[a].destroy() }
        for (a = 0, e = g.length; a < e; a++) { g[a].destroy() }
        for (a = 0, e = c.length; a < e; a++) { c[a].destroy() }
        f.setInteractions(b);
        f.setAxes(b);
        f.setSeries(b);
        if (d) {
    ...
    getRefItems: function(b) {
        var g = this,
            ...
            c = [],
            d, f;
        for (d = 0, f = e.length; d < f; d++) { c.push(e[d]); if (e[d].getRefItems) { c.push.apply(c, e[d].getRefItems(b)) } }
        for (d = 0, f = h.length; d < f; d++) { c.push(h[d]); if (h[d].getRefItems) { c.push.apply(c, h[d].getRefItems(b)) } }
        for (d = 0, f = a.length; d < f; d++) { c.push(a[d]); if (a[d].getRefItems) { c.push.apply(c, a[d].getRefItems(b)) } }
        return c
    }
});
Ext.define("Ext.chart.overrides.AbstractChart", {
    override: "Ext.chart.AbstractChart",
    updateLegend: function(b, a) { this.callParent([b, a]); if (b && b.isDomLegend) { this.addDocked(b) } },
    performLayout: function() {
        if (this.isVisible(true)) { return this.callParent() }
    ...
    allowSchedule: function() { return this.rendered },
    doDestroy: function() {
        this.destroyChart();
        this.callParent()
    }
...
Ext.define("Ext.chart.grid.HorizontalGrid", {
    ...
    inheritableStatics: { def: { processors: { x: "number", y: "number", width: "number", height: "number" }, defaults: { x: 0, y: 0, width: 1, height: 1, strokeStyle: "#DDD" } } },
...
Ext.define("Ext.chart.CartesianChart", {
    ...
    applyInnerPadding: function(b, a) { if (!Ext.isObject(b)) { return Ext.util.Format.parseBox(b) } else { if (!a) { return b } else { return Ext.apply(a, b) } } },
    getDirectionForAxis: function(a) { var b = this.getFlipXY(); if (a === "left" || a === "right") { if (b) { return "X" } else { return "Y" } } else { if (b) { return "Y" } else { return "X" } } },
    performLayout: function() {
        var A = this;
        A.animationSuspendCount++;
        if (A.callParent() === false) {--A.animationSuspendCount; return }
        ...
        for (x = 0, g = q.length; x < g; x++) {
            ...
            r = h.getSurface();
            r.setRect(u);
            if (w) { if (j) { r.matrix.set(0, -1, -1, 0, v.left + p, v.top + s) } else { r.matrix.set(0, -1, 1, 0, v.left, v.top + s) } } else { r.matrix.set(1, 0, 0, -1, v.left, v.top + s) }
    ...
    refloatAxes: function() {
        var j = this,
            g = j.getAxes(),
            p = (g && g.length) || 0,
            c, d, o, f, m, b, l, h = j.getChartRect(),
            r = j.getInsetPadding(),
            q = j.getInnerPadding(),
        ...
        for (e = 0; e < p; e++) {
            ...
            f = c.getFloating();
            m = f ? f.value : null;
            if (m === null) { c.floatingAtCoord = null; continue }
            d = c.getSurface();
            o = d.getRect();
            if (!o) { continue }
            o = o.slice();
            ...
            if (b) {
                ...
                if (Ext.isString(m)) { m = b.getCoordFor(m) }
                ...
                if (k) { c.floatingAtCoord = m + q.top + q.bottom } else { c.floatingAtCoord = m + q.left + q.right }
    ...
    redraw: function() {
        ...
        for (A = 0; A < r.length; A++) {
            ...
            q = { visibleMinX: l[0], visibleMaxX: l[1], visibleMinY: s[0], visibleMaxY: s[1], innerWidth: p, innerHeight: t, flipXY: y };
            f = h.getSprites();
            for (v = 0, g = f.length; v < g; v++) {
                ...
                if (m < x) {
                    ...
                    o.attr.zIndex = m;
                    B = o.getMarker("items");
                    if (B) { u = B.attr.zIndex; if (u === Number.MAX_VALUE) { B.attr.zIndex = m } else { if (u < x) { B.attr.zIndex = m + u } } }
                }
        ...
        for (A = 0; A < z.length; A++) {
            ...
            for (v = 0, g = f.length; v < g; v++) { f[v].setAttributes(q, true) }
        }
        C.renderFrame();
...
Ext.define("Ext.chart.grid.RadialGrid", {
    extend: "Ext.draw.sprite.Path",
    alias: "grid.radial",
    inheritableStatics: { def: { processors: { startRadius: "number", endRadius: "number" }, defaults: { startRadius: 0, endRadius: 1, scalingCenterX: 0, scalingCenterY: 0, strokeStyle: "#DDD" }, triggers: { startRadius: "path,bbox", endRadius: "path,bbox" } } },
    render: function() { this.callParent(arguments) },
...
Ext.define("Ext.chart.PolarChart", {
    ...
    getDirectionForAxis: function(a) { return a === "radial" ? "Y" : "X" },
    applyCenter: function(a, b) { if (b && a[0] === b[0] && a[1] === b[1]) { return } return [+a[0], +a[1]] },
    ...
    applyInnerPadding: function(b, a) { return Ext.isNumber(b) ? b : a },
    ...
    applyAxes: function(f, h) {
        var e = this,
            ...
            b, d, c, a;
        if (g.type === "radar" && f && f.length) { for (b = 0, d = f.length; b < d; b++) { c = f[b]; if (c.position === "angular") { a = true; break } } if (!a) { f.push({ type: "category", position: "angular", fields: g.xField || g.angleField, style: { estStepSize: 1 }, grid: true }) } }
    ...
    performLayout: function() {
        ...
        try {
            ...
            if (this.callParent() === false) { g = false; return }
            ...
            var h = E.getSurface("chart").getRect(),
                ...
                D = j - F,
                b = E.surfaceMap.grid,
                z, n, c, q, y, e, B, d, o, f, a, k;
            E.setMainRect(x);
            E.doSetSurfaceRect(E.getSurface(), x);
            if (b) { for (z = 0, n = b.length; z < n; z++) { E.doSetSurfaceRect(b[z], h) } }
            ...
            for (z = 0, n = u.length; z < n; z++) {
                o = u[z];
                if (o.type === "gauge" && !e) { e = o } else { o.setRadius(D) }
            ...
            E.redraw()
        } finally { E.animationSuspendCount--; if (g) { E.resumeThicknessChanged() } }
    },
    refloatAxes: function() {
        var j = this,
            g = j.getAxes(),
        ...
        for (d = 0, a = g.length; d < a; d++) {
            ...
            if (k !== null) {
                ...
                if (c.getPosition() === "angular") {
                    if (b) { k = b.getLength() * k / b.getRange()[1] } else { k = 0.01 * k * e }
                    c.sprites[0].setAttributes({ length: k }, true)
                } else {
                    if (b) {
                        if (Ext.isString(k)) { k = b.getCoordFor(k) }
                        k = k / (b.getRange()[1] + 1) * Math.PI * 2 - Math.PI * 1.5 + c.getRotation()
                    } else { k = Ext.draw.Draw.rad(k) }
    ...
    performLayout: function() {
        ...
        try {
            ...
            for (c = 0, e = b.length; c < e; c++) {
                ...
                if (d.setRect) { d.setRect(f) }
                d.getOverlaySurface().setRect(h)
            }
            g.redraw()
        } finally { g.animationSuspendCount-- }
    },
    redraw: function() {
        var e = this,
            c = e.getSeries(),
...
Ext.define("Ext.chart.axis.Axis3D", {
    ...
    onSeriesDepthChange: function(b, f) {
        var d = this,
            ...
            e = d.boundSeries,
            a, c;
        if (f > d.getDepth()) { g = f } else { for (a = 0; a < e.length; a++) { c = e[a]; if (c !== b && c.getDepth) { f = c.getDepth(); if (f > g) { g = f } } } }
        d.setDepth(g)
    },
    updateDepth: function(d) {
        var b = this,
            c = b.getSprites(),
            a = { depth: d };
        if (c && c.length) { c[0].setAttributes(a) }
...
Ext.define("Ext.chart.interactions.CrossZoom", {
    ...
    isCrossZoom: true,
    config: { axes: true, gestures: { dragstart: "onGestureStart", drag: "onGesture", dragend: "onGestureEnd", dblclick: "onDoubleTap" }, undoButton: {} },
    stopAnimationBeforeSync: false,
    ...
    applyAxes: function(b) {
        var a = {};
        if (b === true) { return { top: {}, right: {}, bottom: {}, left: {} } } else {
            if (Ext.isArray(b)) {
                a = {};
                Ext.each(b, function(c) { a[c] = {} })
            } else { if (Ext.isObject(b)) { Ext.iterate(b, function(c, d) { if (d === true) { a[c] = {} } else { if (d !== false) { a[c] = d } } }) } }
        }
        return a
    },
    applyUndoButton: function(b, a) { var c = this; if (a) { a.destroy() } if (b) { return Ext.create("Ext.Button", Ext.apply({ cls: [], text: "Undo Zoom", disabled: true, handler: function() { c.undoZoom() } }, b)) } },
    getSurface: function() { return this.getChart() && this.getChart().getSurface("main") },
    setSeriesOpacity: function(b) { var a = this.getChart() && this.getChart().getSurface("series"); if (a) { a.element.setStyle("opacity", b) } },
    onGestureStart: function(h) {
        var j = this,
            ...
            k = n[1];
        h.claimGesture();
        if (j.zoomAnimationInProgress) { return }
        if (m > g && m < b && k > f && k < a) {
            j.gestureEvent = "drag";
            j.lockEvents(j.gestureEvent);
            j.startX = m;
            j.startY = k;
            j.selectionRect = d.add({ type: "rect", globalAlpha: 0.5, fillStyle: "rgba(80,80,140,0.5)", strokeStyle: "rgba(80,80,140,1)", lineWidth: 2, x: m, y: k, width: 0, height: 0, zIndex: 10000 });
    ...
    onGesture: function(h) {
        ...
        if (j.getLocks()[j.gestureEvent] === j) {
            var i = j.getChart(),
                ...
                m = n[0],
                k = n[1];
            if (m < g) { m = g } else { if (m > b) { m = b } }
            if (k < f) { k = f } else { if (k > a) { k = a } }
            j.selectionRect.setAttributes({ width: m - j.startX, height: k - j.startY });
            if (Math.abs(j.startX - m) < 11 || Math.abs(j.startY - k) < 11) { j.selectionRect.setAttributes({ globalAlpha: 0.5 }) } else { j.selectionRect.setAttributes({ globalAlpha: 1 }) }
            d.renderFrame();
            return false
        }
    },
    onGestureEnd: function(i) {
        var l = this;
        if (l.zoomAnimationInProgress) { return }
        if (l.getLocks()[l.gestureEvent] === l) {
            var k = l.getChart(),
                ...
                o = p[0],
                m = p[1];
            if (o < g) { o = g } else { if (o > b) { o = b } }
            if (m < f) { m = f } else { if (m > a) { m = a } }
            if (Math.abs(l.startX - o) < 11 || Math.abs(l.startY - m) < 11) { d.remove(l.selectionRect) } else {
                l.zoomBy([Math.min(l.startX, o) / h, 1 - Math.max(l.startY, m) / j, Math.max(l.startX, o) / h, 1 - Math.min(l.startY, m) / j]);
                l.selectionRect.setAttributes({ x: Math.min(l.startX, o), y: Math.min(l.startY, m), width: Math.abs(l.startX - o), height: Math.abs(l.startY - m) });
        ...
        for (var h = 0; h < j.length; h++) {
            ...
            if (f && f.allowZoom !== false) {
                var e = g.isSide(),
                    m = g.getVisibleRange();
                d[g.getId()] = m.slice(0);
                if (!e) { g.setVisibleRange([(m[1] - m[0]) * o[0] + m[0], (m[1] - m[0]) * o[2] + m[0]]) } else { g.setVisibleRange([(m[1] - m[0]) * o[1] + m[0], (m[1] - m[0]) * o[3] + m[0]]) }
            }
        }
    ...
    undoZoom: function() {
        ...
        if (c) { for (var a = 0; a < d.length; a++) { var b = d[a]; if (c[b.getId()]) { b.setVisibleRange(c[b.getId()]) } } }
        this.getUndoButton().setDisabled(this.zoomHistory.length === 0);
        this.sync()
    },
    onDoubleTap: function(a) { this.undoZoom() },
    destroy: function() {
...
Ext.define("Ext.chart.interactions.Crosshair", {
    ...
    config: { axes: { top: { label: {}, rect: {} }, right: { label: {}, rect: {} }, bottom: { label: {}, rect: {} }, left: { label: {}, rect: {} } }, lines: { horizontal: { strokeStyle: "black", lineDash: [5, 5] }, vertical: { strokeStyle: "black", lineDash: [5, 5] } }, gesture: "drag" },
    applyAxes: function(b, a) { return Ext.merge(a || {}, b) },
    applyLines: function(a, b) { return Ext.merge(b || {}, a) },
    updateChart: function(a) {
        if (a && !a.isCartesian) { Ext.raise("Crosshair interaction can only be used on cartesian charts.") }
        this.callParent(arguments)
    },
    ...
    onGestureStart: function(N) {
        ...
        if (D > 0 && D < n && C > 0 && C < M) {
            ...
            for (K = 0; K < E.length; K++) {
                ...
                L.labelRect = L.add(Ext.apply({ type: "rect", fillStyle: "white", x: z === "right" ? p : 0, y: z === "bottom" ? p : 0, width: d - p - (G === "vertical" && a ? a.width : 0), height: k - p - (G === "horizontal" && a ? a.height : 0), translationX: z === "left" && a ? a.width : 0, translationY: z === "top" && a ? a.height : 0 }, u.rect || u[z].rect));
                if (G === "vertical" && !c.strokeStyle) { c.strokeStyle = l.strokeStyle }
    ...
    onGesture: function(G) {
        var K = this;
        if (K.getLocks()[K.getGesture()] !== K) { return }
        var u = K.getChart(),
        ...
        if (k < 0) { k = 0 } else { if (k > E) { k = E } }
        if (j < 0) { j = 0 } else { if (j > f) { j = f } }
        k += t;
        j += q;
        for (B = 0; B < D.length; B++) {
            c = D[B];
...
Ext.define("Ext.chart.interactions.ItemHighlight", {
    ...
    config: { gestures: { tap: "onTapGesture", mousemove: "onMouseMoveGesture", mousedown: "onMouseDownGesture", mouseup: "onMouseUpGesture", mouseleave: "onMouseUpGesture" }, sticky: false },
            ...
            if (!d.stickyHighlightItem) {
                ...
                if (a) {
                    ...
                    if (c && (f = c.series.getTooltip())) {
                        if (f.trackMouse || !h) { c.series.showTooltip(c, g) }
                        d.tipItem = c
                    }
                }
                return false
    ...
    highlight: function(a) { this.getChart().setHighlightItem(a) },
    showTooltip: function(b, a) {
        a.series.showTooltip(a, b);
        this.tipItem = a
    },
    onMouseDownGesture: function() { this.isDragging = true },
    onMouseUpGesture: function() { this.isDragging = false },
    onTapGesture: function(c) {
        var b = this;
        if (c.pointerType === "mouse" && !b.getSticky()) { return }
        var a = b.getItemForEvent(c);
        if (b.stickyHighlightItem && a && (b.stickyHighlightItem.index === a.index)) { a = null }
...
Ext.define("Ext.chart.interactions.ItemEdit", {
    ...
    isItemEdit: true,
    config: { style: null, renderer: null, tooltip: true, gestures: { dragstart: "onDragStart", drag: "onDrag", dragend: "onDragEnd" }, cursors: { ewResize: "ew-resize", nsResize: "ns-resize", move: "move" } },
    item: null,
    applyTooltip: function(b) {
        if (b) {
            var a = Ext.apply({}, b, { renderer: this.defaultTooltipRenderer, constrainPosition: true, shrinkWrapDock: true, autoHide: true, trackMouse: true, mouseOffset: [20, 20] });
    ...
    defaultTooltipRenderer: function(b, a, f, d) {
        var c = [];
        if (f.xField) { c.push(f.xField + ": " + f.xValue) }
        if (f.yField) { c.push(f.yField + ": " + f.yValue) }
        b.setHtml(c.join("<br>"))
    },
    onDragStart: function(d) {
        ...
        d.claimGesture();
        if (b) { a.fireEvent("beginitemedit", a, c, c.item = b); return false }
    },
    onDrag: function(f) {
        var d = this,
            b = d.getChart(),
    ...
    highlight: function(f) {
        ...
        if (f) {
            switch (c) {
                case "barSeries":
                    if (a) { b.cursor = g.ewResize } else { b.cursor = g.nsResize }
                    break;
                case "scatterSeries":
                    b.cursor = g.move;
                    break
            }
        } else { d.el.dom.style.cursor = "default" }
    },
    onDragBar: function(i) {
        var m = this,
            ...
            j = m.getRenderer(),
            a, n, d, h;
        if (f) { h = l ? c[2] - r[0] : r[0] } else { h = c[3] - r[1] }
        a = { x: p.x, y: h, width: p.width, height: p.height + (p.y - h), radius: p.radius, fillStyle: "none", lineDash: [4, 4], zIndex: 100 };
        Ext.apply(a, m.getStyle());
        if (Ext.isArray(q.series.getYField())) { h = h - p.y - p.height }
        m.target = { index: q.index, yField: q.field, yValue: (h - o.getDY()) / o.getYY() };
    ...
    onDragScatter: function(n) {
        var t = this,
            ...
            i = t.getRenderer(),
            m, u, q, s, r;
        if (l) { r = d ? h[2] - a[0] : a[0] } else { r = h[3] - a[1] }
        if (f) { if (l) { s = h[3] - a[1] } else { s = a[0] } } else { s = p.translationX }
        m = { translationX: s, translationY: r, scalingX: p.scalingX, scalingY: p.scalingY, r: p.r, fillStyle: "none", lineDash: [4, 4], zIndex: 100 };
        Ext.apply(m, t.getStyle());
        t.target = { index: o.index, yField: o.field, yValue: (r - k.getDY()) / k.getYY() };
        if (f) { Ext.apply(t.target, { xField: o.series.getXField(), xValue: (s - k.getDX()) / k.getXX() }) }
    ...
    hideTooltip: function() { var a = this.getTooltip(); if (a && Ext.toolkit !== "modern") { a.hide() } },
    onDragEnd: function(g) {
        ...
        if (f) {
            a = b.getAt(f.index);
            if (f.yField) { a.set(f.yField, f.yValue, { convert: false }) }
            if (f.xField) { a.set(f.xField, f.xValue, { convert: false }) }
            if (f.yField || f.xField) { d.getChart().onDataChanged() }
            d.target = null
        }
        d.hideTooltip();
        if (d.item) { c.fireEvent("enditemedit", c, d, d.item, f) }
...
Ext.define("Ext.chart.interactions.PanZoom", {
    ...
    config: { axes: { top: {}, right: {}, bottom: {}, left: {} }, minZoom: null, maxZoom: null, showOverflowArrows: true, panGesture: "drag", zoomGesture: "pinch", zoomOnPanGesture: null, zoomOnPan: false, modeToggleButton: { xtype: "segmentedbutton", width: 200, defaults: { ui: "default-toolbar" }, cls: Ext.baseCSSPrefix + "panzoom-toggle", items: [{ text: "Pan", value: "pan" }, { text: "Zoom", value: "zoom" }] }, hideLabelInGesture: false },
    ...
    updateZoomOnPanGesture: function(a) { this.setZoomOnPan(a) },
    getZoomOnPanGesture: function() { return this.getZoomOnPan() },
    applyModeToggleButton: function(c, b) {
        var d = this,
            a = Ext.factory(c, "Ext.button.Segmented", b);
        if (!a && b) { b.destroy() }
        if (a && !b) { a.on("toggle", "onModeToggleChange", d) }
        return a
    },
    onModeToggleChange: function(c, a, b) { if (b) { this.setZoomOnPan(a.getValue() === "zoom") } },
    getGestures: function() {
        var b = this,
            d = {},
            c = b.getPanGesture(),
    ...
    onPanGestureMove: function(g) {
        ...
        if (d.getLocks()[d.getPanGesture()] === d) {
            ...
            if (a) { d.transformAxesBy(d.getZoomableAxes(g), 0, 0, (g.getX() - f[0] - c[0]) / d.startX, d.startY / (g.getY() - f[1] - c[1])) } else { d.transformAxesBy(d.getPannableAxes(g), g.getX() - f[0] - c[0] - d.startX, g.getY() - f[1] - c[1] - d.startY, 1, 1) }
            d.sync();
            return false
        }
    },
    ...
    hideLabels: function() { if (this.getHideLabelInGesture()) { this.eachInteractiveAxes(function(a) { a.hideLabels() }) } },
    showLabels: function() { if (this.getHideLabelInGesture()) { this.eachInteractiveAxes(function(a) { a.showLabels() }) } },
    isEventOnAxis: function(c, a) { var b = a.getSurface().getRect(); return b[0] <= c.getX() && c.getX() <= b[0] + b[2] && b[1] <= c.getY() && c.getY() <= b[1] + b[3] },
    getPannableAxes: function(d) {
        var h = this,
            ...
            j = false,
            b;
        if (d) { for (c = 0; c < g; c++) { if (this.isEventOnAxis(d, f[c])) { j = true; break } } }
        for (c = 0; c < g; c++) { b = a[f[c].getPosition()]; if (b && b.allowPan !== false && (!j || this.isEventOnAxis(d, f[c]))) { k.push(f[c]) } }
        return k
    },
    getZoomableAxes: function(f) {
        var j = this,
            ...
            c, k = false,
            b;
        if (f) { for (d = 0; d < h; d++) { if (this.isEventOnAxis(f, g[d])) { k = true; break } } }
        for (d = 0; d < h; d++) {
            c = g[d];
            b = a[c.getPosition()];
            if (b && b.allowZoom !== false && (!k || this.isEventOnAxis(f, c))) { l.push(c) }
        }
        return l
    },
    eachInteractiveAxes: function(c) {
        ...
        for (var a = 0; a < e.length; a++) { if (b[e[a].getPosition()]) { if (false === c.call(this, e[a])) { return } } }
    },
    transformAxesBy: function(d, j, g, h, e) {
        ...
        if (!b) {
            ...
            this.eachInteractiveAxes(function(i) { b[i.getId()] = i.getVisibleRange() })
        }
        if (!f) { return }
...
Ext.define("Ext.chart.interactions.Rotate", {
    ...
    config: { gesture: "rotate", gestures: { rotate: "onRotate", rotateend: "onRotate", dragstart: "onGestureStart", drag: "onGesture", dragend: "onGestureEnd" }, rotation: 0 },
    oldRotations: null,
    getAngle: function(f) {
        var c = this,
            b = c.getChart(),
    ...
    getRadius: function(a) { return this.getChart().getRadius() },
    ...
    onGesture: function(b) {
        ...
        if (a.getLocks().drag === a) { a.doRotateTo(c, true); return false }
    },
...
Ext.define("Ext.chart.interactions.RotatePie3D", {
    ...
    getRadius: function(j) {
        var f = this.getChart(),
            ...
            c = 0,
            b, g;
        for (; c < h; c++) { b = d[c]; if (b.is3D) { g = b.getRadius(); if (g > a) { a = g } } }
        return a
    }
});
Ext.define("Ext.chart.plugin.ItemEvents", {
    ...
    init: function(b) {
        var a = "handleEvent";
        this.chart = b;
        b.addElementListener({ click: a, dblclick: a, mousedown: a, mousemove: a, mouseup: a, mouseover: a, mouseout: a, priority: 1001, scope: this })
    },
    hasItemMouseMoveListeners: function() {
        var b = this.chart.hasListeners,
            a;
        for (a in this.itemMouseMoveEvents) { if (a in b) { return true } }
        return false
    },
    handleEvent: function(g) {
        var d = this,
            ...
            c = d.lastItem,
            f, b;
        if (h && !d.hasItemMouseMoveListeners() && !d.moveEvents) { return }
        f = a.getEventXY(g);
...
Ext.define("Ext.chart.series.Cartesian", {
    ...
    fieldCategoryX: ["X"],
    fieldCategoryY: ["Y"],
    applyXAxis: function(a, b) { return this.getChart().getAxis(a) || b },
    applyYAxis: function(a, b) { return this.getChart().getAxis(a) || b },
    updateXAxis: function(a) { a.processData(this) },
    updateYAxis: function(a) { a.processData(this) },
    coordinateX: function() { return this.coordinate("X", 0, 2) },
    coordinateY: function() { return this.coordinate("Y", 1, 2) },
    getItemForPoint: function(a, g) {
        if (this.getSprites()) {
            var f = this,
                ...
                b = f.getStore(),
                e, c;
            if (f.getHidden()) { return null }
            if (d) { c = d.getIndexNearPoint(a, g); if (c !== -1) { e = { series: f, category: f.getItemInstancing() ? "items" : "markers", index: c, record: b.getData().items[c], field: f.getYField(), sprite: d }; return e } }
        }
    },
    createSprite: function() {
        var c = this,
        ...
        if (a.setAggregator && d && d.getAggregator) { if (d.getAggregator) { a.setAggregator({ strategy: d.getAggregator() }) } else { a.setAggregator({}) } }
        return a
    },
    getSprites: function() {
        var d = this,
            ...
            a;
        if (!c) { return [] }
        if (!f.length) { a = d.createSprite() } else { a = f[0] }
        if (e) {
            if (b) { a.itemsMarker.getTemplate().setAnimation(e) }
            a.setAnimation(e)
        }
        return f
    },
    provideLegendInfo: function(d) {
        var b = this,
            ...
            c = a.fillStyle;
        if (Ext.isArray(c)) { c = c[0] }
        d.push({ name: b.getTitle() || b.getYField() || b.getId(), mark: (Ext.isObject(c) ? c.stops && c.stops[0].color : c) || a.strokeStyle || "black", disabled: b.getHidden(), series: b.getId(), index: 0 })
    },
    getXRange: function() { return [this.dataRange[0], this.dataRange[2]] },
    getYRange: function() { return [this.dataRange[1], this.dataRange[3]] }
});
Ext.define("Ext.chart.series.StackedCartesian", {
    ...
    updateStacked: function() { this.processData() },
    updateSplitStacks: function() { this.processData() },
    coordinateY: function() { return this.coordinateStacked("Y", 1, 2) },
    coordinateStacked: function(D, e, m) {
        ...
        for (w = 0; w < n.length; w++) {
            ...
            for (v = 0; v < H; v++) { if (!x[v]) { q[v] = F.coordinateData(r, s[v], c) } }
            if (A && z) {
                y = [];
                if (a) { b = [] }
                for (v = 0; v < B; v++) {
                    ...
                    for (u = 0; u < H; u++) {
                        ...
                        if (!G) { continue }
                        G = G[v];
                        if (G >= 0 || !a) { y[v] += G } else { if (G < 0) { b[v] += G } }
                    }
                }
            }
            for (v = 0; v < H; v++) {
        ...
        t["dataMin" + D] = p.min;
        t["dataMax" + D] = p.max;
        for (w = 0; w < g.length; w++) { g[w].setAttributes(t) }
    },
    getFields: function(f) {
        var e = this,
            a = [],
    ...
    updateLabelOverflowPadding: function(a) { this.getLabel().setAttributes({ labelOverflowPadding: a }) },
    getSprites: function() {
        var k = this,
            j = k.getChart(),
        ...
        for (d = 0; d < a; d++) {
            ...
            if (!l) {
                ...
                g = true;
                e.push(false);
                if (b) { l.itemsMarker.getTemplate().setAttributes(k.getStyleByIndex(d)) } else { l.setAttributes(k.getStyleByIndex(d)) }
            }
            if (c) {
                if (b) { l.itemsMarker.getTemplate().setAnimation(c) }
                l.setAnimation(c)
            }
        }
        if (g) { k.updateHidden(e) }
        return h
    },
        ...
        if (this.getSprites()) {
            ...
            for (b = 0, g = f.length; b < g; b++) {
                if (!c[b]) {
                    ...
                    if (d !== -1) {
                        e = h.getYField();
                        n = { series: h, index: d, category: a ? "items" : "markers", record: l.getData().items[d], field: typeof e === "string" ? e : e[b], sprite: m };
                        return n
                    }
                }
            }
    ...
    provideLegendInfo: function(e) {
        ...
        for (c = 0; c < f.length; c++) {
            ...
            l = b.fillStyle;
            if (h) { if (Ext.isArray(h)) { a = h[c] } else { if (k) { a = h } } }
            if (!h || !a) { if (Ext.isArray(j)) { a = j[c] } else { a = g.getId() } }
            e.push({ name: a, mark: (Ext.isObject(l) ? l.stops && l.stops[0].color : l) || b.strokeStyle || "black", disabled: d[c], series: g.getId(), index: c })
        }
    },
    onSpriteAnimationStart: function(a) { this.spriteAnimationCount++; if (this.spriteAnimationCount === 1) { this.fireEvent("animationstart") } },
    onSpriteAnimationEnd: function(a) { this.spriteAnimationCount--; if (this.spriteAnimationCount === 0) { this.fireEvent("animationend") } }
});
Ext.define("Ext.chart.series.sprite.Series", { extend: "Ext.draw.sprite.Sprite", mixins: { markerHolder: "Ext.chart.MarkerHolder" }, inheritableStatics: { def: { processors: { dataMinX: "number", dataMaxX: "number", dataMinY: "number", dataMaxY: "number", rangeX: "data", rangeY: "data", dataX: "data", dataY: "data" }, defaults: { dataMinX: 0, dataMaxX: 1, dataMinY: 0, dataMaxY: 1, rangeX: null, rangeY: null, dataX: null, dataY: null }, triggers: { dataX: "bbox", dataY: "bbox", dataMinX: "bbox", dataMaxX: "bbox", dataMinY: "bbox", dataMaxY: "bbox" } } }, config: { store: null, series: null, field: null } });
Ext.define("Ext.chart.series.sprite.Cartesian", {
    extend: "Ext.chart.series.sprite.Series",
    inheritableStatics: {
        def: {
            processors: { labels: "default", labelOverflowPadding: "number", selectionTolerance: "number", flipXY: "bool", renderer: "default", visibleMinX: "number", visibleMinY: "number", visibleMaxX: "number", visibleMaxY: "number", innerWidth: "number", innerHeight: "number" },
            defaults: { labels: null, labelOverflowPadding: 10, selectionTolerance: 20, flipXY: false, renderer: null, transformFillStroke: false, visibleMinX: 0, visibleMinY: 0, visibleMaxX: 1, visibleMaxY: 1, innerWidth: 1, innerHeight: 1 },
            triggers: { dataX: "dataX,bbox", dataY: "dataY,bbox", visibleMinX: "panzoom", visibleMinY: "panzoom", visibleMaxX: "panzoom", visibleMaxY: "panzoom", innerWidth: "panzoom", innerHeight: "panzoom" },
            updaters: {
                dataX: function(a) {
                    this.processDataX();
                    this.scheduleUpdater(a, "dataY", ["dataY"])
                },
                dataY: function() { this.processDataY() },
                panzoom: function(c) {
                    ...
                    if (f && !c.flipXY) { c.translationX = b + c.visibleMinX * b / e } else { c.translationX = -c.visibleMinX * b / e }
    ...
    binarySearch: function(d) {
        var b = this.attr.dataX,
            ...
            a = b.length;
        if (d <= b[0]) { return f }
        if (d >= b[a - 1]) { return a - 1 }
        while (f + 1 < a) {
            var c = (f + a) >> 1,
                e = b[c];
            if (e === d) { return c } else { if (e < d) { f = c } else { a = c } }
        }
        return f
    },
    render: function(b, c, g) {
        ...
        if (a.dataX === null || a.dataX === undefined) { return }
        if (a.dataY === null || a.dataY === undefined) { return }
        if (e.getXX() * e.getYX() || e.getXY() * e.getYY()) { console.log("Cartesian Series sprite does not support rotation/sheering"); return }
...
Ext.define("Ext.chart.series.sprite.StackedCartesian", {
    ...
    inheritableStatics: { def: { processors: { groupCount: "number", groupOffset: "number", dataStartY: "data" }, defaults: { selectionTolerance: 20, groupCount: 1, groupOffset: 0, dataStartY: null }, triggers: { dataStartY: "dataY,bbox" } } },
...
Ext.define("Ext.chart.series.sprite.Area", {
    ...
    renderClipped: function(w, A, G) {
        ...
        if (v.transformFillStroke) { v.matrix.toContext(A) }
        A.fill();
        if (v.transformFillStroke) { v.inverseMatrix.toContext(A) }
        A.beginPath();
        A.moveTo(F, E);
        if (v.step) {
            for (D = c; D <= b; D++) {
...
Ext.define("Ext.chart.series.Area", { extend: "Ext.chart.series.StackedCartesian", alias: "series.area", type: "area", seriesType: "areaSeries", requires: ["Ext.chart.series.sprite.Area"], config: { splitStacks: false } });
Ext.define("Ext.chart.series.sprite.Bar", {
    alias: "sprite.barSeries",
    extend: "Ext.chart.series.sprite.StackedCartesian",
    inheritableStatics: { def: { processors: { minBarWidth: "number", maxBarWidth: "number", minGapWidth: "number", radius: "number", inGroupGapWidth: "number" }, defaults: { minBarWidth: 2, maxBarWidth: 100, minGapWidth: 5, inGroupGapWidth: 3, radius: 0 } } },
    drawLabel: function(k, i, v, h, r) {
        var t = this,
            ...
            n = (o === "horizontal" && q.flipXY) || (o === "vertical" && !q.flipXY) || !o,
            b = e.getCalloutLine(),
            g, m, a, p, u, w, s;
        l.x = d.x(i, h);
        l.y = d.y(i, h);
        if (b) { p = b.length } else { p = 0 }
        if (!q.flipXY) { l.rotationRads = -Math.PI * 0.5 } else { l.rotationRads = 0 }
        ...
        if (e.attr.renderer) {
            ...
            if (typeof u === "string") { l.text = u } else {
                if (typeof u === "object") {
                    if ("text" in u) { l.text = u.text }
                    w = true
                }
            }
        }
        ...
        if (p > 0) { m = p } else { if (p === 0) { m = (n ? a.width : a.height) / 2 } else { m = (n ? a.width : a.height) / 2 + j } }
        if (v > h) { m = -m }
        if (n) { g = (c === "insideStart") ? v + m : h - m } else { g = (c === "insideStart") ? v + j * 2 : h - j * 2 }
        l.x = d.x(i, g);
        l.y = d.y(i, g);
        g = (c === "insideStart") ? v : h;
        ...
        if (b) { if (b.width) { l.calloutWidth = b.width } } else { l.calloutColor = "none" }
        if (v > h) { m = -m }
        if (Math.abs(h - v) <= m * 2 || c === "outside") { l.callout = 1 } else { l.callout = 0 }
        if (w) { Ext.apply(l, u) }
        t.putMarker("labels", l, r)
    },
    drawBar: function(l, b, h, c, g, k, a, d) {
        ...
        if (e) {
            i = Ext.callback(e, null, [f, j, { store: f.getStore() }, d], 0, f.getSeries());
            Ext.apply(j, i)
        }
    ...
    renderClipped: function(F, u, C) {
        ...
        for (J = y; J <= p; J++) {
            ...
            if (f && G[J] != null) { q.drawLabel(G[J], a, b, h, J) }
            q.putMarker("markers", { translationX: c.x(a, h), translationY: c.y(a, h) }, J, true)
        }
    },
    getIndexNearPoint: function(l, k) {
        var m = this,
            ...
            j = b[3],
            e, d, c, n, f = -1;
        if (g.flipXY) { e = j - k; if (a.getInherited().rtl) { d = b[2] - l } else { d = l } } else {
            e = l;
            d = j - k
        }
        for (c = 0; c < h.length; c++) { n = m.getMarkerBBox("items", c); if (Ext.draw.Draw.isPointInBBox(e, d, n)) { f = c; break } }
        return f
    }
});
Ext.define("Ext.chart.series.Bar", {
    ...
    updateStacked: function(c) {
        var e = this,
            ...
            a = {},
            b;
        for (b = 0; b < d; b++) { if (!g[b].attr.hidden) { f.push(g[b]) } }
        d = f.length;
        if (e.getStacked()) {
            a.groupCount = 1;
            a.groupOffset = 0;
            for (b = 0; b < d; b++) { f[b].setAttributes(a) }
        } else {
...
Ext.define("Ext.chart.series.sprite.Bar3D", {
    ...
    inheritableStatics: {
        def: {
            processors: { depthWidthRatio: "number", saturationFactor: "number", brightnessFactor: "number", colorSpread: "number" },
            defaults: { depthWidthRatio: 1 / 3, saturationFactor: 1, brightnessFactor: 1, colorSpread: 1, transformFillStroke: true },
...
Ext.define("Ext.chart.series.sprite.Box", {
    ...
    alias: "sprite.box",
    type: "box",
    inheritableStatics: { def: { processors: { x: "number", y: "number", width: "number", height: "number", depth: "number", orientation: "enums(vertical,horizontal)", showStroke: "bool", saturationFactor: "number", brightnessFactor: "number", colorSpread: "number" }, triggers: { x: "bbox", y: "bbox", width: "bbox", height: "bbox", depth: "bbox", orientation: "bbox" }, defaults: { x: 0, y: 0, width: 8, height: 8, depth: 8, orientation: "vertical", showStroke: false, saturationFactor: 1, brightnessFactor: 1, colorSpread: 1, lineJoin: "bevel" } } },
    constructor: function(a) {
        this.callParent([a]);
    ...
    render: function(l, m) {
        var u = this,
            ...
            a = {},
            s, q, p;
        if (!k.showStroke) { m.strokeStyle = Ext.util.Color.RGBA_NONE }
        if (i) {
            p = j;
            j = f;
            f = p
        ...
        u.topGradient.setStops([{ offset: 0, color: Ext.util.Color.fromHSV(b[0], Ext.Number.constrain(b[1] * h, 0, 1), Ext.Number.constrain((0.5 + t * 0.1) * o, 0, 1)) }, { offset: 1, color: Ext.util.Color.fromHSV(b[0], Ext.Number.constrain(b[1] * h, 0, 1), Ext.Number.constrain((0.5 - t * 0.11) * o, 0, 1)) }]);
        u.rightGradient.setDegrees(d ? 45 : 90);
        u.rightGradient.setStops([{ offset: 0, color: Ext.util.Color.fromHSV(b[0], Ext.Number.constrain(b[1] * h, 0, 1), Ext.Number.constrain((0.5 - t * 0.14) * o, 0, 1)) }, { offset: 1, color: Ext.util.Color.fromHSV(b[0], Ext.Number.constrain(b[1] * (1 + t * 0.4) * h, 0, 1), Ext.Number.constrain((0.5 - t * 0.32) * o, 0, 1)) }]);
        if (d) { u.frontGradient.setDegrees(0) } else { u.frontGradient.setRadians(Math.atan2(j - f, e * 2)) }
        u.frontGradient.setStops([{ offset: 0, color: Ext.util.Color.fromHSV(b[0], Ext.Number.constrain(b[1] * (1 - t * 0.1) * h, 0, 1), Ext.Number.constrain((0.5 + t * 0.1) * o, 0, 1)) }, { offset: 1, color: Ext.util.Color.fromHSV(b[0], Ext.Number.constrain(b[1] * (1 + t * 0.1) * h, 0, 1), Ext.Number.constrain((0.5 - t * 0.23) * o, 0, 1)) }]);
        if (g || i) {
            m.beginPath();
            m.moveTo(r - e, f);
            m.lineTo(r - e + v, f + v);
...
Ext.define("Ext.chart.series.Bar3D", {
    ...
    is3D: true,
    config: { itemInstancing: { type: "box", fx: { customDurations: { x: 0, y: 0, width: 0, height: 0, depth: 0 } } }, highlightCfg: { opacity: 0.8 } },
    updateXAxis: function(b, a) { this.callParent([b, a]) },
    getSprites: function() {
        ...
        for (a = 0; a < c.length; a++) {
            b = c[a];
            d = b.attr.zIndex;
            if (d < 0) { b.setAttributes({ zIndex: -d }) }
            if (b.setSeries) { b.setSeries(this) }
        }
        return c
    },
    getDepth: function() { var a = this.getSprites()[0]; return a ? (a.depth || 0) : 0 },
        ...
        if (this.getSprites()) {
            ...
            for (b = h.length - 1; b >= 0; b--) {
                if (!c[b]) {
                    ...
                    if (d !== -1) {
                        e = j.getYField();
                        p = { series: j, index: d, category: a ? "items" : "markers", record: n.getData().items[d], field: typeof e === "string" ? e : e[b], sprite: o };
                        return p
                    }
                }
            }
...
Ext.define("Ext.draw.LimitedCache", {
    config: { limit: 40, feeder: function() { return 0 }, scope: null },
    cache: null,
    constructor: function(a) {
        this.cache = {};
        this.cache.list = [];
    ...
    get: function(e) {
        ...
        if (c[e]) { return c[e].value }
        if (c.list[c.tail]) { delete c[c.list[c.tail].cacheId] }
        c[e] = c.list[c.tail] = { value: a.apply(d, Array.prototype.slice.call(arguments, 1)), cacheId: e };
        c.tail++;
        if (c.tail === b) { c.tail = 0 }
        return c[e].value
    },
    clear: function() {
        this.cache = {};
...
Ext.define("Ext.draw.SegmentTree", {
    ...
    time: function(m, l, n, c, E, d, e) {
        ...
        for (z = 0; l > f + 1 && z < u.length; z++) {
            ...
            b = u[z];
            s = D.align(s, b[0], b[1]);
            if (D.diff(s, x, b[0]) > n.length * 2 * b[1]) { continue }
    ...
    aggregateData: function(h, a, r, c, d) {
        var b = h.length,
            ...
            m = [],
            k = [],
            g = { startIdx: e, endIdx: s, minIdx: f, maxIdx: q, open: j, minX: p, minY: n, maxX: o, maxY: m, close: k },
            l;
        for (l = 0; l < b; l++) {
            e[l] = l;
            s[l] = l;
        ...
        g.map = { original: [0, b] };
        if (b) { this[this.getStrategy()](g, b, h, a, r, c, d) }
        return g
    },
    binarySearchMin: function(c, g, a, e) {
        var b = this.dataX;
        if (e <= b[c.startIdx[0]]) { return g }
        if (e >= b[c.startIdx[a - 1]]) { return a - 1 }
        while (g + 1 < a) {
            var d = (g + a) >> 1,
                f = b[c.startIdx[d]];
            if (f === e) { return d } else { if (f < e) { g = d } else { a = d } }
        }
        return g
    },
    binarySearchMax: function(c, g, a, e) {
        var b = this.dataX;
        if (e <= b[c.endIdx[0]]) { return g }
        if (e >= b[c.endIdx[a - 1]]) { return a - 1 }
        while (g + 1 < a) {
            var d = (g + a) >> 1,
                f = b[c.endIdx[d]];
            if (f === e) { return d } else { if (f < e) { g = d } else { a = d } }
    ...
    setData: function(d, a, b, c, e) {
        ...
        this.dataLow = c;
        this.dataClose = e;
        if (d.length === b.length && d.length === c.length) { this.cache = this.aggregateData(d, a, b, c, e) }
    },
...
Ext.define("Ext.chart.series.sprite.Aggregative", {
    ...
    inheritableStatics: { def: { processors: { dataHigh: "data", dataLow: "data", dataClose: "data" }, aliases: { dataOpen: "dataY" }, defaults: { dataHigh: null, dataLow: null, dataClose: null } } },
    config: { aggregator: {} },
    applyAggregator: function(b, a) { return Ext.factory(b, Ext.draw.SegmentTree, a) },
    ...
    processDataY: function() {
        ...
        if (b.dataX && c && c.length > 0) { f = d.getAggregator(); if (e) { f.setData(b.dataX, b.dataY, e, a, g) } else { f.setData(b.dataX, b.dataY) } }
    },
    getGapWidth: function() { return 1 },
...
Ext.define("Ext.chart.series.CandleStick", { extend: "Ext.chart.series.Cartesian", requires: ["Ext.chart.series.sprite.CandleStick"], alias: "series.candlestick", type: "candlestick", seriesType: "candlestickSeries", config: { openField: null, highField: null, lowField: null, closeField: null }, fieldCategoryY: ["Open", "High", "Low", "Close"], themeColorCount: function() { return 2 } });
Ext.define("Ext.chart.series.Polar", {
    extend: "Ext.chart.series.Series",
    config: { rotation: 0, radius: null, center: [0, 0], offsetX: 0, offsetY: 0, showInLegend: true, xField: null, yField: null, angleField: null, radiusField: null, xAxis: null, yAxis: null },
    directions: ["X", "Y"],
    fieldCategoryX: ["X"],
    fieldCategoryY: ["Y"],
    ...
    constructor: function(b) {
        var c = this,
            ...
            e = a.configs,
            d;
        if (b) { for (d in c.deprecatedConfigs) { if (d in b && !(b in e)) { Ext.raise("'" + d + "' config has been deprecated. Please use the '" + c.deprecatedConfigs[d] + "' config instead.") } } }
        c.callParent([b])
    },
    getXField: function() { return this.getAngleField() },
    updateXField: function(a) { this.setAngleField(a) },
    getYField: function() { return this.getRadiusField() },
    updateYField: function(a) { this.setRadiusField(a) },
    applyXAxis: function(a, b) { return this.getChart().getAxis(a) || b },
    applyYAxis: function(a, b) { return this.getChart().getAxis(a) || b },
    getXRange: function() { return [this.dataRange[0], this.dataRange[2]] },
    getYRange: function() { return [this.dataRange[1], this.dataRange[3]] },
    ...
    getDefaultSpriteConfig: function() { return { type: this.seriesType, renderer: this.getRenderer(), centerX: 0, centerY: 0, rotationCenterX: 0, rotationCenterY: 0 } },
    applyRotation: function(a) { return Ext.draw.sprite.AttributeParser.angle(a) },
    updateRotation: function(a) { var b = this.getSprites(); if (b && b[0]) { b[0].setAttributes({ baseRotation: a }) } }
});
Ext.define("Ext.chart.series.Gauge", {
    ...
    config: { needle: false, needleLength: 90, needleWidth: 4, donut: 30, showInLegend: false, value: null, colors: null, sectors: null, minimum: 0, maximum: 100, rotation: 0, totalAngle: Math.PI / 2, rect: [0, 0, 1, 1], center: [0.5, 0.75], radius: 0.5, wholeDisk: false },
    coordinateX: function() { return this.coordinate("X", 0, 2) },
    coordinateY: function() { return this.coordinate("Y", 1, 2) },
    updateNeedle: function(b) {
        ...
        if (d && d.length) {
            d[0].setAttributes({ startAngle: (b ? c : 0), endAngle: c, strokeOpacity: (b ? 1 : 0), lineWidth: (b ? a.getNeedleWidth() : 0) });
            a.doUpdateStyles()
        }
    },
    themeColorCount: function() {
    ...
    updateColors: function(a, b) {
        var f = this,
            ...
            d;
        if (!g || !a[0]) { return }
        for (d = 0; d < j; d++) { c[d + 1] = h[d].color || c[d + 1] || a[d % g] }
        if (e.length) { e[0].setAttributes({ strokeStyle: c[0] }) }
    ...
    valueToAngle: function(a) { a = this.applyValue(a); return this.getTotalAngle() * (a - this.getMinimum()) / (this.getMaximum() - this.getMinimum()) },
    applyValue: function(a) { return Math.min(this.getMaximum(), Math.max(a, this.getMinimum())) },
    ...
    processData: function() {
        var f = this,
            ...
            c, i;
        if (e) { c = f.getXField(); if (c) { i = e.get(c) } }
        if (a = f.getXAxis()) {
            d = a.getMinimum();
            h = a.getMaximum();
            b = a.getSprites()[0].fx;
            g = b.getDuration();
            b.setDuration(0);
            if (Ext.isNumber(d)) { f.setMinimum(d) } else { a.setMinimum(f.getMinimum()) }
            if (Ext.isNumber(h)) { f.setMaximum(h) } else { a.setMaximum(f.getMaximum()) }
            b.setDuration(g)
        }
        if (!Ext.isNumber(i)) { i = f.getMinimum() }
        f.setValue(i)
    },
    getDefaultSpriteConfig: function() { return { type: this.seriesType, renderer: this.getRenderer(), fx: { customDurations: { translationX: 0, translationY: 0, rotationCenterX: 0, rotationCenterY: 0, centerX: 0, centerY: 0, startRho: 0, endRho: 0, baseRotation: 0 } } } },
    normalizeSectors: function(f) {
        ...
        if (c) {
            for (b = 0; b < c; b++) {
                e = f[b];
                if (typeof e === "number") { f[b] = { start: (b > 0 ? f[b - 1].end : d.getMinimum()), end: Math.min(e, d.getMaximum()) }; if (b == (c - 1) && f[b].end < d.getMaximum()) { f[b + 1] = { start: f[b].end, end: d.getMaximum() } } } else {
                    if (typeof e.start === "number") { g = Math.max(e.start, d.getMinimum()) } else { g = (b > 0 ? f[b - 1].end : d.getMinimum()) }
                    if (typeof e.end === "number") { a = Math.min(e.end, d.getMaximum()) } else { a = d.getMaximum() }
                    f[b].start = g;
                    f[b].end = a
                }
            }
        } else { f = [{ start: d.getMinimum(), end: d.getMaximum() }] }
        return f
    },
    getSprites: function() {
        var j = this,
            ...
            l = j.getValue(),
            c, g;
        if (!m && !Ext.isNumber(l)) { return [] }
        var h = j.getChart(),
            b = j.getAnimation() || h && h.getAnimation(),
            f = j.sprites,
            k = 0,
            o, n, e, d, a = [];
        if (f && f.length) { f[0].setAnimation(b); return f }
        d = { store: m, field: j.getXField(), angleField: j.getXField(), value: l, series: j };
        o = j.createSprite();
        ...
        for (c = 0, g = n.length; c < g; c++) {
            e = { startAngle: j.valueToAngle(n[c].start), endAngle: j.valueToAngle(n[c].end), label: n[c].label, fillStyle: n[c].color, strokeOpacity: 0, doCallout: false, labelOverflowPadding: -1 };
...
Ext.define("Ext.chart.series.sprite.Line", {
    ...
    inheritableStatics: {
        def: {
            processors: { smooth: "bool", fillArea: "bool", step: "bool", nullStyle: "enums(gap,connect,origin)", preciseStroke: "bool", xAxis: "default", yCap: "default" },
            defaults: { smooth: false, nullStyle: "connect", fillArea: false, step: false, preciseStroke: true, xAxis: null, yCap: Math.pow(2, 20), yJump: 50 },
    ...
    drawStrip: function(a, c) { a.moveTo(c[0], c[1]); for (var b = 2, d = c.length; b < d; b += 2) { a.lineTo(c[b], c[b + 1]) } },
    ...
    drawSmoothStroke: function(u, v, c, b, C, f) {
        var G = this,
            ...
            o, F, n, E, h, g, B, a, A, w, H, D, l = { type: "line", smooth: true, step: d };
        v.beginPath();
    ...
    drawLabel: function(k, i, h, o, a) {
        ...
        m.x = c.x(i, h);
        m.y = c.y(i, h);
        if (n.flipXY) { m.rotationRads = Math.PI * 0.5 } else { m.rotationRads = 0 }
        m.text = k;
        if (d.attr.renderer) {
            p = [k, e, m, q.rendererData, o];
            r = Ext.callback(d.attr.renderer, null, p, 0, q.getSeries());
            if (typeof r === "string") { m.text = r } else {
                if (typeof r === "object") {
                    if ("text" in r) { m.text = r.text }
                    s = true
                }
            }
        }
    ...
    drawStroke: function(a, c, h, b, f, e) {
        ...
        if (g) { d.drawSmoothStroke(a, c, h, b, f, e) } else { d.drawStraightStroke(a, c, h, b, f, e) }
    },
        ...
        for (V = A; V < l; V++) {
            ...
            if (S < r) {
                ...
                if (S > r) {
                    s.push(o ? (r * E + c) : null, m ? (p * f + b) : null, Y[V]);
                    s.push(N ? (S * E + c) : null, L ? (Q * f + b) : null, Y[V])
                } else { s.push(o ? (r * E + c) : null, m ? (p * f + b) : null, Y[V]) }
            }
        }
        if (s.length) {
            for (V = 0; V < s.length; V += 3) {
                ...
                if (Ext.isNumber(P) && Ext.isNumber(O)) {
                    if (O > a) { O = a } else { if (O < -a) { O = -a } }
                    s[V + 1] = O
                } else { X = false; continue }
                I = s[V + 2];
                if (v) { n.drawMarker(P, O, I) }
                if (d && h[I]) { n.drawLabel(h[I], P, O, I, F) }
            }
            n.isContinuousLine = X;
            if (g && !X) { Ext.raise("Line smoothing in only supported for gapless data, where all data points are finite numbers.") }
            if (z) { W = z.getAlignment() === "vertical"; if (Ext.isNumber(z.floatingAtCoord)) { U = (W ? F[2] : F[3]) - z.floatingAtCoord } else { U = W ? F[0] : F[1] } } else { U = k.flipXY ? F[0] : F[1] }
            if (k.preciseStroke) {
                if (k.fillArea) { q.fill() }
                if (k.transformFillStroke) { k.inverseMatrix.toContext(q) }
                n.drawStroke(R, q, A, l, s, U);
                if (k.transformFillStroke) { k.matrix.toContext(q) }
                q.stroke()
            } else {
                n.drawStroke(R, q, A, l, s, U);
                ...
                if (k.transformFillStroke) { k.matrix.toContext(q) }
                if (k.fillArea) { q.fillStroke(k, true) } else { q.stroke(true) }
            }
        }
    }
});
Ext.define("Ext.chart.series.Line", {
    ...
    config: { selectionTolerance: 20, smooth: false, step: false, nullStyle: "gap", fill: undefined, aggregator: { strategy: "double" } },
    defaultSmoothness: 3,
    overflowBuffer: 1,
    themeMarkerCount: function() { return 1 },
        ...
        if (typeof d.config.fill != "undefined") {
            if (d.config.fill) {
                ...
                if (typeof c.fillStyle == "undefined") {
                    ...
                    } else { c.fillStyle = c.strokeStyle }
                }
            }
        } else { if (c.fillStyle) { a = true } }
        if (!a) { delete c.fillStyle }
        c = Ext.apply(e || {}, c);
        return Ext.apply(c, { fillArea: a, step: d.config.step, smooth: d.config.smooth, selectionTolerance: d.config.selectionTolerance })
    },
    updateStep: function(b) { var a = this.getSprites()[0]; if (a && a.attr.step !== b) { a.setAttributes({ step: b }) } },
    updateFill: function(b) { var a = this.getSprites()[0]; if (a && a.attr.fillArea !== b) { a.setAttributes({ fillArea: b }) } },
    updateSmooth: function(a) { var b = this.getSprites()[0]; if (b && b.attr.smooth !== a) { b.setAttributes({ smooth: a }) } },
    updateNullStyle: function(a) { var b = this.getSprites()[0]; if (b && b.attr.nullStyle !== a) { b.setAttributes({ nullStyle: a }) } }
});
Ext.define("Ext.chart.series.sprite.PieSlice", {
    extend: "Ext.draw.sprite.Sector",
    mixins: { markerHolder: "Ext.chart.MarkerHolder" },
    alias: "sprite.pieslice",
    inheritableStatics: { def: { processors: { doCallout: "bool", label: "string", rotateLabels: "bool", labelOverflowPadding: "number", renderer: "default" }, defaults: { doCallout: true, rotateLabels: true, label: "", labelOverflowPadding: 10, renderer: null } } },
    ...
    render: function(b, c, f) {
        var e = this,
            ...
            d;
        if (a.renderer) {
            g = { type: "sector", text: a.text, centerX: a.centerX, centerY: a.centerY, margin: a.margin, startAngle: Math.min(a.startAngle, a.endAngle), endAngle: Math.max(a.startAngle, a.endAngle), startRho: Math.min(a.startRho, a.endRho), endRho: Math.max(a.startRho, a.endRho) };
            d = Ext.callback(a.renderer, null, [e, g, e.getRendererData(), e.getRendererIndex()], 0, e.getSeries());
            e.setAttributes(d);
            e.useAttributes(c, f)
        }
        e.callParent([b, c, f]);
        if (a.label && e.getMarker("labels")) { e.placeLabel() }
    },
    placeLabel: function() {
        var z = this,
            s = z.attr,
        ...
        if (!s.rotateLabels) { o.rotationRads = 0 } else {
        ...
        if (a) { if (a.width) { o.calloutWidth = a.width } } else { o.calloutColor = "none" }
        o.globalAlpha = s.globalAlpha * s.fillOpacity;
        if (d.display !== "none") { o.hidden = (s.startAngle == s.endAngle) }
        if (d.attr.renderer) {
            w = [z.attr.label, e, o, z.getRendererData(), z.getRendererIndex()];
            A = Ext.callback(d.attr.renderer, null, w, 0, z.getSeries());
            if (typeof A === "string") { o.text = A } else { Ext.apply(o, A) }
        }
        z.putMarker("labels", o, r);
        u = z.getMarkerBBox("labels", r, true);
        if (u) { if (s.doCallout) { if (d.attr.display === "outside") { z.putMarker("labels", { callout: 1 }, r) } else { if (d.attr.display === "inside") { z.putMarker("labels", { callout: 0 }, r) } else { z.putMarker("labels", { callout: 1 - z.sliceContainsLabel(s, u) }, r) } } } else { z.putMarker("labels", { globalAlpha: z.sliceContainsLabel(s, u) }, r) } }
    },
    sliceContainsLabel: function(d, f) {
        var e = d.labelOverflowPadding,
            ...
            j, c, b, a;
        if (e < 0) { return 1 }
        if (f.width + e * 2 > (d.endRho - d.startRho)) { return 0 }
        ...
        if (f.height + e * 2 > Math.min(c, b, a) * 2) { return 0 }
        return 1
    }
});
Ext.define("Ext.chart.series.Pie", {
    ...
    seriesType: "pieslice",
    isPie: true,
    config: { donut: 0, rotation: 0, clockwise: true, totalAngle: 2 * Math.PI, hidden: [], radiusFactor: 100, highlightCfg: { margin: 20 }, style: {} },
    directions: ["X"],
    applyLabel: function(a, b) { if (Ext.isObject(a) && !Ext.isString(a.orientation)) { Ext.apply(a = Ext.Object.chain(a), { orientation: "vertical" }) } return this.callParent([a, b]) },
    updateLabelData: function() {
        var h = this,
            j = h.getStore(),
            g = j.getData().items,
        ...
        if (e.length && a) {
            c = [];
            for (b = 0, f = g.length; b < f; b++) { c.push(g[b].get(a)) }
    ...
    coordinateX: function() {
        ...
        for (q = 0; q < c; q++) {
            ...
            if (!p[q]) { a += l; if (k > t) { t = k } }
            d[q] = a;
            if (q >= p.length) { p[q] = false }
        }
        p.length = c;
        u.maxY = t;
        if (a !== 0) { m = h / a }
        for (q = 0; q < c; q++) { j[q].setAttributes({ startAngle: g, endAngle: g = (m ? s * d[q] * m : 0), globalAlpha: 1 }) }
        ...
        for (q = c; q < u.sprites.length; q++) { j[q].setAttributes({ startAngle: h, endAngle: h, globalAlpha: 0 }) }
        n = u.getChart();
        if (!n.isConfiguring) { n.refreshLegendStore() }
    },
    updateCenter: function(a) {
    ...
    updateTotalAngle: function(a) { this.processData() },
    getSprites: function() {
        var k = this,
            h = k.getChart(),
            n = k.getStore();
        ...
        var j = n.getData().items,
            ...
            a = m.getTemplate(),
            e, f;
        f = { store: n, field: k.getXField(), angleField: k.getXField(), radiusField: k.getYField(), series: k };
        for (e = 0; e < b; e++) {
            o = g[e];
            if (!o) {
                o = k.createSprite();
    ...
    getItemByIndex: function(a, b) { b = b || "sprites"; return this.callParent([a, b]) },
    getItemForAngle: function(a) {
        ...
        if (f) {
            var j = h.getStore(),
                ...
                b = 0,
                e = j.getCount();
            for (; b < e; b++) { if (!c[b]) { d = f[b].attr; if (d.startAngle <= a && d.endAngle >= a) { return { series: h, sprite: f[b], index: b, record: g[b], field: h.getXField() } } } }
        }
        return null
    },
    getItemForPoint: function(f, e) {
        ...
        if (c) {
            var s = t.getCenter(),
                ...
                m = t.getHidden(),
                n, d, k;
            for (n = 0, d = o.length; n < d; n++) { if (!m[n]) { k = c[n].attr; if (a >= l + k.margin && a <= k.endRho + k.margin) { if (t.betweenAngle(r, k.startAngle, k.endAngle)) { return { series: t, sprite: c[n], index: n, record: o[n], field: t.getXField() } } } } }
            return null
        }
    },
    provideLegendInfo: function(f) {
        ...
        if (j) {
            ...
            for (d = 0; d < g.length; d++) {
                ...
                k = a.fillStyle;
                if (Ext.isObject(k)) { k = k.stops && k.stops[0].color }
                f.push({ name: b ? String(g[d].get(b)) : c + " " + d, mark: k || a.strokeStyle || "black", disabled: e[d], series: h.getId(), index: d })
            }
        }
    }
});
Ext.define("Ext.chart.series.sprite.Pie3DPart", {
    ...
    partColorUpdater: function(a) {
        ...
        switch (a.part) {
            case "top":
                c = new Ext.draw.gradient.Radial({ start: { x: 0, y: 0, r: 0 }, end: { x: 0, y: 0, r: 1 }, stops: [{ offset: 0, color: d.createLighter(0.1 * e) }, { offset: 1, color: d.createDarker(0.1 * e) }] });
                break;
            case "bottom":
                c = new Ext.draw.gradient.Radial({ start: { x: 0, y: 0, r: 0 }, end: { x: 0, y: 0, r: 1 }, stops: [{ offset: 0, color: d.createDarker(0.2 * e) }, { offset: 1, color: d.toString() }] });
                break;
            case "outerFront":
            case "outerBack":
                c = new Ext.draw.gradient.Linear({ stops: [{ offset: 0, color: d.createDarker(0.15 * e).toString() }, { offset: 0.3, color: b }, { offset: 0.8, color: d.createLighter(0.2 * e).toString() }, { offset: 1, color: d.createDarker(0.25 * e).toString() }] });
                break;
            case "start":
                c = new Ext.draw.gradient.Linear({ stops: [{ offset: 0, color: d.createDarker(0.1 * e).toString() }, { offset: 1, color: d.createLighter(0.2 * e).toString() }] });
                break;
            case "end":
                c = new Ext.draw.gradient.Linear({ stops: [{ offset: 0, color: d.createDarker(0.1 * e).toString() }, { offset: 1, color: d.createLighter(0.2 * e).toString() }] });
                break;
            case "innerFront":
            case "innerBack":
                c = new Ext.draw.gradient.Linear({ stops: [{ offset: 0, color: d.createDarker(0.1 * e).toString() }, { offset: 0.2, color: d.createLighter(0.2 * e).toString() }, { offset: 0.7, color: b }, { offset: 1, color: d.createDarker(0.1 * e).toString() }] });
                break
        }
        a.fillStyle = c;
    ...
    partZIndexUpdater: function(a) {
        ...
        switch (a.part) {
            ...
            case "outerFront":
                d = c(d + e);
                b = c(b + e);
                if (d >= 0 && b < 0) { f = Math.sin(d) } else { if (d <= 0 && b > 0) { f = Math.sin(b) } else { if (d >= 0 && b > 0) { if (d > b) { f = 0 } else { f = Math.max(Math.sin(d), Math.sin(b)) } } else { f = 1 } } }
                a.zIndex = 4 + f;
                break;
            case "outerBack":
                a.zIndex = 1;
    ...
    updatePlainBBox: function(k) {
        var f = this.attr,
            ...
            d = f.centerY,
            j, c, i, h, g, l;
        if (a === "start") { c = f.startAngle + b } else { if (a === "end") { c = f.endAngle + b } }
        if (Ext.isNumber(c)) {
            g = Math.sin(c);
            l = Math.cos(c);
        ...
        if (a === "innerFront" || a === "innerBack") { j = f.startRho } else { j = f.endRho }
        k.width = j * 2;
        k.height = j * f.distortion * 2 + f.thickness;
        k.x = f.centerX - j;
        k.y = f.centerY - j * f.distortion
    },
    updateTransformedBBox: function(a) { if (this.attr.part === "start" || this.attr.part === "end") { return this.callParent(arguments) } return this.updatePlainBBox(a) },
    updatePath: function(a) {
        if (!this.attr.globalAlpha) { return }
    ...
    render: function(b, c) {
        var d = this,
            a = d.attr;
        if (!a.globalAlpha || Ext.Number.isEqual(a.startAngle, a.endAngle, 1e-8)) { return }
        d.callParent([b, c]);
        d.bevelRenderer(b, c);
        if (a.label && d.getMarker("labels")) { d.placeLabel() }
    },
    placeLabel: function() {
        var z = this,
            u = z.attr,
    ...
    topRenderer: function(a) { this.lidRenderer(a, 0) },
    bottomRenderer: function(c) {
        var a = this.attr,
            b = Ext.util.Color.RGBA_NONE;
        if (a.globalAlpha < 1 || a.fillOpacity < 1 || a.shadowColor !== b) { this.lidRenderer(c, a.thickness) }
    },
    ...
    startRenderer: function(a) { this.sideRenderer(a, "start") },
    endRenderer: function(a) { this.sideRenderer(a, "end") },
                    ...
                    if (u <= 0 && r > 0) { f() } else {
                        if (u <= 0 && r < 0) {
                            ...
                            if (u > r) {
                                ...
                                if (c && !o || n && o) { w.bevelParams.push(v) }
                                q.ellipse.apply(q, v);
    ...
    innerFrontRenderer: function(a) { this.rimRenderer(a, this.attr.startRho, true, true) },
    innerBackRenderer: function(a) { this.rimRenderer(a, this.attr.startRho, true, false) },
    outerFrontRenderer: function(a) { this.rimRenderer(a, this.attr.endRho, false, true) },
    outerBackRenderer: function(a) { this.rimRenderer(a, this.attr.endRho, false, false) }
});
Ext.define("Ext.chart.series.Pie3D", {
    ...
    alias: "series.pie3d",
    is3D: true,
    config: { rect: [0, 0, 0, 0], thickness: 35, distortion: 0.5, donut: false, hidden: [], highlightCfg: { margin: 20 }, shadow: false },
    rotationOffset: -Math.PI / 2,
    setField: function(a) { return this.setXField(a) },
    getField: function() { return this.getXField() },
    updateRotation: function(a) {
        this.setStyle({ baseRotation: a + this.rotationOffset });
        this.doUpdateStyles()
    },
    updateDistortion: function() { this.setRadius() },
    updateThickness: function() { this.setRadius() },
    updateColors: function(a) { this.setSubStyle({ baseColor: a }) },
    applyShadow: function(a) { if (a === true) { a = { shadowColor: "rgba(0,0,0,0.8)", shadowBlur: 30 } } else { if (!Ext.isObject(a)) { a = { shadowColor: Ext.util.Color.RGBA_NONE } } } return a },
    updateShadow: function(g) {
        var e = this,
            ...
            c = f && f.length,
            b, a;
        for (b = 1; b < c; b += d) { a = f[b]; if (a.attr.part = "bottom") { a.setAttributes(g) } }
    },
    getStyleByIndex: function(b) {
    ...
    doUpdateStyles: function() {
        var g = this,
            ...
            b = 0,
            a, d;
        for (; c < e; c += f, b++) { d = g.getStyleByIndex(b); for (a = 0; a < f; a++) { h[c + a].setAttributes(d) } }
    },
    coordinateX: function() {
        var w = this,
            m = w.getChart(),
        ...
        for (r = 0; r < d; r++) {
            ...
            if (!s[r]) { c += n }
            e[r] = c;
            if (r >= s.length) { s[r] = false }
        }
        s.length = d;
        if (c === 0) { return }
        h = 2 * Math.PI / c;
        for (r = 0; r < d; r++) { e[r] *= h }
        for (r = 0; r < a; r++) { k[r].setAnimation(u) }
        for (r = 0; r < d; r++) {
            for (q = 0; q < l; q++) { k[r * l + q].setAttributes({ startAngle: g, endAngle: e[r] - v, globalAlpha: 1, baseRotation: p }) }
            g = e[r]
        }
        for (r *= l; r < a; r++) {
            k[r].setAnimation(u);
            k[r].setAttributes({ startAngle: o, endAngle: o, globalAlpha: 0, baseRotation: p })
        }
    },
    updateLabelData: function() {
        var l = this,
        ...
        if (h.length && b) {
            e = [];
            for (d = 0, g = k.length; d < g; d++) { e.push(k[d].get(b)) }
    ...
    applyRadius: function() {
        var f = this,
            ...
            g = c / 2,
            b = g * f.getDistortion();
        if (b > a / 2) { return a / (f.getDistortion() * 2) } else { return g }
    },
    getSprites: function() {
        var y = this,
            e = y.getStore();
        ...
        var n = y.getChart(),
            ...
            q = y.getRotation(),
            d = y.getHighlight(),
            c = { centerX: x[0] + w, centerY: x[1] + u - y.getThickness() / 2, endRho: b, startRho: b * y.getDonut() / 100, thickness: y.getThickness(), distortion: y.getDistortion() },
            k = y.sprites,
            h = y.getLabel(),
            f = h.getTemplate(),
            m, g, o, s, r;
        for (s = 0; s < a; s++) {
            ...
            if (!k[s * l]) {
                for (r = 0; r < y.partNames.length; r++) {
                    ...
                    if (r === 0 && f.getField()) { o.bindMarker("labels", h) }
        ...
        if (g) {
            ...
            for (c = 0, f = b.length; c < f; c++) {
                if (!e[c]) {
                    d = c * a;
                    m = g[d];
                    if (m.hitTest([k, j])) { return { series: h, sprite: g.slice(d, d + a), index: c, record: b[c], category: "sprites", field: h.getXField() } }
                }
            }
            return null
        }
    ...
    provideLegendInfo: function(f) {
        ...
        if (k) {
            ...
            for (d = 0; d < g.length; d++) {
                a = h.getStyleByIndex(d);
                c = a.baseColor;
                f.push({ name: b ? String(g[d].get(b)) : j + " " + d, mark: c || "black", disabled: e[d], series: h.getId(), index: d })
            }
        }
    }
}, function() {
...
Ext.define("Ext.chart.series.sprite.Polar", {
    extend: "Ext.chart.series.sprite.Series",
    inheritableStatics: { def: { processors: { centerX: "number", centerY: "number", startAngle: "number", endAngle: "number", startRho: "number", endRho: "number", baseRotation: "number", labels: "default", labelOverflowPadding: "number" }, defaults: { centerX: 0, centerY: 0, startAngle: 0, endAngle: Math.PI, startRho: 0, endRho: 150, baseRotation: 0, labels: null, labelOverflowPadding: 10 }, triggers: { centerX: "bbox", centerY: "bbox", startAngle: "bbox", endAngle: "bbox", startRho: "bbox", endRho: "bbox", baseRotation: "bbox" } } },
    updatePlainBBox: function(b) {
        var a = this.attr;
...
Ext.define("Ext.chart.series.sprite.Radar", {
    ...
    getDataPointXY: function(d) {
        var u = this,
            ...
            g = n.baseRotation,
            i, h, m, c, b, a, q;
        if (n.rangeY) { q = n.rangeY[1] } else { q = n.dataMaxY }
...
Ext.define("Ext.chart.series.Radar", {
    ...
    isStoreDependantColorCount: false,
    themeMarkerCount: function() { return 1 },
    updateAngularAxis: function(a) { a.processData(this) },
    updateRadialAxis: function(a) { a.processData(this) },
    coordinateX: function() { return this.coordinate("X", 0, 2) },
    coordinateY: function() { return this.coordinate("Y", 1, 2) },
    updateCenter: function(a) {
        this.setStyle({ translationX: a[0] + this.getOffsetX(), translationY: a[1] + this.getOffsetY() });
    ...
    getItemForPoint: function(k, j) {
        ...
        if (m && e) {
            ...
            for (d = 0; d < a; d++) {
                ...
                if (Math.abs(p[0] - k) < b && Math.abs(p[1] - j) < b) { o = { series: h, sprite: m, index: d, category: "markers", record: l.getData().items[d], field: h.getYField() }; return o }
            }
        }
    ...
    getDefaultSpriteConfig: function() {
        var a = this.callParent(),
            b = { customDurations: { translationX: 0, translationY: 0, rotationRads: 0, dataMinX: 0, dataMaxX: 0 } };
        if (a.fx) { Ext.apply(a.fx, b) } else { a.fx = b }
        return a
    },
    getSprites: function() {
        var d = this,
            ...
            a;
        if (!c) { return [] }
        if (!b) { b = d.createSprite() }
        if (e) {
            a = b.getMarker("markers");
            if (a) { a.getTemplate().setAnimation(e) }
            b.setAnimation(e)
        }
        return d.sprites
    },
...
Ext.define("Ext.chart.series.sprite.Scatter", {
    ...
    renderClipped: function(r, s, w, u) {
        ...
        for (v = 0; v < l.length; v++) {
            ...
            if (a <= g && g <= A && o <= f && f <= e) {
                ...
                if (b && z[v]) { C.drawLabel(z[v], g, f, v, u) }
            }
        }
    },
    drawLabel: function(j, h, g, p, a) {
        ...
        if (o) { l.rotationRads = Math.PI * 0.5 } else { l.rotationRads = 0 }
        k = n.height / 2;
        f = h;
        ...
        if (c.attr.renderer) {
            ...
            if (typeof s === "string") { l.text = s } else { Ext.apply(l, s) }
        }
        r.putMarker("labels", l, p)
    }
});
...
Ext.define("Ext.draw.Point", {
    ...
    constructor: function(a, c) {
        var b = this;
        if (typeof a === "number") { b.x = a; if (typeof c === "number") { b.y = c } else { b.y = a } } else {
            if (Ext.isArray(a)) {
                b.x = a[0];
                b.y = a[1]
            } else {
    ...
    calculateCartesian: function() {
        var b = this,
            a = b.length,
            c = b.angle;
        if (b.angleUnits === "degrees") { c = Ext.draw.Draw.rad(c) }
        b.x = Math.cos(c) * a;
        b.y = Math.sin(c) * a
    },
    calculatePolar: function() {
        ...
        if (b.angleUnits === "degrees") { b.angle = Ext.draw.Draw.degrees(b.angle) }
    },
    setX: function(a) {
        this.x = a;
        this.calculatePolar()
    ...
    set: function(a, b) { this.constructor(a, b) },
    setAngle: function(a) {
        this.angle = a;
    ...
    clone: function() { return new Ext.draw.Point(this.x, this.y) },
    add: function(a, c) { var b = Ext.draw.Point.fly(a, c); return new Ext.draw.Point(this.x + b.x, this.y + b.y) },
    sub: function(a, c) { var b = Ext.draw.Point.fly(a, c); return new Ext.draw.Point(this.x - b.x, this.y - b.y) },
    mul: function(a) { return new Ext.draw.Point(this.x * a, this.y * a) },
    div: function(a) { return new Ext.draw.Point(this.x / a, this.y / a) },
    dot: function(a, c) { var b = Ext.draw.Point.fly(a, c); return this.x * b.x + this.y * b.y },
    equals: function(a, c) { var b = Ext.draw.Point.fly(a, c); return this.x === b.x && this.y === b.y },
    rotate: function(f, c) {
        var d, e, b, g, a;
    ...
    transform: function(a) { if (a && a.isMatrix) { return new Ext.draw.Point(a.transformPoint(this)) } else { if (arguments.length === 6) { return new Ext.draw.Point(Ext.draw.Matrix.fly(arguments).transformPoint(this)) } else { Ext.raise("Invalid parameters.") } } },
    round: function() { return new Ext.draw.Point(Math.round(this.x), Math.round(this.y)) },
    ceil: function() { return new Ext.draw.Point(Math.ceil(this.x), Math.ceil(this.y)) },
    floor: function() { return new Ext.draw.Point(Math.floor(this.x), Math.floor(this.y)) },
    abs: function(a, b) { return new Ext.draw.Point(Math.abs(this.x), Math.abs(this.y)) },
    ...
    isNumber: function() { return Ext.isNumber(this.x) && Ext.isNumber(this.y) }
});
Ext.define("Ext.draw.plugin.SpriteEvents", {
    ...
    init: function(a) {
        ...
        a.addElementListener({ click: b, dblclick: b, mousedown: b, mousemove: b, mouseup: b, mouseover: b, mouseout: b, priority: 1001, scope: this })
    },
    hasSpriteMouseMoveListeners: function() {
        var b = this.drawContainer.hasListeners,
            a;
        for (a in this.spriteMouseMoveEvents) { if (a in b) { return true } }
        return false
    },
    ...
    handleEvent: function(f) {
        var d = this,
            ...
            a = d.lastSprite,
            c;
        if (g && !d.hasSpriteMouseMoveListeners()) { return }
        c = d.hitTestEvent(f);
        if (g && !Ext.Object.equals(c, a)) { if (a) { b.fireEvent("spritemouseout", a, f) } if (c) { b.fireEvent("spritemouseover", c, f) } }
        if (c) { b.fireEvent("sprite" + f.type, c, f) }
        d.lastSprite = c
    }
});
Ext.define("Ext.chart.interactions.ItemInfo", {
    ...
    config: { extjsGestures: { start: { event: "click", handler: "onInfoGesture" }, move: { event: "mousemove", handler: "onInfoGesture" }, end: { event: "mouseleave", handler: "onInfoGesture" } } },
    item: null,
    onInfoGesture: function(f, a) {
        var c = this,
            b = c.getItemForEvent(f),
            d = b && b.series.tooltip;
        if (d) { d.onMouseMove.call(d, f) }
        if (b !== c.item) {
            if (b) { b.series.showTip(b) } else { c.item.series.hideTip(c.item) }
            c.item = b
        }
        return false
    }
