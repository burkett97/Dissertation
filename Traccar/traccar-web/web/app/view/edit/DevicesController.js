...
Ext.define('Traccar.view.edit.DevicesController', {
    ...
    onLocateClick: function() {
        ...
        var device, devicephone;
        // From selected device get phone number and pass to contactlocator.py to send a location request to the device
        device = this.getView().getSelectionModel().getSelection()[0];
        deviceid = device.get('id');
        devicephone = device.get('phone');

        const Http = new XMLHttpRequest();
        const url = 'http://127.0.0.1:5000/locate/' + devicephone + "/" + deviceid;
        Http.open("GET", url);
        Http.send();
    },
