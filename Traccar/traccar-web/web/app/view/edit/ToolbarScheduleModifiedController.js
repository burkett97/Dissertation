 ...
 */
Ext.define('Traccar.view.edit.ToolbarScheduleModifiedController', {
    extend: 'Ext.app.ViewController',
    alias: 'controller.toolbarschedulemodifiedController',
    ...
    onRemoveClick: function() {
        var schedule = this.getView().getSelectionModel().getSelection()[0];
        var scheduleid = schedule.get('id');
        var userid = Traccar.app.getUser().get('id');

        Ext.Msg.confirm("Schedule", "Remove item", function(btn) {
            if (btn === 'yes') {
                const Http = new XMLHttpRequest();
                const url = 'http://127.0.0.1:5000/removeschedule/' + scheduleid + "?" + "id=" + userid;
                Http.open("GET", url);
                Http.send();
